...
public class RootDockGroupTests : VerifyBase, IDisposable
{
    ...
    [TestMethod]
    public void RemoveGroup_ShouldNotRemoveLeftOrItsTray()
    {
        // Arrange
        using var dock = new SimpleDock();
        this.sut.DockLeft(dock);
        var left = this.sut.First;
        _ = left.Should().NotBeNull();
        var leftTray = left?.First;
        _ = leftTray.Should().NotBeNull();

        // Act
        this.sut.RemoveGroup(leftTray!);
        this.sut.RemoveGroup(left!);

        // Assert
        _ = this.sut.First.Should().NotBeNull();
        _ = this.sut.First?.First.Should().NotBeNull();
    }

    [TestMethod]
    public void RemoveGroup_ShouldNotRemoveTopOrItsTray()
    {
        // Arrange
        using var dock = new SimpleDock();
        this.sut.DockTop(dock);
        var top = this.sut.First;
        _ = top.Should().NotBeNull();
        var topTray = top?.First;
        _ = topTray.Should().NotBeNull();

        // Act
        this.sut.RemoveGroup(topTray!);
        this.sut.RemoveGroup(top!);
    ...
    [TestMethod]
    public void RemoveGroup_ShouldNotRemoveRightOrItsTray()
    {
        // Arrange
        using var dock = new SimpleDock();
        this.sut.DockRight(dock);
        var right = this.sut.Second;
        _ = right.Should().NotBeNull();
        var rightTray = right?.Second;
        _ = rightTray.Should().NotBeNull();

        // Act
        this.sut.RemoveGroup(rightTray!);
        this.sut.RemoveGroup(right!);

        // Assert
        _ = this.sut.Second.Should().NotBeNull();
        _ = this.sut.Second?.Second.Should().NotBeNull();
    }

    [TestMethod]
    public void RemoveGroup_ShouldNotRemoveBottomOrItsTray()
    {
        // Arrange
        using var dock = new SimpleDock();
        this.sut.DockBottom(dock);
        var bottom = this.sut.Second;
        _ = bottom.Should().NotBeNull();
        var bottomTray = bottom?.Second;
        _ = bottomTray.Should().NotBeNull();

        // Act
        this.sut.RemoveGroup(bottomTray!);
        this.sut.RemoveGroup(bottom!);
