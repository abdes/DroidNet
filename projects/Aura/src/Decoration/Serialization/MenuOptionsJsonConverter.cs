...
/// persisted to JSON. The actual MenuSource is not serializable and is
/// resolved at runtime from the DI container using the provider ID.
/// </para>
/// <para>
...
public sealed class MenuOptionsJsonConverter : JsonConverter<MenuOptions>
{
    ...
    /// <param name="options">Serializer options.</param>
    /// <remarks>
    /// Only the menu provider ID and compact mode flag are serialized. The menu source
    /// is not persisted as it must be resolved from the DI container at runtime.
    /// </remarks>
