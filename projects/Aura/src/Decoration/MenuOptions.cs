...
/// The menu provider is resolved from the dependency injection container using the
/// <see cref="MenuProviderId"/>.
/// </para>
/// <para>
/// Menu options are serialized to JSON using a custom converter that stores only the provider ID
/// and compact mode setting. The actual menu provider is resolved at runtime from the DI container.
/// </para>
/// <para>
/// Use <see langword="null"/> for the Menu property in WindowDecorationOptions
/// to indicate that no menu should be displayed.
/// </para>
/// </remarks>
/// <example>
/// <code>
...
public sealed record MenuOptions
{
    ...
    /// </value>
    /// <remarks>
    /// The provider ID is used to resolve a menu provider from the dependency
    /// injection container. If the provider is not found, a warning is logged and the window
    /// is created without a menu (graceful degradation).
    /// </remarks>
    ...
    /// </value>
    /// <remarks>
    /// Compact mode is useful for tool windows or secondary windows where vertical space is limited.
    /// The exact visual appearance of compact mode depends on the menu implementation.
    /// </remarks>
