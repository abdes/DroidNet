...
{
    /// <summary>
    /// Generates <see cref="IRouterState" /> from a <see cref="IUrlTree" /> by
    /// attempting to match each node in the tree to the routes in the router
    /// config.
    /// </summary>
    /// <param name="urlTree">
    /// The <see cref="IUrlTree" /> from which to generate the new router state.
    /// </param>
    /// <returns>
    ...
    /// </remarks>
    IRouterState CreateFromUrlTree(IUrlTree urlTree);
}
