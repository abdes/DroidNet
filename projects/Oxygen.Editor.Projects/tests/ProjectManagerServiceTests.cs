...
public partial class ProjectManagerServiceTests : TestSuiteWithAssertions
{
    ...
    public async Task LoadProjectAsync_ShouldReturnTrue_WhenProjectIsLoadedSuccessfully()
    {
        ...
        _ = sceneDocumentMock.Setup(d => d.Name).Returns("scene1.oscene.json");
    ...
    public async Task LoadProjectAsync_ShouldReturnFalse_WhenProjectScenesFailToLoad()
    {
        ...
        var projectInfo = new ProjectInfo("name", Category.Games, "valid/path", "Media/Preview.png");
        var projectFolderMock = new Mock<IFolder>();
        var contentFolderMock = new Mock<IFolder>();

        _ = this.mockStorage.Setup(s => s.GetFolderFromPathAsync(It.IsAny<string>(), CancellationToken.None))
            .ReturnsAsync(projectFolderMock.Object);
        _ = projectFolderMock.Setup(f => f.GetFolderAsync(Constants.ContentFolderName, CancellationToken.None))
            .ReturnsAsync(contentFolderMock.Object);
        _ = contentFolderMock.Setup(f => f.ExistsAsync()).ReturnsAsync(value: true);
        _ = contentFolderMock.Setup(f => f.GetFolderAsync(Constants.ScenesFolderName, CancellationToken.None))
    ...
    private static void SetupScenesFolder(Mock<IFolder> projectFolderMock, Mock<IFolder> scenesFolderMock)
    {
        var contentFolderMock = new Mock<IFolder>();
        _ = projectFolderMock.Setup(f => f.GetFolderAsync(Constants.ContentFolderName, CancellationToken.None))
            .ReturnsAsync(contentFolderMock.Object);
        ...
        _ = scenesFolderMock.Setup(f => f.ExistsAsync()).ReturnsAsync(value: true);
    }
}
