...
public partial class TabStrip : Control
{
    ...
    internal IEnumerable<(UIElement element, int index)> GetRealizedRegularElements()
        => this.regularItemsRepeater is not null
            ? GetRealizedElementsWithIndex(this.regularItemsRepeater)
            : [];

    internal IEnumerable<(UIElement element, int index)> GetRealizedPinnedElements()
        => this.pinnedItemsRepeater is not null
            ? GetRealizedElementsWithIndex(this.pinnedItemsRepeater)
            : [];
    ...
    internal FrameworkElement? GetTabStripItemForItem(TabItem draggedItem)
    {
        TabStripItem? FindInRepeater(ItemsRepeater? repeater)
        {
            if (repeater != null)
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    if (wrapper?.DataContext is TabItem ti && ReferenceEquals(ti, draggedItem))
                    {
        ...
        // Look first in the regular items repeater
        var result = FindInRepeater(this.regularItemsRepeater);
        if (result != null)
        {
            return result;
        }

        // if not found, look in the pinned items repeater
        return FindInRepeater(this.pinnedItemsRepeater);
    }
