...
/// }
/// ]]></code>
/// <para><strong>XAML for the View:</strong></para>
...
public abstract partial class DynamicTreeViewModel : ObservableObject
{
    ...
    private async Task RemoveSingleSelectedItem()
    {
        ...
        Debug.Assert(singleSelection is not null, $"{nameof(this.RemoveSingleSelectedItem)} should only be used in single selection mode");
    ...
    private async Task RemoveMultipleSelectionItems()
    {
        ...
        Debug.Assert(multipleSelection is not null, $"{nameof(this.RemoveMultipleSelectionItems)} should only be used in multiple selection mode");
    ...
    /// <summary>
    /// Removes the children of the specified item from the item and from the <see cref="ShownItems"/> if the item is expanded.
    /// </summary>
    ...
    /// <returns>A task representing the asynchronous operation, with the final insert index after all children have been processed.</returns>
    /// <remarks>
