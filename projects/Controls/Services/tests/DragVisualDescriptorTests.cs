...
public class DragVisualDescriptorTests : VisualUserInterfaceTests
{
    ...
    public void Descriptor_CanBeCreated_WithDefaultValues()
    {
        ...
        _ = descriptor.Title.Should().BeNull("Title should be null by default");
    ...
    /// <summary>
    /// Verifies that Title property can be set and retrieved.
    /// </summary>
    [TestMethod]
    public void Descriptor_Title_CanBeSet()
    {
        // Arrange
        var descriptor = new DragVisualDescriptor();
        const string expectedTitle = "Test Tab";

        // Act
        descriptor.Title = expectedTitle;

        // Assert
        _ = descriptor.Title.Should().Be(expectedTitle, "Title should be set correctly");
    }
    ...
    /// <summary>
    /// Verifies that PropertyChanged event is raised when Title changes.
    /// </summary>
    [TestMethod]
    public void Descriptor_RaisesPropertyChanged_WhenTitleChanges()
    {
        // Arrange
        var descriptor = new DragVisualDescriptor();
        var eventRaised = false;
        ...
        {
            propertyName = e.PropertyName;
            if (string.Equals(e.PropertyName, nameof(DragVisualDescriptor.Title), StringComparison.Ordinal))
            {
                eventRaised = true;
            }
        };
        ...
        // Assert
        _ = eventRaised.Should().BeTrue("PropertyChanged should be raised for Title");
        _ = propertyName.Should().Be(nameof(DragVisualDescriptor.Title), "Property name should match");
    }
