...
public sealed class GltfImporter : IAssetImporter
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            var extracted = await TryExtractTextureAsync(
                context,
                model.LogicalTextures[i],
                i,
                sourcePath,
                lastWriteTimeUtc,
                sourceHash,
            ...
            if (extracted != null)
            {
                texturePaths[extracted.Index] = extracted.VirtualPath;
            }
        }
    ...
    private static async Task<ExtractedAssetPath?> TryExtractTextureAsync(
        ImportContext context,
        Texture tex,
        int textureIndex,
        string sourcePath,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var texName = string.Create(CultureInfo.InvariantCulture, $"{stem}__texture__{textureIndex:0000}");
        var virtualPath = "/" + Filesystem.VirtualPath.Combine(dir, texName + ".otex");
        var intermediatePath = GetImportedIntermediatePath(virtualPath, AssetPipelineConstants.PngExtension);

        var (width, height) = await WriteExtractedTexturePngAsync(context, img, intermediatePath, cancellationToken).ConfigureAwait(false);
        ...
        return new ExtractedAssetPath(textureIndex, virtualPath);
    }

    private static async Task<(int width, int height)> WriteExtractedTexturePngAsync(
        ImportContext context,
        SharpGLTF.Schema2.Image img,
        ...
        CancellationToken cancellationToken)
    {
        ...
        await context.Files.WriteAllBytesAsync(intermediatePath, ms.ToArray(), cancellationToken).ConfigureAwait(false);
    ...
    private static async Task<Dictionary<int, AssetKey>> ExtractMaterialsAsync(
        ...
        string dir,
        string stem,
        IReadOnlyDictionary<int, string> texturePaths,
        IReadOnlyDictionary<string, string> settings,
        CancellationToken cancellationToken)
    {
    ...
    private static async Task<Dictionary<int, string>> ExtractMeshesAsync(
        ...
        string dir,
        string stem,
        IReadOnlyDictionary<int, AssetKey> materialKeys,
        CancellationToken cancellationToken)
    {
        ...
        for (var meshIndex = 0; meshIndex < model.LogicalMeshes.Count; meshIndex++)
        {
            ...
            var extracted = await TryExtractMeshAsync(
                context,
                mesh,
                sourcePath,
                lastWriteTimeUtc,
            ...
            if (extracted != null)
            {
                meshPaths[extracted.Index] = extracted.VirtualPath;
            }
        }
    ...
    private static async Task<ExtractedAssetPath?> TryExtractMeshAsync(
        ImportContext context,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var intermediatePath = GetImportedIntermediatePath(virtualPath, AssetPipelineConstants.GlbExtension);
        ...
        var generatedSourcePath = virtualPath.TrimStart('/') + ".json";
        await PreserveMaterialAssignmentsAsync(context, generatedSourcePath, subMeshes, cancellationToken).ConfigureAwait(false);
        ...
        return new ExtractedAssetPath(meshIndex, virtualPath);
    }

    private static async Task PreserveMaterialAssignmentsAsync(
        ImportContext context,
        string generatedSourcePath,
        List<ImportedSubMesh> subMeshes,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (oldMeta != null)
            {
                CopyMaterialAssignments(subMeshes, oldMeta.SubMeshes);
            }
        }
        catch (Exception ex) when (ex is JsonException or IOException or UnauthorizedAccessException or NotSupportedException)
        {
            // Invalid existing metadata should not block a fresh import.
        }
    }

    private static void CopyMaterialAssignments(List<ImportedSubMesh> subMeshes, IReadOnlyList<ImportedSubMesh> oldSubMeshes)
    {
        for (var i = 0; i < subMeshes.Count && i < oldSubMeshes.Count; i++)
        {
            subMeshes[i] = subMeshes[i] with { MaterialAssetKey = oldSubMeshes[i].MaterialAssetKey };
        }
    }

    private static string GetImportedIntermediatePath(string virtualPath, string extension)
    {
        var relativePath = virtualPath.TrimStart('/');
        var relativePathWithExtension = Path.ChangeExtension(relativePath, extension);
        return Path.Combine(AssetPipelineConstants.ImportedFolderName, relativePathWithExtension);
    }
    ...
    private static (VertexPositionNormal positionNormal, VertexTexture1 texture) ToGltfVertex(ImportedVertex v)
    {
        var vpn = new VertexPositionNormal(v.Position, v.Normal);
        var vt = new VertexTexture1(v.Texcoord);

        return (positionNormal: vpn, texture: vt);
    }
    ...
    private static async Task ExtractScenesAsync(
        ...
        string dir,
        string stem,
        IReadOnlyDictionary<int, string> meshPaths,
        CancellationToken cancellationToken)
    {
    ...
    private static SceneNodeSource BuildSceneNode(SharpGLTF.Schema2.Node node, IReadOnlyDictionary<int, string> meshPaths)
    {
        string? meshUri = null;
        if (node.Mesh != null)
        {
    ...
    private static MaterialSource BuildMaterial(SharpGLTF.Schema2.Material mat, IReadOnlyDictionary<int, string> texturePaths)
    {
    ...
    private static TextureSlot? GetTextureSlot(MaterialChannel? channel, IReadOnlyDictionary<int, string> texturePaths)
    {
    ...
    private sealed record ExtractedAssetPath(int Index, string VirtualPath);

    private sealed record TextureSlot(string Uri, float Scale, float Strength);
}
