...
{
    private static readonly JsonSerializerOptions GeometryJsonOptions = new() { PropertyNameCaseInsensitive = true };
    ...
    private async Task<GeometryAsset?> ResolveGeometryAsync(string relativePath, Uri uri)
    {
        ...
        {
            var jsonBytes = await File.ReadAllBytesAsync(sourcePath).ConfigureAwait(false);
            var geometry = JsonSerializer.Deserialize<ImportedGeometry>(jsonBytes, GeometryJsonOptions);
            if (geometry != null)
            {
