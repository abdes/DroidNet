...
- [X] Rename `projects/DroidNet.Storage` → `projects/DroidNet.Storage` (single commit) and update references
- [X] Verify build & tests; fix any import/reference breaks

### Phase 2 — Assets scaffold

- [X] Create `projects/Oxygen.Managed.Assets` (single assembly).
...
- [X] Implement initial catalog providers:
  ...
  - `FileSystemAssetCatalog` (enumerates loose assets via `DroidNet.Storage` and ingests changes via watcher/event source).
...
- [~] Implement `Oxygen.Managed.Assets.Import` (real, minimal MVP)
...
- [X] Implement `Oxygen.Managed.Assets.Cook` build step (runtime-ready loose cooked outputs)
...
- [~] Add `Oxygen.Managed.Assets.Tests`:
...
In `Oxygen.Managed.Assets`, we currently use `asset:///{MountPoint}/{Path}` URIs as catalog identities.
...
This means the editor-side pipeline (in `Oxygen.Managed.Assets`) must treat **LooseCookedIndex v1** as the compatibility target.
If we revise the format, it must be versioned and coordinated with runtime.

## Module decomposition

- `DroidNet.Storage` (infrastructure) — filesystem abstraction, path normalization, async IO helpers. Must not depend on `Oxygen.Managed.Assets`.
...
- `Oxygen.Managed.Assets` (editor/tooling, C#, single assembly, organized as namespaces):
  - `Oxygen.Managed.Assets.Model` — canonical editor/tooling DTOs and minimal interfaces:
  ...
  - `Oxygen.Managed.Assets.Persistence` — read/write `container.index.bin` compatible with runtime LooseCookedIndex v1.
  - `Oxygen.Managed.Assets.Validation` — validation routines for cooked outputs (index integrity, SHA-256 verification, invariants).
  - `Oxygen.Managed.Assets.Import` — source importers (editor-only; runtime never imports sources).
  - `Oxygen.Managed.Assets.Cook` — cook pipeline producing cooked descriptors + resource tables/data + `container.index.bin`.
  - `Oxygen.Managed.Assets.Pack` (logical area; can be a namespace) — pack pipeline producing `.pak` while enforcing intra-container deps.

Separate layer:

- `Oxygen.Managed.Assets.Tests` — tests (unit + integration) for the combined assembly and integration scenarios.
...
- No separate `Oxygen.Managed.Assets.Api` assembly is created unless a clear, demonstrated need arises (lightweight consumers that must avoid pulling persistence/IO). If that need appears later, extract the minimal public interfaces and DTOs into a small package.
...
- `Oxygen.Editor.ContentBrowser` — Content Browser UI project (views, viewmodels, UI glue). This is an editor assembly and depends on the `Oxygen.Managed.Assets.*` non-UI modules and on `DroidNet.Storage` (infrastructure).

- Infrastructure (I/O / filesystem abstractions - no UI):
  - `DroidNet.Storage` — file system abstraction, path normalization, async IO helpers. This assembly is infrastructure and must not depend on `Oxygen.Managed.Assets`.
...
subgraph Assets ["Oxygen.Managed.Assets (single assembly)"]
 Model["Oxygen.Managed.Assets.Model\n(types & interfaces)"]
 Persistence["Oxygen.Managed.Assets.Persistence\n(index persistence)"]
 Validation["Oxygen.Managed.Assets.Validation\n(validation routines)"]
 Import["Oxygen.Managed.Assets.Import\n(importers)"]
 CookNs["Oxygen.Managed.Assets.Cook\n(pipeline orchestration)"]
 Annotate["Oxygen.Managed.Assets.Annotate\n(metadata/annotations)"]
 Loose["Oxygen.Managed.Assets.Loose\n(loose cooked helpers)"]
 Pak["Oxygen.Managed.Assets.Pak\n(pak builders/readers)"]
 FileSystem["Oxygen.Managed.Assets.FileSystem\n(IO adapters)"]
 Runtime["Oxygen.Managed.Assets.Runtime\n(PIE adapters)"]
end
...
subgraph TestsLayer ["Tests (QA)"]
 Tests["Oxygen.Managed.Assets.Tests\n(unit + integration)"]
end
