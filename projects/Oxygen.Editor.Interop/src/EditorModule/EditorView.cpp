...
auto EditorView::Config::ResolveExtent() const -> SubPixelExtent
{
  ...
  if (width == 1 && height == 1) {
    LOG_F(WARNING, "View '{}' has no compositing target and is using default 1x1 dimensions. "
                   "This likely indicates a misconfigured view.", name);
  }
...
void EditorView::OnSceneMutation()
{
  ...
  // If we already have an ID, update it. Otherwise register new.
  if (view_id_ != kInvalidViewId) {
    current_context_->frame_context.UpdateView(view_id_, std::move(vc));
  } else {
    view_id_ = current_context_->frame_context.RegisterView(std::move(vc));
  }
}
...
auto EditorView::OnPreRender(engine::Renderer& renderer) -> oxygen::co::Co<>
{
  ...
  if (need_resize) {
    if (auto gfx = graphics_.lock()) {
       ...
       if (color_texture_) graphics::DeferredObjectRelease(color_texture_, reclaimer);
       if (depth_texture_) graphics::DeferredObjectRelease(depth_texture_, reclaimer);
       if (framebuffer_) graphics::DeferredObjectRelease(framebuffer_, reclaimer);
