...
### UI instrumentation and SDK compatibility

`ui_tests=True` requires the full engine and development examples. It selects
ImGui Test Engine in Conan and sets `OXYGEN_BUILD_UI_TESTS`; all configurations
in a tree must agree. Developer `build-tree generate` enables it by default;
use `-UiTests:$false` when generating an uninstrumented tree. Ordinary Conan
consumers retain the disabled default.

Unlike optional executable targets, instrumentation is an ABI choice and cannot
be narrowed during configuration. Platform exports the matching ImGui compile
definitions, and DemoShell exports its UI instrumentation definition so installed
SDK consumers see the same public class layouts as the built libraries.
