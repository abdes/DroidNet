...
function(oxygen_embed_json_schemas)
  set(options)
  set(
    oneValueArgs
    TARGET
  ...
  if(NOT DEFINED x_TARGET OR x_TARGET STREQUAL "")
    message(FATAL_ERROR "oxygen_embed_json_schemas: TARGET is required.")
  endif()
  if(NOT TARGET "${x_TARGET}")
    message(
      FATAL_ERROR
      "oxygen_embed_json_schemas: target `${x_TARGET}` does not exist."
    )
  endif()
  if(NOT DEFINED x_OUTPUT_HEADER OR x_OUTPUT_HEADER STREQUAL "")
    message(FATAL_ERROR "oxygen_embed_json_schemas: OUTPUT_HEADER is required.")
  endif()
  if(NOT DEFINED x_NAMESPACE OR x_NAMESPACE STREQUAL "")
    message(FATAL_ERROR "oxygen_embed_json_schemas: NAMESPACE is required.")
  endif()
  if(NOT DEFINED x_SCHEMAS OR x_SCHEMAS STREQUAL "")
    message(FATAL_ERROR "oxygen_embed_json_schemas: SCHEMAS is required.")
  endif()
  ...
  if(NOT x_CHUNK_SIZE MATCHES "^[1-9][0-9]*$")
    message(
      FATAL_ERROR
      "oxygen_embed_json_schemas: CHUNK_SIZE must be a positive integer."
    )
  endif()
  ...
  if(NOT _schema_arg_mod EQUAL 0)
    message(
      FATAL_ERROR
      "oxygen_embed_json_schemas: SCHEMAS must be pairs of "
  ...
  while(_schema_index LESS _schema_arg_count)
    ...
    if(NOT _schema_symbol MATCHES "^[A-Za-z_][A-Za-z0-9_]*$")
      message(
        FATAL_ERROR
        "oxygen_embed_json_schemas: invalid schema symbol `${_schema_symbol}`. "
    ...
    if(NOT EXISTS "${_schema_file}")
      message(
        FATAL_ERROR
        "oxygen_embed_json_schemas: schema file not found: `${_schema_file}`."
      )
    endif()
  ...
  set(
    _manifest_file
    "${CMAKE_CURRENT_BINARY_DIR}/oxygen_json_schemas_${_generation_id_short}.cmake"
  )

  set(
    _stamp_file
    "${CMAKE_CURRENT_BINARY_DIR}/oxygen_json_schemas_${_generation_id_short}.stamp"
  )
  set(_manifest "# Auto-generated by oxygen_embed_json_schemas\n")
  string(
    APPEND
    _manifest
    "set(OXYGEN_JSON_SCHEMA_OUTPUT_HEADER [==[${_output_header}]==])\n"
    "set(OXYGEN_JSON_SCHEMA_STAMP [==[${_stamp_file}]==])\n"
    "set(OXYGEN_JSON_SCHEMA_NAMESPACE [==[${x_NAMESPACE}]==])\n"
    "set(OXYGEN_JSON_SCHEMA_CHUNK_SIZE [==[${x_CHUNK_SIZE}]==])\n"
    "set(OXYGEN_JSON_SCHEMA_NAMES)\n"
    "set(OXYGEN_JSON_SCHEMA_FILES)\n"
  )
  ...
  foreach(_idx RANGE 0 ${_schema_last_index})
    ...
    string(
      APPEND
      _manifest
      "list(APPEND OXYGEN_JSON_SCHEMA_NAMES [==[${_schema_symbol}]==])\n"
      "list(APPEND OXYGEN_JSON_SCHEMA_FILES [==[${_schema_file}]==])\n"
    )
  endforeach()
  # Keep literal path data intact and preserve the manifest's timestamp on a
  # no-change configure. This file must exist before execute_process below.
  file(WRITE "${_manifest_file}.tmp" "${_manifest}")
  file(COPY_FILE "${_manifest_file}.tmp" "${_manifest_file}" ONLY_IF_DIFFERENT)
  file(REMOVE "${_manifest_file}.tmp")

  set(
    _generator_script
    "${_json_schema_helpers_dir}/GenerateEmbeddedJsonSchemas.cmake"
  )
  # Keep generated headers available to editors immediately after configure.
  # The generator's freshness check also serves the ordered build-time rule.
  execute_process(
    COMMAND
  ...
  # Other targets compiling the same implementation sources need this ordering
  # edge too; an include directory alone cannot provide generation dependencies.
  set_property(
    TARGET
      "${x_TARGET}"
    APPEND
    PROPERTY
      OXYGEN_JSON_SCHEMA_TARGETS
        "${_generation_target}"
  )
