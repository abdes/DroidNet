...
struct VirtualShadowBuildDrawsPassConfig {
  std::string debug_name { "VirtualShadowBuildDrawsPass" };
};

class VirtualShadowBuildDrawsPass : public ComputeRenderPass {
public:
  using Config = VirtualShadowBuildDrawsPassConfig;

  OXGN_RNDR_API VirtualShadowBuildDrawsPass(
    observer_ptr<Graphics> gfx, std::shared_ptr<Config> config);
  OXGN_RNDR_API ~VirtualShadowBuildDrawsPass() override;

  OXYGEN_MAKE_NON_COPYABLE(VirtualShadowBuildDrawsPass)
  OXYGEN_DEFAULT_MOVABLE(VirtualShadowBuildDrawsPass)
...
private:
  ...
  const detail::VirtualShadowScheduleResources* active_schedule_resources_ { nullptr };
