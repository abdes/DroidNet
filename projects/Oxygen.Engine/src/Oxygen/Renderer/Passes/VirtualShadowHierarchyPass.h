...
struct VirtualShadowHierarchyPassConfig {
  std::string debug_name { "VirtualShadowHierarchyPass" };
};

class VirtualShadowHierarchyPass : public ComputeRenderPass {
public:
  using Config = VirtualShadowHierarchyPassConfig;

  OXGN_RNDR_API VirtualShadowHierarchyPass(
    observer_ptr<Graphics> gfx, std::shared_ptr<Config> config);
  OXGN_RNDR_API ~VirtualShadowHierarchyPass() override;

  OXYGEN_MAKE_NON_COPYABLE(VirtualShadowHierarchyPass)
  OXYGEN_DEFAULT_MOVABLE(VirtualShadowHierarchyPass)
...
private:
  ...
  ShaderVisibleIndex active_page_flags_uav_ { kInvalidShaderVisibleIndex };
