...
struct VirtualShadowPageAllocPassConfig {
  std::string debug_name { "VirtualShadowPageAllocPass" };
};

class VirtualShadowPageAllocPass : public ComputeRenderPass {
public:
  using Config = VirtualShadowPageAllocPassConfig;

  OXGN_RNDR_API VirtualShadowPageAllocPass(
    observer_ptr<Graphics> gfx, std::shared_ptr<Config> config);
  OXGN_RNDR_API ~VirtualShadowPageAllocPass() override;

  OXYGEN_MAKE_NON_COPYABLE(VirtualShadowPageAllocPass)
  OXYGEN_DEFAULT_MOVABLE(VirtualShadowPageAllocPass)
