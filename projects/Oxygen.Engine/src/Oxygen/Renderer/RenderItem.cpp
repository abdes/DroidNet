...
glm::vec4 ComputeTransformedBoundingSphere(const RenderItem& item)
{
  if (!item.mesh) {
    return RenderItem::kDefaultBoundingSphere;
  }
  const glm::vec4& local_sphere = item.mesh->BoundingSphere();
  glm::vec3 center_ws = glm::vec3(item.world_transform
...
glm::vec3 ComputeTransformedBoundingBoxMin(const RenderItem& item)
{
  if (!item.mesh) {
...
glm::vec3 ComputeTransformedBoundingBoxMax(const RenderItem& item)
{
  if (!item.mesh) {
    return glm::vec3(0.0f);
  }
...
void UpdateNormalTransform(RenderItem& item)
{
  glm::mat3 world_3x3 = glm::mat3(item.world_transform);
...
void RenderItem::UpdatedTransformedProperties()
{
...
void RenderItem::UpdateComputedProperties()
{
