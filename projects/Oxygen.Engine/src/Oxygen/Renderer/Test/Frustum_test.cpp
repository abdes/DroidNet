...
NOLINT_TEST(Frustum_BasicTest, ExtractPlanes_And_IntersectAabb)
{
  ...
  const ::oxygen::Vec3 bmax(+0.5F);
  EXPECT_TRUE(fr.IntersectsAABB(bmin, bmax));

  // AABB far away beyond far plane should be outside
  const ::oxygen::Vec3 bmin_far(0.0F, 0.0F, -200.0F);
  const ::oxygen::Vec3 bmax_far(1.0F, 1.0F, -150.0F);
...
NOLINT_TEST(Frustum_BasicTest, IntersectSphere_And_ReverseZ)
{
  ...
  EXPECT_TRUE(fr_r.IntersectsSphere(::oxygen::Vec3(0), 0.5F));
  EXPECT_FALSE(fr_r.IntersectsSphere(::oxygen::Vec3(0, 0, -50000.0F), 1.0F));
}
