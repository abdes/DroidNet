...
NOLINT_TEST_F(RenderGraphHarnessFacadeTest, CanFinalizeRequiresRenderGraph)
{
  ...
  facade.SetRenderGraph(
    [](ViewId, const oxygen::engine::RenderContext&,
...
NOLINT_TEST_F(
  RenderGraphHarnessFacadeTest, ExecuteClearsRegisteredPassesBetweenRuns)
{
  auto facade = renderer_->ForRenderGraphHarness();
  facade.SetFrameSession(Renderer::FrameSessionInput {
    .frame_slot = oxygen::frame::Slot { 0U },
  ...
  facade.SetRenderGraph(
    [&execution_count](ViewId, const oxygen::engine::RenderContext& context,
      oxygen::graphics::CommandRecorder&) -> oxygen::co::Co<void> {
      EXPECT_EQ(context.GetPass<DepthPrePass>(), nullptr);
      context.RegisterPass(
        reinterpret_cast<DepthPrePass*>(std::uintptr_t { 0x1000 }));
      ++execution_count;
      co_return;
    });
  ...
  auto recorder = AcquireRecorder("RenderGraphHarnessFacade.RegistryReset");
  ASSERT_NE(recorder, nullptr);
