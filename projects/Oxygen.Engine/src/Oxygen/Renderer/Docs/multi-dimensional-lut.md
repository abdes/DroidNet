...
- `SampleSkyViewLut()` loads `Texture2D<float4>` from bindless heap.
- Uses `GetSkyViewLutUv()` (horizon-aware V mapping) + `ApplyHalfTexelOffset()`.
- Zenith filter does azimuth averaging near zenith using 4 samples.
- Returns `float4(inscatter.rgb, transmittance)`.

### Current GPU Struct (GpuSkyAtmosphereParams)
```
float planet_radius_m;          // +0
float atmosphere_height_m;      // +4
float multi_scattering_factor;  // +8
float aerial_perspective_...;   // +12

float3 ground_albedo_rgb;       // +16
float sun_disk_angular_...;     // +28
...
float _reserved5;                // +116  ← AVAILABLE
float _reserved6;                // +120  ← AVAILABLE
float _reserved7;                // +124  ← AVAILABLE
```
...
- [EnvironmentStaticData.hlsli](src/Oxygen/Graphics/Direct3D12/Shaders/Renderer/EnvironmentStaticData.hlsli)
	...
	- `_reserved7` stays.
- [SkyViewLut_CS.hlsl](src/Oxygen/Graphics/Direct3D12/Shaders/Passes/Atmosphere/SkyViewLut_CS.hlsl)
...
- [SkySphereCommon.hlsli](src/Oxygen/Graphics/Direct3D12/Shaders/Passes/Sky/SkySphereCommon.hlsli)
