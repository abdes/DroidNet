...
namespace {
  ...
  TEST(TokenizerTest, Example)
  {
    ...
    const auto expected = std::vector<Token> {
      Token { TokenType::Chunk, "Why?" }, Token { TokenType::WhiteSpace, " " },
      Token { TokenType::NewLine, "" }, Token { TokenType::Chunk, "Just" },
      Token { TokenType::WhiteSpace, " " }, Token { TokenType::Chunk, "plain" },
      Token { TokenType::WhiteSpace, " " },
      Token { TokenType::Chunk, "finger-" },
      Token { TokenType::Chunk, "licking" },
  ...
  TEST(TokenizerTest, CallsTokenConsumerWhenTokenIsReady)
  {
    ...
    EXPECT_THAT(
      tokens, Eq(std::vector<Token> { Token { TokenType::Chunk, "Hello" } }));
  }
  ...
  {
    const auto& [text, tab, collapse_ws, break_on_hyphens, expected]
      = GetParam();
    const Tokenizer tokenizer { tab, collapse_ws, break_on_hyphens };
