...
//! Different types of tokens that can be produced by this Tokenizer.
...
//! String representation of enum values in `TokenType`.
OXGN_TXW_NDAPI auto to_string(TokenType value) -> const char*;

//! A token is a pair of TokenType and a string representing the token value.
/*!
 The EndOfInput token type will always have an empty string ("") as a value.
*/
using Token = std::pair<TokenType, std::string>;

//! A callback that will be called each time a token is ready to be consumed.
/*!
 ### Usage Example
...
//! Transform a text formed of lines and paragraphs into a stream of typed
...
/*!
 ...
 Chunks are not the same as words; for example when word breaking on hyphens is
 activated, a hyphenated word would be broken into multiple chunks just where
 the hyphens are located.
 ...
 In addition to breaking text into chunks, the Tokenizer is also responsible for
 implementing two specific behaviors prior to the text wrapping/ formatting, and
 which can be controlled by configuration parameters passed to the Tokenizer
 constructor:

 1. **Tab expansion**:

    controlled with the `tab` configuration parameter. All tab characters in the
    text will be replaced with the content of `tab`. For example, to expand tabs
    to spaces, one would specify the `tab` value to be as many spaces as a tab
    character should expand to. To keep tabs as they are, simply specify a `tab`
    value of `\\t`.
 ...
 4. **Break on hyphens**:
    ...
    it is customary in English. If `false`, only white spaces will be considered
    as chunk boundaries.
...
class Tokenizer {
public:
  //! Create a new instance of the Tokenizer class configured with the given
  //! parameters.
  /*!
   ...
   @param collapse_ws controls collapsing of multiple white spaces into a single
   space.
