...
protected:
  ...
  static auto ParseV7(std::string_view text) -> oxygen::Result<oxygen::Uuid>
  {
    return oxygen::Uuid::FromString(text);
  }

  static auto ByteAt(const oxygen::Uuid& uuid, const std::size_t index)
    -> uint8_t
  {
    auto it = oxygen::as_bytes(uuid).begin();
    std::ranges::advance(it, static_cast<std::ptrdiff_t>(index));
    return std::to_integer<uint8_t>(*it);
  }

  static auto VariantBits(const oxygen::Uuid& uuid) -> uint8_t
  {
...
static_assert(sizeof(oxygen::Uuid) == oxygen::Uuid::kSize);
static_assert(std::is_trivially_copyable_v<oxygen::Uuid>);
static_assert(std::is_standard_layout_v<oxygen::Uuid>);
...
NOLINT_TEST_F(UuidV7Test, GenerateIsUniqueAcrossThreads)
{
  ...
  for (auto worker_index = std::size_t { 0U }; worker_index < kThreadCount;
    ++worker_index) {
    workers.emplace_back([&all, &all_mutex]() {
