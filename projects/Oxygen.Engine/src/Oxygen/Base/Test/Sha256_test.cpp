...
[[nodiscard]] auto HexToDigest(std::string_view hex)
...
[[nodiscard]] auto DigestToHex(const oxygen::base::Sha256Digest& digest)
  -> std::string
{
  std::string result;
  result.reserve(64);
...
NOLINT_TEST(ComputeSha256_basic, EmptyInputProducesCorrectDigest)
{
  ...
  // Act
  const auto result = oxygen::base::ComputeSha256({});
...
NOLINT_TEST(ComputeSha256_basic, AbcInputProducesCorrectDigest)
{
  ...
  // Act
  const auto result = oxygen::base::ComputeSha256(ToBytes(input));
...
NOLINT_TEST(ComputeSha256_basic, IsDeterministic)
{
  ...
  // Act
  const auto result1 = oxygen::base::ComputeSha256(ToBytes(input));
  const auto result2 = oxygen::base::ComputeSha256(ToBytes(input));
...
NOLINT_TEST(ComputeSha256_basic, DifferentInputsProduceDifferentDigests)
{
  ...
  // Act
  const auto result1 = oxygen::base::ComputeSha256(ToBytes(input1));
  const auto result2 = oxygen::base::ComputeSha256(ToBytes(input2));
...
{
  // Arrange
  const auto expected = oxygen::base::ComputeSha256({});
...
NOLINT_TEST_F(Sha256StreamingTest, SingleUpdateMatchesOneShot)
{
  ...
  const auto expected = oxygen::base::ComputeSha256(ToBytes(input));
...
NOLINT_TEST_F(Sha256StreamingTest, MultipleUpdatesMatchOneShot)
{
  ...
  const auto expected = oxygen::base::ComputeSha256(ToBytes(full));
...
NOLINT_TEST_F(Sha256StreamingTest, CanReuseAfterFinalize)
{
  ...
  const auto expected1 = oxygen::base::ComputeSha256(ToBytes(input1));
  const auto expected2 = oxygen::base::ComputeSha256(ToBytes(input2));
...
NOLINT_TEST_F(Sha256LargeDataTest, OneMegabyteProducesConsistentResult)
{
  ...
  // Act
  const auto result1 = oxygen::base::ComputeSha256(data);
  const auto result2 = oxygen::base::ComputeSha256(data);

  // Assert
  EXPECT_THAT(result1, ElementsAreArray(result2));
  // Ensure it's not empty or trivial
  EXPECT_FALSE(oxygen::base::IsAllZero(result1));
}
...
NOLINT_TEST_F(Sha256LargeDataTest, StreamingLargeDataMatchesOneShot)
{
  ...
  const auto expected = oxygen::base::ComputeSha256(data);

  // Act - Stream in 4KB chunks
  oxygen::base::Sha256 hasher;
...
{
  // Arrange
  oxygen::base::Sha256Digest zero_digest = {};

  // Act & Assert
  EXPECT_TRUE(oxygen::base::IsAllZero(zero_digest));
}
...
NOLINT_TEST_F(IsAllZeroTest, ReturnsFalseForNonZeroDigest)
{
  ...
  // Act & Assert
  EXPECT_FALSE(oxygen::base::IsAllZero(digest));
}
...
NOLINT_TEST_F(IsAllZeroTest, ReturnsFalseForActualHash)
{
  ...
  const auto digest = oxygen::base::ComputeSha256(ToBytes(input));
...
NOLINT_TEST_F(Sha256FileTest, SmallFileProducesCorrectHash)
{
  ...
  WriteFile(content);
  const auto expected = oxygen::base::ComputeSha256(ToBytes(content));

  // Act
  const auto result = oxygen::base::ComputeFileSha256(temp_file_);
...
{
  // Act
  const bool has_support = oxygen::base::Sha256::HasHardwareSupport();
...
NOLINT_TEST_F(Sha256EdgeCaseTest, SingleByteHash)
{
  ...
  // Act
  const auto result = oxygen::base::ComputeSha256({ &single_byte, 1 });
...
NOLINT_TEST_F(Sha256EdgeCaseTest, NullByteVsFFByte)
{
  ...
  // Act
  const auto result_null = oxygen::base::ComputeSha256({ &null_byte, 1 });
  const auto result_ff = oxygen::base::ComputeSha256({ &ff_byte, 1 });
