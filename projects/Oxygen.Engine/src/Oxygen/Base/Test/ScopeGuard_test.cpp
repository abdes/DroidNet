...
NOLINT_TEST(ScopeGuardTest, ExecutesFunctionOnScopeExit)
{
  bool called = false;
  {
    auto guard = ScopeGuard([&called]() noexcept { called = true; });
    EXPECT_FALSE(called);
  }
  EXPECT_TRUE(called);
}
...
NOLINT_TEST(ScopeGuardTest, ExecutesFunctionWithMultipleGuards)
{
  ...
  bool called2 = false;
  {
    auto guard1 = ScopeGuard([&called1]() noexcept { called1 = true; });
    auto guard2 = ScopeGuard([&called2]() noexcept { called2 = true; });
  ...
  GivenLambdaWithReferenceCaptureWhenScopeExitsThenLambdaIsExecuted)
{
  int i = 0;
  {
    ScopeGuard guard([&i]() noexcept { Increment(i); });
    EXPECT_EQ(i, 0);
  }
  EXPECT_EQ(i, 1);
}
