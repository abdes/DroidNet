...
auto Composition::AddComponentImpl(std::unique_ptr<Component> component) const -> Component&
{
    ...
    DCHECK_F(!pimpl_->component_index_.contains(component->GetTypeId()), "Component already exists");
...
void Composition::DeepCopyComponentsFrom(const Composition& other)
{
    ...
    for (const auto& entry : other.pimpl_->components_) {
        if (const auto* comp = entry.get(); comp->IsCloneable()) {
            ...
            pimpl_->component_index_[clone->GetTypeId()] = pimpl_->components_.size() - 1;
        } else {
...
void Composition::PrintComponents(std::ostream& out) const
{
    ...
    out << "> Object \"" << object_name << "\" has " << components_.size() << " components:\n";
