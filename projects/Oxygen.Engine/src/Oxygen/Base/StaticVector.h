...
/*!
 ...
 \note When constructing or assigning a StaticVector from an initializer list or
...
public:
    ...
    constexpr StaticVector(InputIt first, InputIt last)
    {
        ...
        assert(input_size <= MaxElements && "StaticVector: range constructor input exceeds maximum size");
    }

    constexpr StaticVector(std::initializer_list<T> init)
        : current_size_(std::min(init.size(), MaxElements))
    {
        assert(init.size() <= MaxElements && "StaticVector: initializer list size exceeds maximum size");
        auto it = init.begin();
        for (size_type i = 0; i < current_size_; ++i, ++it) {
            new (storage_ + i * sizeof(T)) T(*it);
        }
    ...
    constexpr auto operator=(std::initializer_list<T> list) -> StaticVector&
    {
        ...
        assert(list.size() <= MaxElements && "StaticVector: initializer list size exceeds maximum size");
        auto it = list.begin();
        for (size_type i = 0; i < current_size_; ++i, ++it) {
            new (storage_ + i * sizeof(T)) T(*it);
        }
    ...
    [[nodiscard]] constexpr auto at(size_type pos) -> reference
    {
        if (pos >= current_size_) {
            throw std::out_of_range("StaticVector::at: pos (which is " + std::to_string(pos) + ") >= this->size() (which is " + std::to_string(current_size_) + ")");
        }
        return (*this)[pos];
    }
    ...
    [[nodiscard]] constexpr auto end() const noexcept { return data() + current_size_; }
    [[nodiscard]] constexpr auto cend() const noexcept { return data() + current_size_; }

    // ===== Capacity =====

    [[nodiscard]] constexpr auto empty() const noexcept { return current_size_ == 0; }
    ...
    constexpr void push_back(const T& value)
    {
        if (current_size_ >= MaxElements) {
            throw std::length_error("StaticVector::push_back: cannot add element, container is full");
        }
    ...
    {
        if (current_size_ >= MaxElements) {
            throw std::length_error("StaticVector::emplace_back: cannot add element, container is full");
        }
        ...
        requires std::default_initializable<T>
    {
        if (new_size > MaxElements) {
            throw std::length_error("StaticVector::resize: new_size exceeds max_size()");
        }
