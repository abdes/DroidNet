...
public:
  ...
  void Short(std::string short_name) { short_name_ = std::move(short_name); }

  [[nodiscard]] auto Long() const -> const std::string& { return long_name_; }

  void Long(std::string long_name) { long_name_ = std::move(long_name); }
  ...
  OXGN_CLP_API void Print(std::ostream& out, unsigned width = 0) const;
...
struct Options {
  ...
  explicit Options(std::string label)
  ...
  */
  OXGN_CLP_API void Add(const std::shared_ptr<Option>& option);

  auto begin() -> OptionsCollectionType::iterator { return options_.begin(); }

  auto end() -> OptionsCollectionType::iterator { return options_.end(); }
