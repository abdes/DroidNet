...
namespace core {
    ...
    public:
        OXYGEN_NGIN_API Module(std::string_view name, std::weak_ptr<Engine> engine);

        OXYGEN_NGIN_API ~Module() override;

        OXYGEN_MAKE_NON_COPYABLE(Module)
        OXYGEN_MAKE_NON_MOVABLE(Module)

        [[nodiscard]] OXYGEN_NGIN_API auto Name() const -> std::string_view;
        ...
        OXYGEN_NGIN_API void Initialize(const Graphics* gfx);
        OXYGEN_NGIN_API void Shutdown() noexcept;
