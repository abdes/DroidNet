...
void Engine::AttachModule(const ModulePtr& module, const uint32_t layer)
{
  ...
  if (std::ranges::find_if(modules_,
        [&module](
          const auto& module_ctx) { return module_ctx.module == module; })
    != modules_.end()) {
...
{
  std::ranges::for_each(
    modules_, [](auto& module) { module.module->Shutdown(); });
}
...
void Engine::InitializeImGui()
{
  ...
  if (gfx && props_.enable_imgui_layer) {
    ...
    // imgui_module_ = gfx->CreateImGuiModule(shared_from_this(),
    // props_.main_window_id); imgui_module_->Initialize(gfx.get());
  }
}
...
auto Engine::Run() -> void
{
  ...
  bool continue_running { true };
  {
    ...
    std::ranges::for_each(
      modules_, [](auto& module) { module.frame_time.Reset(); });
  }
      ...
      modules_, [this, &continue_running](auto& module) {
        ...
        if (continue_running) {
          ...
          if (module.log_timer.ElapsedTime() >= std::chrono::seconds(1)) {
            LOG_F(
              INFO, "FPS: {} UPS: {}", module.fps.Value(), module.ups.Value());
            module.log_timer = {};
          }
        }
      });
