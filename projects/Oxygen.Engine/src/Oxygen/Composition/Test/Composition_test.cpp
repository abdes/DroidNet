...
public:
  ...
  template <typename T> void RemoveComponent() { Base::RemoveComponent<T>(); }
...
namespace destruction {
  ...
  public:
    ~DependencyComponent() override { order.emplace_back(ClassTypeName()); }
  };
  ...
  public:
    ~DependentComponent() override { order.emplace_back(ClassTypeName()); }
  };
} // namespace destruction
...
NOLINT_TEST_F(CompositionTest, ComponentsDestroyedInReverseOrder)
{
  ...
  EXPECT_EQ(destruction::order[0],
    destruction::DependentComponent::ClassTypeName()); // Dependent destroyed
                                                       // first
  EXPECT_EQ(destruction::order[1],
    destruction::DependencyComponent::ClassTypeName()); // Dependency destroyed
                                                        // last
}
        ...
        if (phase == 0 || phase == 1) { // Read-heavy phases
          ...
          EXPECT_NO_THROW({
            if (composition_.HasComponent<SimpleComponent>()) {
              [[maybe_unused]] auto& comp
                = composition_.GetComponent<SimpleComponent>();
      ...
      while (phase < 4) {
        if (phase == 2 || phase == 3) { // Write-heavy phases
          ...
          if (bool expected = false;
            writerActive.compare_exchange_strong(expected, true)) {
            EXPECT_NO_THROW({
              if (composition_.HasComponent<SimpleComponent>()) {
                composition_.RemoveComponent<SimpleComponent>();
              } else {
...
{
  [[maybe_unused]] auto& original
    = composition_.AddComponent<SimpleComponent>();
  EXPECT_FALSE(composition_.Value());
  const auto& replaced
    = composition_.ReplaceComponent<SimpleComponent, BetterComponent>(10);
