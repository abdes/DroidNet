...
class FastIntMap {
  ...
  static constexpr float kLoadThreshold
    = 0.7F; //!< Load factor threshold for growth
  size_t size_ { 0 }; //!< Number of occupied slots

public:
  //! Creates a new hash map with specified initial capacity
  //! \param initial_capacity Desired initial capacity (rounded up to power of
  //! 2)
  ...
  void Insert(const uint64_t key, const uint64_t value)
  {
    ...
    if (static_cast<float>(size_ + 1) / static_cast<float>(capacity_)
      >= kLoadThreshold) {
      Grow();
    }
...
private:
  ...
  void Grow()
  {
    ...
    auto cleanup = oxygen::Finally([&] { delete[] old_entries; });
