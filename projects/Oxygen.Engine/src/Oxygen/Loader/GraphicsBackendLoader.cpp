...
auto SerializeConfigToJson(const GraphicsConfig& config, BackendType backend_type) -> std::string
{
    ...
    json += "  \"enable_debug\": " + std::string(config.enable_debug ? "true" : "false") + ",\n";
    json += "  \"enable_validation\": " + std::string(config.enable_validation ? "true" : "false") + ",\n";
    json += "  \"headless\": " + std::string(config.headless ? "true" : "false") + ",\n";
    json += "  \"enable_imgui\": " + std::string(config.enable_imgui ? "true" : "false") + ",\n";
    ...
    if (config.preferred_card_device_id.has_value()) {
        json += R"(  "preferred_card_device_id": )" + std::to_string(config.preferred_card_device_id.value()) + ",\n";
    }
...
public:
    explicit Impl(std::shared_ptr<PlatformServices> services = nullptr)
        : platform_services(services ? std::move(services) : std::make_shared<PlatformServices>())
    {
    }
    ...
    auto LoadBackend(const BackendType backend, const GraphicsConfig& config) -> GraphicsPtr
    {
        if (backend_instance) {
    ...
    [[nodiscard]] auto GetPlatformServices() const -> const std::shared_ptr<PlatformServices>&
    {
        return platform_services;
    }

private:
    void CreateBackendInstance(GraphicsModuleApi* backend_api, BackendType backend_type, const GraphicsConfig& config)
    {
        if (!backend_instance) {
            ...
            // Store the instance with a custom deleter that will call the destroy function
            backend_instance = std::shared_ptr<oxygen::Graphics>(
                static_cast<oxygen::Graphics*>(instance),
                [destroyFunc = backend_api->DestroyBackend](const oxygen::Graphics* instance) {
                    if (instance != nullptr) {
                        destroyFunc();
                    }
                });
...
{
    PlatformServices::ModuleHandle moduleHandle = platform_services->GetModuleHandleFromReturnAddress(returnAddress);

    if (!platform_services->IsMainExecutableModule(moduleHandle)) {
        throw oxygen::loader::InvalidOperationError(fmt::format("Function `{}` called from non-main module", functionName));
    }
}
...
auto GraphicsBackendLoader::GetInstance(std::shared_ptr<loader::detail::PlatformServices> platform_services)
    -> GraphicsBackendLoader&
{
    static bool first_call = true;
    static std::shared_ptr<PlatformServices> services = platform_services ? std::move(platform_services) : std::make_shared<PlatformServices>();
    static auto instance = std::unique_ptr<GraphicsBackendLoader>(new GraphicsBackendLoader(services));
    ...
    if (!first_call && platform_services) {
        ...
        instance = std::unique_ptr<GraphicsBackendLoader>(new GraphicsBackendLoader(services));
    }
...
GraphicsBackendLoader::GraphicsBackendLoader(std::shared_ptr<loader::detail::PlatformServices> platform_services)
...
auto GraphicsBackendLoader::GetBackend() const noexcept -> std::weak_ptr<Graphics>
{
