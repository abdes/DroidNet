...
public:
    ...
    MOCK_METHOD(std::unique_ptr<oxygen::graphics::SynchronizationCounter>, CreateSynchronizationCounter, (const std::string_view, std::shared_ptr<oxygen::graphics::CommandQueue>), (const, override));
...
public:
    ...
    MOCK_METHOD(ModuleHandle, GetModuleHandleFromReturnAddress, (void*), (const, override));

    // Make the protected method public in the mock for testing
    using PlatformServices::GetRawFunctionAddress;
    MOCK_METHOD(RawFunctionPtr, GetRawFunctionAddress, (ModuleHandle, const std::string&), (override));
    // NOLINTEND(modernize-use-trailing-return-type)
};

// Helper to convert from function pointer type to PlatformServices::RawFunctionPtr
...
protected:
    ...
    private:
        ...
        static auto CreateBackendStatic(const SerializedBackendConfig& config) -> void*
        {
    ...
    void SetUp() override
    {
        ...
        ON_CALL(*platform, LoadModule(testing::_))
            // NOLINTNEXTLINE
            .WillByDefault(testing::Return(reinterpret_cast<void*>(MODULE_HANDLE_VALUE)));
...
NOLINT_TEST_F(GraphicsBackendLoaderTest, GetInstanceFromNonMainModule)
{
    ...
    EXPECT_THROW({ oxygen::GraphicsBackendLoader::GetInstance(platform); }, oxygen::loader::InvalidOperationError);
}
...
NOLINT_TEST_F(GraphicsBackendLoaderTest, GetBackendAfterLoading)
{
    ...
    // Load a backend
    oxygen::GraphicsConfig config;
    auto loaded_backend = loader.LoadBackend(oxygen::graphics::BackendType::kDirect3D12, config);
...
NOLINT_TEST_F(GraphicsBackendLoaderTest, LoadBackendTwiceReturnsSameInstance)
{
    ...
    oxygen::GraphicsConfig config;
    auto first_backend = loader.LoadBackend(oxygen::graphics::BackendType::kDirect3D12, config);
    auto second_backend = loader.LoadBackend(oxygen::graphics::BackendType::kDirect3D12, config);
...
NOLINT_TEST_F(GraphicsBackendLoaderTest, LoadBackendWithDifferentConfigs)
{
    ...
    auto backend = loader.LoadBackend(oxygen::graphics::BackendType::kDirect3D12, debug_config);
    EXPECT_FALSE(backend.expired());
    ...
    backend = loader.LoadBackend(oxygen::graphics::BackendType::kDirect3D12, validation_config);
    EXPECT_FALSE(backend.expired());
}
...
NOLINT_TEST_F(GraphicsBackendLoaderTest, GetInstanceWithNewPlatformServices_ResetsTheLoader)
{
    auto& loader1 = oxygen::GraphicsBackendLoader::GetInstance(platform);

    // Load a backend to verify state transition
    oxygen::GraphicsConfig config;
    auto backend = loader1.LoadBackend(oxygen::graphics::BackendType::kDirect3D12, config);
    ...
    auto new_backend = loader2.LoadBackend(oxygen::graphics::BackendType::kDirect3D12, config);
...
NOLINT_TEST_F(GraphicsBackendLoaderTest, ConfigSerialization)
{
    ...
    EXPECT_TRUE(json_str.find(R"("backend_type": "Direct3D12")") != std::string::npos);
    EXPECT_TRUE(json_str.find(R"("enable_debug": true)") != std::string::npos);
    EXPECT_TRUE(json_str.find(R"("enable_validation": true)") != std::string::npos);
    EXPECT_TRUE(json_str.find(R"("headless": false)") != std::string::npos);
    EXPECT_TRUE(json_str.find(R"("enable_imgui": true)") != std::string::npos);
    EXPECT_TRUE(json_str.find(R"("preferred_card_name": "Test GPU")") != std::string::npos);
    EXPECT_TRUE(json_str.find(R"("preferred_card_device_id": 1)") != std::string::npos);
    EXPECT_TRUE(json_str.find(R"("custom_key": "custom_value")") != std::string::npos);
