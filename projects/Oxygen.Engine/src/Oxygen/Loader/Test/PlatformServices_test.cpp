...
NOLINT_TEST(PlatformServicesTest, GetExecutableDirectory_ReturnsValidPath)
{
  ...
  const std::string result = services.GetExecutableDirectory();
  ...
  const std::filesystem::path dir_path(result);
...
NOLINT_TEST(
  PlatformServicesTest, GetModuleHandleFromReturnAddressWithCurrentFunction)
{
  const PlatformServices services;
  ...
  auto* module_handle
    = services.GetModuleHandleFromReturnAddress(return_address);
...
// Load a system library and look for a function that exists on all platforms
  ...
  const auto function_ptr
    = services.GetFunctionAddress<FunctionType>(module_handle, function_name);
...
NOLINT_TEST(PlatformServicesTest, GetFunctionAddressWithNullModule_Fails)
{
  ...
  EXPECT_THROW(
    {
      [[maybe_unused]] auto _
        = services.GetFunctionAddress<void (*)()>(nullptr, "SomeFunction");
    },
...
// Load a system library
  ...
  const std::string non_existent_function
    = "ThisFunctionDefinitelyDoesNotExist_XYZ123";
  EXPECT_THROW(
    {
      [[maybe_unused]] auto _ = services.GetFunctionAddress<FunctionType>(
        module_handle, non_existent_function);
    },
    std::runtime_error);
...
NOLINT_TEST(PlatformServicesTest, CallGetFunctionAddressResult)
{
  ...
  const auto get_process_id
    = services.GetFunctionAddress<ProcessIdFn>(module_handle, function_name);
  EXPECT_NE(get_process_id, nullptr);

  // Actually call the function to verify it works
  const auto process_id = get_process_id();
