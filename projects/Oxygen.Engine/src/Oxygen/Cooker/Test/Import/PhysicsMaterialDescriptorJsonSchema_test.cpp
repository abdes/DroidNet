...
auto FindRepoRoot() -> std::filesystem::path
{
  ...
  while (!path.empty()) {
    if (std::filesystem::exists(path / "src" / "Oxygen" / "Cooker" / "Import"
          / "Schemas" / "oxygen.physics-material-descriptor.schema.json")) {
      return path;
    }
...
NOLINT_TEST(PhysicsMaterialDescriptorJsonSchemaTest, AcceptsCanonicalDocument)
{
...
NOLINT_TEST(PhysicsMaterialDescriptorJsonSchemaTest, RejectsUnknownFields)
{
  const auto repo_root = FindRepoRoot();
  ASSERT_FALSE(repo_root.empty());
  const auto schema = LoadJsonFile(SchemaFile(repo_root));
...
NOLINT_TEST(PhysicsMaterialDescriptorJsonSchemaTest, RejectsNonPositiveDensity)
{
  const auto repo_root = FindRepoRoot();
  ASSERT_FALSE(repo_root.empty());
  const auto schema = LoadJsonFile(SchemaFile(repo_root));
