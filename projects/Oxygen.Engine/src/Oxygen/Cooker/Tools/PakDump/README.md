...
(target name `Oxygen.Cooker.PakDump`) will be placed with other tools.
...
```bash
Oxygen.Cooker.PakDump --help
```

```bash
Oxygen.Cooker.PakDump <pakfile> [options]
```
...
```bash
Oxygen.Cooker.PakDump game.pak
```

Verbose with resource data previews:

```bash
Oxygen.Cooker.PakDump game.pak --verbose --show-data
```

Inspect only assets (skip resources):

```bash
Oxygen.Cooker.PakDump game.pak --no-resources --hex-dump-assets
```

Full deep dive:

```bash
Oxygen.Cooker.PakDump game.pak --verbose --show-data --hex-dump-assets --max-data=1024
```
