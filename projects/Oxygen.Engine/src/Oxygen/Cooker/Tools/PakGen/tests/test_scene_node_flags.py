"""Scene v7 node layout and canonical flag-mask producer contract."""
...
@pytest.mark.parametrize("version", [None, 1, 2, 3, 4, 5, 6, "7", 7.0, True])
def test_only_current_scene_descriptor_version_is_emitted(version):
    ...
    with pytest.raises(PakError, match="Scene asset version 7 is required"):
        _pack_scene(scene)
...
def test_maintained_scene_fixtures_use_v7_local_masks(name):
