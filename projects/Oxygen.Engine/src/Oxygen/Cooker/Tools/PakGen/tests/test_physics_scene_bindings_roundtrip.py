...
def _read_directory_entries(data: bytes) -> list[dict[str, int]]:
    footer = data[-256:]
    directory_offset, directory_size, _asset_count = struct.unpack_from("<QQQ", footer, 0)
    ...
    for i in range(directory_size // 64):
        ...
        entry_offset, desc_offset, desc_size = struct.unpack_from("<QQI", entry, 17)
        entries.append(
            {
...
def _parse_physics_tables(desc: bytes) -> tuple[int, int, list[dict[str, int]]]:
    target_node_count, table_count, directory_offset = struct.unpack_from("<IIQ", desc, 111)
    tables: list[dict[str, int]] = []
    for i in range(table_count):
        off = int(directory_offset) + i * 20
        binding_type = struct.unpack_from("<I", desc, off)[0]
        table_offset, count, entry_size = struct.unpack_from("<QII", desc, off + 4)
        tables.append(
            {
...
def _validate_physics_scene_descriptor(desc: bytes) -> list[str]:
    ...
    if table_count == 0:
        if directory_offset != 0:
            issues.append("directory_offset must be zero when table_count is zero")
        return issues
    ...
    for i in range(table_count):
        ...
        table_offset, count, entry_size = struct.unpack_from("<QII", desc, eoff + 4)
...
def test_physics_scene_with_all_bindings_roundtrip(tmp_path: Path):
    spec = Path(__file__).parent / "_golden" / "scene_with_physics_sidecar_spec.yaml"
    ...
    assert issues == []
    assert info["footer"]["tables"]["physics"]["count"] == 3
    ...
    assert all(PHYSICS_BINDING_ENTRY_SIZES[t["binding_type"]] == t["entry_size"] for t in tables)
...
def test_physics_scene_negative_structural_cases(tmp_path: Path):
    ...
    _target_node_count, table_count, _tables = _parse_physics_tables(bytes(desc))
    assert table_count >= 2
    directory_offset = struct.unpack_from("<Q", desc, 119)[0]

    bad_dir = bytearray(desc)
    struct.pack_into("<Q", bad_dir, 119, desc_size + 8)
    assert any("directory out of bounds" in i for i in _validate_physics_scene_descriptor(bytes(bad_dir)))
    ...
    assert any("entry_size mismatch" in i for i in _validate_physics_scene_descriptor(bytes(wrong_entry_size)))

    overlap = bytearray(desc)
    first_table_offset = struct.unpack_from("<Q", overlap, int(directory_offset) + 4)[0]
    second_entry_offset = int(directory_offset) + 20
    struct.pack_into("<Q", overlap, second_entry_offset + 4, first_table_offset)
    assert any("overlap" in i for i in _validate_physics_scene_descriptor(bytes(overlap)))

    unknown_binding = bytearray(desc)
    struct.pack_into("<I", unknown_binding, first_entry_offset, 0xDEADBEEF)
    assert any("unknown binding type" in i for i in _validate_physics_scene_descriptor(bytes(unknown_binding)))
