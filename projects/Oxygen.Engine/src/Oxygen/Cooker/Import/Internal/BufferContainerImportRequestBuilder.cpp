...
namespace {
    ...
    std::ostream& error_stream) -> std::optional<json>
  {
    ...
    if (!input.is_open()) {
      error_stream << "ERROR: failed to open buffer-container descriptor: "
    ...
    try {
      ...
      input >> doc;
      if (!doc.is_object()) {
        error_stream << "ERROR: buffer-container descriptor root must be a "
                        "JSON object\n";
        return std::nullopt;
      }
      return doc;
    } catch (const std::exception& ex) {
      error_stream << "ERROR: invalid buffer-container descriptor JSON: "
                   << ex.what() << "\n";
      return std::nullopt;
    }
  }

  auto GetBufferContainerValidator() -> json_validator&
  {
    static auto validator = []() {
      auto out = json_validator {};
      out.set_root_schema(json::parse(kBufferContainerSchema));
      return out;
    }();
    return validator;
  }
    ...
    const json& descriptor_doc, std::ostream& error_stream) -> bool
  {
    const auto config = JsonSchemaValidationDiagnosticConfig {
      ...
      .validation_failed_prefix = "Buffer container validation failed: ",
    ...
    return ValidateJsonSchemaWithDiagnostics(GetBufferContainerValidator(),
