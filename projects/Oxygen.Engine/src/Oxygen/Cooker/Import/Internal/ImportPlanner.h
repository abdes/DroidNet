...
struct ReadinessTracker {
  ...
  OXGN_COOK_NDAPI auto IsReady() const noexcept -> bool;

  //! Mark a producer dependency as ready.
  OXGN_COOK_API auto MarkReady(const DependencyToken& token) -> bool;
};
...
public:
  ...
  OXGN_COOK_API auto MakePlan() -> std::vector<PlanStep>;

  //! Access a plan item by ID.
  OXGN_COOK_API auto Item(PlanItemId item) -> PlanItem&;

  //! Resolve the pipeline type ID for a plan item.
  OXGN_COOK_NDAPI auto PipelineTypeFor(PlanItemId item) const noexcept
    -> std::optional<TypeId>;

  //! Access the readiness tracker for a plan item.
  OXGN_COOK_API auto Tracker(PlanItemId item) -> ReadinessTracker&;

  //! Access the readiness event for a plan item.
  OXGN_COOK_API auto ReadyEvent(PlanItemId item) -> ReadinessEvent&;
