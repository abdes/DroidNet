...
auto ScriptingSidecarImportJob::ExecuteAsync() -> co::Co<ImportReport>
{
  DLOG_F(INFO, "Starting scripting sidecar job: job_id={} path={}", JobId(),
  ...
  if (Request().options.scripting.import_kind
    ...
    AddDiagnostic(session, Request(), ImportSeverity::kError,
      ...
      "ScriptingSidecarImportJob requires "
  ...
  ReportPhaseProgress(
    ImportPhase::kWorking, 0.5F, "Applying scripting sidecar...");

  auto pipeline
    = ScriptingSidecarImportPipeline(ScriptingSidecarImportPipeline::Config {
  ...
  if (!result.success) {
    ReportPhaseProgress(
      ImportPhase::kFailed, 1.0F, "Scripting sidecar apply failed");
    co_return co_await FinalizeWithTelemetry(session);
  }
...
auto ScriptingSidecarImportJob::LoadSource(ImportSession& session)
  -> co::Co<LoadedSource>
{
  ...
  if (reader == nullptr) {
    AddDiagnostic(session, req, ImportSeverity::kError,
      "script.sidecar.reader_unavailable",
  ...
  if (!read.has_value()) {
    AddDiagnostic(session, req, ImportSeverity::kError,
      "script.sidecar.source_read_failed",
...
auto ScriptingSidecarImportJob::FinalizeSession(ImportSession& session)
  -> co::Co<ImportReport>
{
