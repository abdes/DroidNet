...
public:
  ...
  OXGN_COOK_API explicit AsyncImporter(Config config);

  OXGN_COOK_API ~AsyncImporter() override;
  ...
  */
  OXGN_COOK_NDAPI auto SubmitJob(JobEntry entry) -> co::Co<>;
  ...
  */
  OXGN_COOK_NDAPI auto TrySubmitJob(JobEntry entry) -> bool;

  //! Check if the importer has capacity for another job.
  OXGN_COOK_NDAPI auto CanAcceptJob() const noexcept -> bool;

  //! Close the job channel (no more jobs accepted).
  OXGN_COOK_API void CloseJobChannel();
  ...
  OXGN_COOK_NDAPI auto ActiveJobCount() const noexcept -> size_t;

  //! Get the number of jobs currently running.
  OXGN_COOK_NDAPI auto RunningJobCount() const noexcept -> size_t;

  //! Get the number of jobs queued but not yet running.
  OXGN_COOK_NDAPI auto PendingJobCount() const noexcept -> size_t;
