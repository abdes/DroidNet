// File: bc7enc.h - Richard Geldreich, Jr. - MIT license or public domain (see
// end of bc7enc.c) If you use this software in a product, attribution / credits
...
struct bc7enc_compress_block_params {
  ...
  // m_max_partitions may range from 0 (disables mode 1) to
  // BC7ENC_MAX_PARTITIONS. The higher this value, the slower the compressor,
  ...
  // m_uber_level may range from 0 to BC7ENC_MAX_UBER_LEVEL. The higher this
  // value, the slower the compressor, but the higher the quality.
  uint32_t m_uber_level;

  // If m_perceptual is true, colorspace error is computed in YCbCr space,
  // otherwise RGB.
  bool m_perceptual;

  // Set m_try_least_squares to false for slightly faster/lower quality
  // compression.
  bool m_try_least_squares;

  // When m_mode17_partition_estimation_filterbank, the mode1 partition
  // estimator skips lesser used partition patterns unless they are strongly
  // predicted to be potentially useful. There's a slight loss in quality with
  // this enabled (around .08 dB RGB PSNR or .05 dB Y PSNR), but up to a 11%
  // gain in speed depending on the other settings.
  ...
  void print()
  {
    ...
    printf("Mode error weights: %f %f %f %f\n", m_mode1_error_weight,
      m_mode5_error_weight, m_mode6_error_weight, m_mode7_error_weight);
    printf(
      "Low frequency partition weight: %f\n", m_low_frequency_partition_weight);
  }
};
...
inline void bc7enc_compress_block_params_init_perceptual_weights(
  bc7enc_compress_block_params* p)
{
  p->m_perceptual = true;
  p->m_weights[0] = 128;
...
// Packs a single block of 16x16 RGBA pixels (R first in memory) to 128-bit BC7
// block pBlock, using either mode 1 and/or 6. Alpha blocks will always use mode
// 6, and by default opaque blocks will use either modes 1 or 6. Returns true if
// the block had any pixels with alpha < 255, otherwise it return false. (This
// is not an error code - a block is always encoded.)
