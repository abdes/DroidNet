...
OXGN_COOK_NDAPI auto DecodeImageRgba8FromMemory(
  std::span<const std::byte> bytes) -> ImageDecodeResult;

//! Decode an image file into RGBA8.
OXGN_COOK_NDAPI auto DecodeImageRgba8FromFile(const std::filesystem::path& path)
  -> ImageDecodeResult;
...
*/
OXGN_COOK_NDAPI auto IsExrSignature(std::span<const std::byte> bytes) noexcept
  -> bool;
...
*/
OXGN_COOK_NDAPI auto IsHdrSignature(std::span<const std::byte> bytes) noexcept
  -> bool;
...
*/
OXGN_COOK_NDAPI auto IsHdrFormat(std::span<const std::byte> bytes,
...
*/
OXGN_COOK_NDAPI auto DecodeToScratchImage(
