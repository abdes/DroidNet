...
public:
  ...
  OXGN_COOK_API ImportSession(const ImportRequest& request,
  ...
  OXGN_COOK_NDAPI auto Request() const noexcept -> const ImportRequest&;

  //! Get the cooked root path for this session.
  OXGN_COOK_NDAPI auto CookedRoot() const noexcept
  ...
  OXGN_COOK_NDAPI auto CookedWriter() noexcept -> LooseCookedWriter&;
  ...
  OXGN_COOK_NDAPI auto TextureEmitter() -> TextureEmitter&;
  ...
  */
  OXGN_COOK_NDAPI auto Diagnostics() const -> std::vector<ImportDiagnostic>;

  //! Check if any error-level diagnostics have been added.
  OXGN_COOK_NDAPI auto HasErrors() const noexcept -> bool;
