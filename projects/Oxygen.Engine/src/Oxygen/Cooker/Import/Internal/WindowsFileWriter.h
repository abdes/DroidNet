...
public:
  ...
  */
  OXGN_COOK_API explicit WindowsFileWriter(ImportEventLoop& loop);

  OXGN_COOK_API ~WindowsFileWriter() override;
  ...
  OXGN_COOK_NDAPI auto Write(const std::filesystem::path& path,
  ...
  OXGN_COOK_API void WriteAsync(const std::filesystem::path& path,
    std::span<const std::byte> data, WriteOptions options,
    WriteCompletionCallback callback) override;

  //! Write data at a specific byte offset, waiting for completion.
  OXGN_COOK_NDAPI auto WriteAt(const std::filesystem::path& path,
  ...
  //! notification.
  OXGN_COOK_API void WriteAtAsync(const std::filesystem::path& path,
    uint64_t offset, std::span<const std::byte> data, WriteOptions options,
    WriteCompletionCallback callback) override;

  //! Wait for all pending operations to complete.
  OXGN_COOK_NDAPI auto Flush() -> co::Co<Result<void, FileErrorInfo>> override;

  //! Cancel all pending operations.
  OXGN_COOK_API void CancelAll() override;

  //! Get number of pending async operations.
  [[nodiscard]] OXGN_COOK_API auto PendingCount() const -> size_t override;
