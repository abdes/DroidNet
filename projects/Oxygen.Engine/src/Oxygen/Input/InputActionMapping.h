...
namespace input {
    ...
    public:
        OXYGEN_INPUT_API InputActionMapping(std::shared_ptr<Action> action,
        ...
        OXYGEN_INPUT_API void AddTrigger(std::shared_ptr<ActionTrigger> trigger);
        [[nodiscard]] auto GetTriggers() const -> const auto& { return triggers_; }

        OXYGEN_INPUT_API void HandleInput(const platform::InputEvent& event);
        OXYGEN_INPUT_API void CancelInput();
        OXYGEN_INPUT_API [[nodiscard]] auto Update(Duration delta_time) -> bool;
