...
class InputActionMappingFixture : public ::testing::Test {
protected:
...
NOLINT_TEST_F(InputActionMappingFixture, KeyPressed_TriggersOnce)
{
  // Arrange
  auto action
  ...
  const bool consumed1 = mapping.Update(Duration::zero());
  ...
  const bool consumed2 = mapping.Update(Duration::zero());
...
NOLINT_TEST_F(InputActionMappingFixture, KeyDown_OngoingWhileHeld)
{
  // Arrange
  auto action
  ...
  mapping.Update(oxygen::SecondsToDuration(0.016F));
...
NOLINT_TEST_F(InputActionMappingFixture, MouseMotion_UsesMappedAxis)
{
  // Arrange X
...
NOLINT_TEST_F(InputActionMappingFixture, MouseWheel_RespectsSlot)
{
  // X
  auto ax = std::make_shared<Action>(
...
NOLINT_TEST_F(InputActionMappingFixture, Blocker_PreventsTrigger)
{
  auto action
  ...
  mapping.HandleInput(MakeKey(ButtonState::kPressed));
  const bool consumed = mapping.Update(Duration::zero());
  EXPECT_FALSE(action->IsTriggered());
  EXPECT_FALSE(consumed);
}

//! CancelInput should mark action canceled in current evaluation
NOLINT_TEST_F(InputActionMappingFixture, CancelInput_SetsCanceled)
{
...
NOLINT_TEST_F(InputActionMappingFixture, ImplicitOnly_AllMustTrigger_Mapping)
{
  // Arrange
  auto action
  ...
  EXPECT_FALSE(mapping.Update(oxygen::SecondsToDuration(0.03F)));
  EXPECT_FALSE(action->IsTriggered());
...
NOLINT_TEST_F(InputActionMappingFixture, ImplicitWithBlocker_Blocked)
{
  // Arrange
  auto action
...
NOLINT_TEST_F(InputActionMappingFixture, MouseMotion_ValueClearsNextUpdate)
{
  // Arrange
  auto axy
...
NOLINT_TEST_F(InputActionMappingFixture, Pressed_NoRepeatWithoutEvent)
{
  // Arrange
  auto action
  ...
  const bool consumed = mapping.Update(oxygen::SecondsToDuration(0.016F));
  EXPECT_FALSE(consumed);
...
NOLINT_TEST_F(InputActionMappingFixture, WheelDirectional_SpecificSlots)
{
  // Left (dx < 0)
  auto aleft
...
NOLINT_TEST_F(InputActionMappingFixture, ConsumesInput_True_ConsumesOnTrigger)
{
  ...
  const bool consumed2 = mapping.Update(oxygen::SecondsToDuration(0.01F));
...
NOLINT_TEST_F(InputActionMappingFixture, NoTriggers_NoEffect)
{
  // Arrange
  auto action
...
NOLINT_TEST_F(
  InputActionMappingFixture, ExplicitAndImplicit_RequiresExplicitEachUpdate)
{
  ...
  mapping.HandleInput(MakeKey(ButtonState::kPressed));
  EXPECT_FALSE(mapping.Update(oxygen::SecondsToDuration(0.02F)));
  EXPECT_FALSE(action->IsTriggered());
  ...
  EXPECT_FALSE(mapping.Update(oxygen::SecondsToDuration(0.04F)));
  EXPECT_FALSE(action->IsTriggered());
}
