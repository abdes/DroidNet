...
//! the origin.
OXGN_DATA_NDAPI auto MakeCubeMeshAsset() -> std::shared_ptr<Mesh>;
...
OXGN_DATA_NDAPI auto MakeSphereMeshAsset(unsigned int latitude_segments = 16,
  unsigned int longitude_segments = 32) -> std::shared_ptr<Mesh>;
...
OXGN_DATA_NDAPI auto MakePlaneMeshAsset(unsigned int x_segments = 1,
  unsigned int z_segments = 1, float size = 1.0f) -> std::shared_ptr<Mesh>;

//! Creates a new Mesh representing a cylinder.
OXGN_DATA_NDAPI auto MakeCylinderMeshAsset(unsigned int segments = 32,
  float height = 1.0f, float radius = 0.5f) -> std::shared_ptr<Mesh>;
...
OXGN_DATA_NDAPI auto MakeTorusMeshAsset(unsigned int major_segments = 32,
  ...
  float minor_radius = 0.25f) -> std::shared_ptr<Mesh>;
...
OXGN_DATA_NDAPI auto MakeArrowGizmoMeshAsset() -> std::shared_ptr<Mesh>;
