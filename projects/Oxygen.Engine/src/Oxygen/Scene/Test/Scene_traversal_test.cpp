...
protected:
    ...
    {
        return
            [this](SceneNodeImpl& node, const Scene& /*scene*/) -> VisitResult {
        ...
        const std::vector<std::string>& forbidden_nodes) const
    {
        for (const auto& forbidden : forbidden_nodes) {
            EXPECT_THAT(
                visit_order_, testing::Not(testing::Contains(forbidden)))
                << "Found forbidden node (should not be present): "
                << forbidden;
        }
    }
...
NOLINT_TEST_F(SceneTraversalFilterTest, AcceptAllFilter)
{
    ...
    const auto result = traversal_->Traverse(CreateTrackingVisitor(),
        TraversalOrder::kDepthFirst, AcceptAllFilter {});
...
NOLINT_TEST_F(SceneTraversalEdgeCaseTest, FilterRejectingAllNodes)
{
    ...
    const auto result = traversal_->Traverse(CreateTrackingVisitor(),
        TraversalOrder::kDepthFirst, reject_all_filter);
...
NOLINT_TEST_F(SceneTraversalEdgeCaseTest, VisitorStoppingImmediately)
{
    ...
    auto immediate_stop_visitor = [this](const SceneNodeImpl& node,
                                      const Scene& /*scene*/) -> VisitResult {
        visit_order_.emplace_back(
            node.GetName()); // Convert string_view to string
