...
protected:
    ...
    void ExpectVisitedNodes(const std::vector<std::string>& expected_names) const
    {
        ...
        for (size_t i = 0; i < expected_names.size(); ++i) {
            EXPECT_EQ(visit_order_[i], expected_names[i]) << "Mismatch at position " << i;
        }
    }
    ...
    void ExpectContainsAllNodes(const std::vector<std::string>& expected_nodes) const
    {
    ...
    void ExpectContainsNoForbiddenNodes(const std::vector<std::string>& forbidden_nodes) const
    {
        ...
        const std::vector<std::string>& level2_nodes)
    {
        ...
        for (const auto& node : level1_nodes) {
            max_level1_pos = std::max(max_level1_pos, static_cast<size_t>(find_pos(node)));
        }

        size_t min_level2_pos = visit_order_.size();
        for (const auto& node : level2_nodes) {
            min_level2_pos = std::min(min_level2_pos, static_cast<size_t>(find_pos(node)));
        }

        EXPECT_LT(max_level1_pos, min_level2_pos)
            << "Level 1 nodes should come before level 2 nodes in breadth-first traversal";
    }
...
protected:
    ...
    // Member variables using optional since SceneNode has no default constructor
...
NOLINT_TEST_F(SceneTraversalBasicTest, DepthFirstTraversal)
{
    ...
    // Assert: All nodes should be visited (order not guaranteed due to Scene storage)
...
//! Tests that breadth-first traversal visits all nodes level by level and in correct order.
...
//! Tests that traversing from multiple roots only visits the specified subtrees.
...
//! Tests that subtree skipping works in depth-first traversal (children of A are skipped).
...
//! Tests that subtree skipping works in breadth-first traversal (children of A are skipped).
...
//! Tests that rejecting specific nodes with a filter excludes them but still visits their children.
...
//! Tests that rejecting a subtree with a filter excludes the node and all its descendants.
...
//! Tests that rejecting a subtree in breadth-first traversal excludes the node and all its descendants.
...
NOLINT_TEST_F(SceneTraversalBuiltinFilterTest, VisibleFilter)
{
    ...
    // Assert: Only visible_root should be visited; invisible_node rejects subtree
    ExpectTraversalResult(result, 1, 1, true);
    EXPECT_THAT(visit_order_, testing::ElementsAre("visible_root"));
}

//! Tests that only nodes with dirty transforms are visited when using DirtyTransformFilter (builtin filter).
...
//! Tests that a deep hierarchy (chain) can be traversed without stack overflow and all nodes are visited.
...
//! Tests that a wide hierarchy (many children at root) can be traversed and all nodes are visited.
...
//! Tests that a filter rejecting all nodes results in no nodes visited and correct filter count.
NOLINT_TEST_F(SceneTraversalEdgeCaseTest, FilterRejectingAllNodes)
{
    ...
    // Assert: No nodes should be visited, but filter count depends on implementation
    // If the filter is applied to root and rejects the subtree, only 1 node is "filtered"
    // The child is never reached because the entire subtree is rejected at root
    ExpectTraversalResult(result, 0, 1, true);
    EXPECT_TRUE(visit_order_.empty());
}

//! Tests that a visitor which stops immediately results in only the first node being visited.
...
//! Tests that combining a visible filter and subtree skipping works as expected in a complex hierarchy.
...
//! Tests that only dirty nodes are updated in a complex hierarchy using UpdateTransforms().
...
//! Tests that transform update with a custom filter (dirty and visible) only updates nodes that are both dirty and visible.
NOLINT_TEST_F(SceneTraversalComplexTest, UpdateTransformsWithVisibleFilter)
{
    ...
    auto dirty_and_visible = [](const SceneNodeImpl& node, FilterResult /*parent_result*/) -> FilterResult {
    ...
    // Assert: Only B should be updated (K is invisible, F is not dirty, root is not dirty, A and E are not dirty)
