    ...
    ClonesAreIndependent_ChangingOneDoesNotAffectOther)
{
    ...
    SetTransformValues(
        cloned, { 100.0f, 200.0f, 300.0f }, { 3.0f, 3.0f, 3.0f });
    ...
    EXPECT_DEATH(
        {
            [[maybe_unused]] auto result = target_scene_->CreateChildNodeFrom(
                invalid_parent, original, "ChildClone");
        },
...
NOLINT_TEST_F(
    SceneCloningErrorTest, CreateChildNodeFrom_InvalidOriginal_TriggersDeath)
{
    // Arrange
    const auto parent = target_scene_->CreateNode("Parent");
    ASSERT_TRUE(parent.IsValid());
    ...
    EXPECT_DEATH(
        {
            [[maybe_unused]] auto result = target_scene_->CreateChildNodeFrom(
                parent, invalid_original, "ChildClone");
        },
        "expecting a valid original node handle");
}

NOLINT_TEST_F(SceneCloningErrorTest,
    CreateChildNodeFrom_OriginalRemovedFromScene_TriggersDeath)
{
    // Arrange
    const auto parent = target_scene_->CreateNode("Parent");
    auto original = source_scene_->CreateNode("OriginalNode");
    ...
    EXPECT_DEATH(
        {
            [[maybe_unused]] auto result = target_scene_->CreateChildNodeFrom(
                parent, original, "ChildClone");
        },
        "expecting a valid original node handle");
}

NOLINT_TEST_F(SceneCloningErrorTest,
    CreateChildNodeFrom_CloningFromDestroyedHierarchy_TriggersDeath)
{
    // Arrange
    auto source_parent = source_scene_->CreateNode("SourceParent");
    const auto source_child_opt
        = source_scene_->CreateChildNode(source_parent, "SourceChild");
    ...
    EXPECT_DEATH(
        {
            [[maybe_unused]] auto result = target_scene_->CreateChildNodeFrom(
                target_parent, source_child, "ClonedChild");
        },
