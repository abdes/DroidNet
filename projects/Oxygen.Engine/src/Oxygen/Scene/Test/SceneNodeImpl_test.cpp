...
public:
  explicit TestSceneNodeImpl(
    const std::string& name = "TestNode", const Flags& flags = kDefaultFlags)
...
NOLINT_TEST_F(
  SceneNodeImplDefaultFlagsTest, DefaultFlags_IgnoreParentTransformSetCorrectly)
{
  // Arrange: Create node with default flags
  auto node = CreateDefaultNode();
  const auto& flags = node.GetFlags();
...
NOLINT_TEST_F(
  SceneNodeImplTransformDirtyTest, MarkTransformDirty_MultipleCallsRemainDirty)
{
  // Arrange: Create node and clear transform dirty
  auto node = CreateDefaultNode();
  node.ClearTransformDirty();
