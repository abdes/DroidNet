...
//! Concrete implementation of a scene node, stored in a resource table and
//! accessible via a handle.
/*!
 SceneNodeImpl decouples the data of a scene node from its handle, allowing for
 efficient resource management and retrieval.

 It extends oxygen::Composition, giving it the flexibility to compose itself
 with various components and functionality.

 At a minimum, it has:
 - an ObjectMetaData component to store the node name and any other custom
   properties,
 - a SceneNodeData component to store the actual scene data and its state,
 - and a TransformComponent to manage the node's position, rotation, and scale
   in the scene.

 \note: SceneNodeImpl is primarily used by the engine. Application logic should
        interact with SceneNode instead.
*/
...
public:
    ...
    */
    ...
    //! @{
    //! Transform update and dirty marking (Scene class only)
    ...
    //! Updates the world transform for this node.
    /*!
     Only the Scene should call this method, and does it during the scene update
     pass, to compute the world transformation matrix for this node based on its
     local matrix and the world matrices of its parent nodes.

     A scene node with no parent (root node), or one that has the
     kIgnoreParentTransform flag set, will compute its world matrix solely using
     its local transform.

     \note Expects the node to be valid and the parent transformation to be up
           to date.
    */
