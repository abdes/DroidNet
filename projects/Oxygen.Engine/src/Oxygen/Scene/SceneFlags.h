...
/*!
 ...
 __5-Bit State Layout__

 The flag uses a compact 5-bit layout to store all state information:
 - Bit 0 (Effective value): Current resolved boolean state that's actively used
 - Bit 1 (Inheritance flag): Whether this flag inherits its value from parent
   node
 - Bit 2 (Pending value): Staged value that becomes effective after
   ProcessDirty()
 - Bit 3 (Dirty flag): Indicates the flag needs processing in the update cycle
 - Bit 4 (Previous value): Previous effective value for transition detection

 __Inheritance System__

 The inheritance system enables scene graph nodes to automatically propagate
 flag values from parent to child nodes:

 - **Local Values**: When SetLocalValue() is called, inheritance is disabled and
   the flag takes on a local value that overrides any parent value.

 - **Inherited Values**: When SetInherited(true) is called, the flag will
   receive its value from the parent node during scene updates via
   UpdateValueFromParent().

 - **Deferred Processing**: All value changes are staged in the pending value
   and only become effective when ProcessDirty() is called during the scene
   update cycle. This enables batch processing for optimal performance.

 - **Transition Detection**: The previous value bit enables detection of state
   changes between update cycles, useful for triggering dependent operations.

 __Optimization Features__

 - Redundant change detection prevents unnecessary dirty marking
 - Revert detection clears dirty flags when reverting to current effective value
 - Batch processing through deferred updates minimizes scene traversal overhead
*/
class SceneFlag {
public:
  ...
  */
  ...
  {
    // Set inheritance state based on the parameter
  ...
  */
  ...
  [[nodiscard]] constexpr auto operator==(const SceneFlag& other) const noexcept
    -> bool
  {
  ...
  [[nodiscard]] constexpr auto effective_true_flags() const noexcept
  {
    return std::views::iota(std::size_t { 0 }, flag_count)
      | std::views::transform(
        [](std::size_t i) { return static_cast<FlagEnum>(i); })
      | std::views::filter(
        [this](FlagEnum flag) { return this->GetEffectiveValue(flag); });
  }
...
public:
  ...
  */
  [[nodiscard]] auto Exchange(const flags_type& flags,
    std::memory_order order = std::memory_order_seq_cst) noexcept -> flags_type
  {
    flags_type result;
