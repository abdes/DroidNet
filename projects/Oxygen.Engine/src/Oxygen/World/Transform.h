...
namespace transform {
   ...
   * entities must have a transform component. Therefore, it will always be true
   * that for a transform component id, the index and generation will be the
   * same as for the corresponding game entity.
   */
  struct Descriptor {
    glm::vec3 position {};
    glm::quat rotation {};
...
public:
  ...
  OXGN_GFX_NDAPI auto GetPosition() const noexcept -> glm::vec3;
  OXGN_GFX_NDAPI auto GetRotation() const noexcept -> glm::quat;
  OXGN_GFX_NDAPI auto GetScale() const noexcept -> glm::vec3;

  OXGN_GFX_API void SetPosition(glm::vec3 position) const noexcept;
  OXGN_GFX_API void SetRotation(glm::quat rotation) const noexcept;
  OXGN_GFX_API void SetScale(glm::vec3 scale) const noexcept;
};
