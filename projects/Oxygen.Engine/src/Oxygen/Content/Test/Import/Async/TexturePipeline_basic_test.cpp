...
//! Verify placeholder policy returns a placeholder payload on failure.
NOLINT_TEST_F(
  TexturePipelineTest, Collect_WithPlaceholderPolicy_UsesPlaceholder)
{
  // Arrange
  ...
  EXPECT_EQ(result.cooked->desc.width, 1U);
  EXPECT_EQ(result.cooked->desc.height, 1U);
}
