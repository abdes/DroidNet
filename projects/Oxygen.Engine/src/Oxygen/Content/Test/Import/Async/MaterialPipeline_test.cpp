...
//! Verify mismatched UV transforms fall back to identity extension.
NOLINT_TEST_F(MaterialPipelineUvTest, Collect_MismatchedTransforms_Identity)
{
  // Arrange
  auto item = MakeBaseItem();
  item.textures.base_color = MaterialTextureBinding {
  ...
  EXPECT_FLOAT_EQ(uv.uv_scale[0], 1.0f);
  EXPECT_FLOAT_EQ(uv.uv_scale[1], 1.0f);
