...
namespace {
  ...
  {
    const auto* file_tex = emit::ResolveFileTexture(texture);
    if (file_tex == nullptr) {
      return std::nullopt;
    }
    ...
    if (identity.embedded) {
      const auto bytes = std::span<const std::byte>(
        reinterpret_cast<const std::byte*>(file_tex->content.data),
        file_tex->content.size);
      identity.texture_id
    ...
    if (!identity.resolved_path.empty()) {
      identity.texture_id
        = emit::NormalizeTexturePathId(identity.resolved_path);
    }

    if (identity.texture_id.empty()) {
      const auto id = emit::TextureIdString(*file_tex);
      if (!id.empty()) {
    ...
    -> std::optional<TexturePipeline::SourceBytes>
  {
    ...
    auto bytes = emit::TryReadWholeFileBytes(identity.resolved_path);
  ...
  [[nodiscard]] auto HasMaterialTextures(const ufbx_material* material) -> bool
  {
    ...
    return emit::SelectBaseColorTexture(*material) != nullptr
      || emit::SelectNormalTexture(*material) != nullptr
      || emit::SelectMetallicTexture(*material) != nullptr
      || emit::SelectRoughnessTexture(*material) != nullptr
      || emit::SelectAmbientOcclusionTexture(*material) != nullptr
      || emit::SelectEmissiveTexture(*material) != nullptr;
  }
  ...
  [[nodiscard]] auto DecomposeTransform(const glm::mat4& transform,
    glm::vec3& translation, glm::quat& rotation, glm::vec3& scale) -> bool
  {
    ...
    if (!glm::decompose(transform, scale_out, rotation_out, translation_out,
          skew, perspective)) {
      return false;
    }
  ...
  const AdapterInput& input) -> WorkItemStreamResult
{
  ...
  for (uint32_t i = 0; i < material_count; ++i) {
    ...
    if (material != nullptr) {
      ...
      const auto* metallic_file = emit::ResolveFileTexture(metallic_tex);
      const auto* roughness_file = emit::ResolveFileTexture(roughness_tex);
      ...
      if (orm_packed) {
        ...
        if (texture_id.has_value()) {
          ...
          const auto* ao_file = emit::ResolveFileTexture(ao_tex);
