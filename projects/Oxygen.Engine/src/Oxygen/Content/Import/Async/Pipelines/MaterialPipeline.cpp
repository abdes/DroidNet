...
namespace {
  ...
  [[nodiscard]] auto NeedsUvTransformBake(const MaterialTextureBinding& lhs,
    const MaterialTextureBinding& rhs) -> bool
  {
    if (lhs.uv_set != rhs.uv_set) {
      return true;
    }
    const auto& a = lhs.uv_transform;
    const auto& b = rhs.uv_transform;
    ...
    -> MaterialUvTransformDesc
  {
    ...
    for (const auto* binding : bindings) {
      ...
      if (NeedsUvTransformBake(*reference, *binding)) {
        return desc;
      }
    }
...
auto MaterialPipeline::Worker() -> co::Co<>
{
  while (true) {
    ...
    auto hash = co_await ComputeContentHashOnThreadPool(thread_pool_,
      std::span<const std::byte>(bytes.data(), bytes.size()), item.stop_token);
    if (!hash.has_value()) {
      co_await ReportCancelled(std::move(item));
      continue;
    }
