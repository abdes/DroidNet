...
//! Handle to a transform entry managed by TransformUploader.
/*!
 ...
 part of finalization, and can be used as index into shared GPU transform
 buffer during rendering (bindless access).

 Handles are stable for the lifetime of the residency entry but may be recycled
 during long-running execution. Handle generation is part of identity and must
 be validated by TransformUploader before use.
*/
class TransformHandle {
public:
  using Index = NamedType<uint32_t, struct TransformHandleIndexTag,
    // clang-format off
    Comparable,
    Printable
    >; // clang-format on
  using Generation = oxygen::bindless::Generation;
  using Packed = NamedType<uint64_t, struct TransformHandlePackedTag>;

  constexpr TransformHandle() noexcept = default;
  constexpr TransformHandle(Index index, Generation generation) noexcept
    : index_(index)
    ...
    -> TransformHandle
  {
    ...
    return TransformHandle { Index { index }, Generation { generation } };
  }
  ...
  constexpr auto operator<=>(const TransformHandle& other) const noexcept
  {
  ...
  [[nodiscard]] constexpr auto operator==(
    const TransformHandle& other) const noexcept
  {
