...
namespace {
  ...
  auto ProjectToLightBasis(
    const LightBasis& basis, const glm::vec3 world_position) -> glm::vec3
  {
    return glm::vec3 {
    ...
    -> CascadeSphere
  {
    ...
    for (std::size_t i = 0; i < 4U; ++i) {
      ...
      near_diagonal_sq
        = (std::max)(near_diagonal_sq, glm::dot(near_offset, near_offset));
      far_diagonal_sq
        = (std::max)(far_diagonal_sq, glm::dot(far_offset, far_offset));
    }

    const auto optimal_offset
      = ((near_diagonal_sq - far_diagonal_sq) / (2.0F * split_length))
      + split_length * 0.5F;
    const auto center_distance
      = std::clamp(split_length - optimal_offset, 0.0F, split_length);
    const auto sphere_center = near_center + camera_direction * center_distance;

    auto radius_sq = 0.0F;
    for (const auto& corner : corners) {
      const auto corner_offset = corner - sphere_center;
      radius_sq = (std::max)(radius_sq, glm::dot(corner_offset, corner_offset));
    }
    ...
    const std::uint32_t split_index, const std::uint32_t cascade_count) -> float
  {
    ...
    const auto safe_exponent
      = std::isfinite(exponent) && exponent > 0.0F ? exponent : 1.0F;
    ...
    const float near_plane, const float far_plane) -> float
  {
    const auto max_shadow_distance
      = std::isfinite(directional_light.max_shadow_distance)
    ...
    if (directional_light.cascade_split_mode
      ...
      const auto authored_index = (std::min)(cascade_index,
        static_cast<std::uint32_t>(
          directional_light.cascade_distances.size() - 1U));
      authored_end = directional_light.cascade_distances[authored_index];
    } else {
      const auto split_scale
        = ComputeAccumulatedScale(directional_light.distribution_exponent,
  ...
  auto BuildCascadeMatrix(const ResolvedView& resolved_view,
    ...
    const float split_far, const std::uint32_t shadow_resolution)
    -> CascadeMatrixData
  {
    ...
    const auto depth_span
      = (std::max)(camera_far - camera_near, kMinCascadeSpan);
    const auto near_t
      = std::clamp((split_near - camera_near) / depth_span, 0.0F, 1.0F);
    const auto far_t
      = std::clamp((split_far - camera_near) / depth_span, 0.0F, 1.0F);
    ...
    const auto light_projection = MakeReversedZOrthographicProjectionRH_ZO(
      -cascade_radius, cascade_radius, -cascade_radius, cascade_radius, 0.1F,
      2.0F * depth_extent);
    const auto resolution = (std::max)(shadow_resolution, 1U);
  ...
  const -> DirectionalShadowFrameData
{
  ...
  for (std::uint32_t cascade_index = 0U; cascade_index < cascade_count;
    ++cascade_index) {
    const auto cascade_end = ResolveCascadeEnd(
      directional_light, cascade_index, cascade_count, view_near, view_far);
