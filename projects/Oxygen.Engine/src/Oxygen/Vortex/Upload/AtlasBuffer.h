...
public:
  ...
  */
  ...
  OXGN_VRTX_API AtlasBuffer(observer_ptr<Graphics> gfx, std::uint32_t stride,
  ...
  OXGN_VRTX_API ~AtlasBuffer();

  //! Ensure minimum element capacity (create/resize if needed).
  OXGN_VRTX_API auto EnsureCapacity(std::uint32_t min_elements, float slack)
    -> std::expected<EnsureBufferResult, std::error_code>;

  //! Allocate a single element slot.
  OXGN_VRTX_API auto Allocate(std::uint32_t count = 1)
    -> std::expected<ElementRef, std::error_code>;

  //! Release an element (deferred recycle).
  OXGN_VRTX_API auto Release(ElementRef ref, frame::Slot slot) -> void;
  ...
  OXGN_VRTX_NDAPI auto Stride() const noexcept -> std::uint32_t
  {
    return stride_;
  }
  OXGN_VRTX_NDAPI auto CapacityElements() const noexcept -> std::uint32_t
  {
    return capacity_elements_;
  }

  //! Build upload descriptor from ElementRef.
  OXGN_VRTX_API auto MakeUploadDesc(
    const ElementRef& ref, std::uint64_t size_bytes) const
    -> std::expected<oxygen::vortex::upload::UploadBufferDesc, std::error_code>;

  //! Build upload descriptor from raw element index.
  OXGN_VRTX_API auto MakeUploadDescForIndex(
  ...
  */
  OXGN_VRTX_NDAPI auto GetBinding() const noexcept -> Binding
  {
  ...
  */
  OXGN_VRTX_NDAPI auto GetElementIndex(const ElementRef& ref) const noexcept
    -> std::uint32_t
  {
    return ref.element_;
  }
  OXGN_VRTX_NDAPI auto GetSrvIndex(const ElementRef& ref) const noexcept
    -> ShaderVisibleIndex
  {
    return ref.srv_;
  }

  OXGN_VRTX_NDAPI auto GetStats() const noexcept -> Stats { return stats_; }
