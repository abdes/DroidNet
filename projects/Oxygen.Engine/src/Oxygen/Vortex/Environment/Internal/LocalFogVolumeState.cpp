...
namespace {
  ...
  struct LocalFogVolumeSortKey {
    ...
    [[nodiscard]] auto operator<(
      const LocalFogVolumeSortKey& rhs) const noexcept -> bool
    {
  ...
  auto MakeGpuInstance(const scene::environment::LocalFogVolume& local_fog,
    ...
    const ResolvedView& resolved_view) -> LocalFogVolumeGpuInstance
  {
    ...
    const float uniform_scale = std::max(maximum_axis_scale
...
auto LocalFogVolumeState::Prepare(RenderContext& ctx) -> ViewProducts&
{
  ...
  const auto max_instances_per_tile
    = renderer_.GetLocalFogTileMaxInstanceCount();
  ...
  static_cast<void>(scene_ptr->Traverse().Traverse(
    [&gathered_instances, resolved_view](const scene::ConstVisitedNode& visited,
      ...
      auto gathered = SortedLocalFogVolume {
        ...
        .sort_key
        = PackSortKey(static_cast<std::uint16_t>(gathered_instances.size()),
          glm::length(
  ...
  if (!culling_allocation->TryWriteRange(
        std::span<const LocalFogVolumeCullingInstance>(
          cpu_culling_instances_))) {
    return current_products_;
  }
