...
namespace {
  ...
  [[nodiscard]] auto MakeShaderPlane(const Frustum::Plane& plane,
    const bool flip_plane = false) -> std::array<float, 4>
  {
...
LocalFogVolumeTiledCullingPass::~LocalFogVolumeTiledCullingPass()
{
  if (indirect_count_clear_buffer_ != nullptr
    && indirect_count_clear_buffer_->IsMapped()) {
  ...
  const std::uint32_t slice_count) -> bool
{
  ...
  if (tile_data_texture_ != nullptr
    ...
    && tile_data_slice_count_ >= slice_count && tile_data_texture_srv_.IsValid()
    && tile_data_texture_uav_.IsValid()) {
    return true;
  }
  ...
  if (!srv_handle.IsValid()) {
    throw std::runtime_error("LocalFogVolumeTiledCullingPass: failed to "
                             "allocate tile-data texture SRV");
  }
  ...
  if (!uav_handle.IsValid()) {
    throw std::runtime_error("LocalFogVolumeTiledCullingPass: failed to "
                             "allocate tile-data texture UAV");
  }
  ...
  const std::uint32_t tile_count) -> bool
{
  ...
  if (occupied_tile_buffer_ != nullptr && indirect_args_buffer_ != nullptr
    ...
    && occupied_tile_buffer_uav_.IsValid()
    && occupied_tile_buffer_srv_.IsValid()
  ...
  auto new_indirect_args = create_device_buffer(
    "Vortex.Environment.LocalFogTileDrawArgs", sizeof(std::uint32_t) * 4U);
  ...
  auto occupied_srv_handle
    = allocator.AllocateRaw(graphics::ResourceViewType::kStructuredBuffer_SRV,
      graphics::DescriptorVisibility::kShaderVisible);
  ...
  internal::LocalFogVolumeState::ViewProducts& products) -> RecordState
{
  ...
  auto state = RecordState {
    .requested = renderer_.GetLocalFogEnabled()
      && ctx.current_view.with_local_fog && products.buffer_ready
  ...
  recorder.CopyBuffer(*indirect_args_buffer_, 0U, *indirect_count_clear_buffer_,
    0U, sizeof(std::uint32_t) * 4U);
  ...
  graphics::GpuEventScope pass_scope(recorder,
    "Vortex.Stage14.LocalFogTiledCulling",
  ...
  LOG_F(INFO, "local_fog_hzb_consumed={} screen_hzb={}x{} mips={}",
