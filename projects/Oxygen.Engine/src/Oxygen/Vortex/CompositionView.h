...
/*!
  ...
  Vortex renderer. It decouples the user's intent from the renderer's internal
...
struct CompositionView {
  ...
  //!   in the input span provided to the renderer) acts as the tie-breaker,
  ...
  //! Optional 3D source. If set, the renderer executes the full
  ...
  /*
    FUTURE EXTENSIONS:
    - Temporal History: Persistent ViewId allows the renderer to cache motion
    vectors and history buffers across frames for TAA, Motion Blur, and
    Upscaling.
    - Renderer Overrides: Support for per-view shader permutations or quality
    settings.
