  ...
  Fp16EligibilityRequiresStableCompleteFramesAndPreservesExposure)
{
  ...
  const auto step
    = [&](std::uint64_t sequence, Revisions revisions, const Signal& signal,
        std::uint32_t expected = 1024U, bool invalidate = false,
        bool metering_available
        = true) -> std::pair<ExposureStateData, ExposureCompletedStatus> {
    config = SharedConfig(settings, {}, revisions.settings);
    return EligibilityStep(signal, config, sequence, revisions.layout, expected,
  ...
  EXPECT_EQ(step(1U,
              ...
              ordinary)
              .first.fp16_eligible_streak,
    1U);
  const auto eligible = step(2U,
    {
      .layout = 7U,
      .settings = 1U,
    },
...
NOLINT_TEST_F(ExposureGpuTest, Fp16EligibilityBelongsToEachBorrowingImage)
{
  ...
  const auto root = CompositionView::ViewStateHandle {
    10U,
  };
  const auto borrower = CompositionView::ViewStateHandle {
    20U,
  };
  const auto ordinary = Uniform(1.0F, 4U, 4U);
  const std::array wide_pixels {
    Pixel {
      0x1p30F,
      0x1p30F,
      0x1p30F,
  ...
  const auto step
    = [&](std::uint64_t sequence, bool sharing,
        const Signal& signal) -> oxygen::vortex::ExposureStateData {
    ctx_.current_view.view_state_handle = sharing ? borrower : root;
    ctx_.current_view.view_id = ViewId {
      sharing ? 20U : 10U,
    };
    return EligibilityStep(
      signal, config, sequence, 1U, 1024U, false, sharing ? &source : nullptr)
      .first;
  };
  ...
  CompletedPrecisionAdmissionTracksViewSettingsLayoutAndDiagnostics)
{
  ...
  const auto ordinary = Uniform(1.0F, 4U, 4U);
  const auto invalid = Uniform(std::numeric_limits<float>::infinity(), 4U, 4U);
  auto layout = std::uint64_t {
    7U,
  };
  const auto step
    = [&](bool expected_candidate, std::uint32_t expected_streak,
        const Signal& signal,
        bool diagnostic = false) -> postprocess::ExposurePass::StateLease {
    WaitForQueueIdle();
    ctx_.frame_sequence = frame::SequenceNumber {
      ++sequence_,
    };
  ...
  ExposureGpuTest, PrecisionStatusRetriesTransportWithoutEarlyAdmission)
{
  ...
  const auto begin = [&] -> postprocess::ExposurePass::StateLease {
    WaitForQueueIdle();
    ctx_.frame_sequence = frame::SequenceNumber {
      ++sequence_,
    };
