...
NOLINT_TEST_F(ExposureGpuTest, ConservedTwoBinMassAndIndependentMeter)
{
  ...
  EXPECT_EQ(result.histogram.at(bin), 323U * (4095U - upper));
  EXPECT_EQ(result.histogram.at(bin + 1U), 323U * upper);
  ...
  EXPECT_EQ(result.histogram.at(256), 323U);
  EXPECT_EQ(result.histogram.at(257), 323U);
  EXPECT_EQ(result.histogram.at(260), 0U);
}
...
NOLINT_TEST_F(ExposureGpuTest, MaximumGridMassRemainsBounded)
{
  ...
  EXPECT_EQ(std::accumulate(
              result.histogram.begin(), result.histogram.begin() + 256, 0U),
    1073479680U);
  EXPECT_EQ(result.histogram.at(256), 262144U);
  EXPECT_EQ(result.histogram.at(257), 262144U);
}
...
NOLINT_TEST_F(ExposureGpuTest, DarkValidAndZeroMaskInvalidRemainDistinct)
{
  ...
  EXPECT_EQ(result.histogram.at(258), 1U);
  EXPECT_EQ(result.histogram.at(261), 1U);
  EXPECT_EQ(result.histogram.at(0), 0U);
  ...
  EXPECT_EQ(invalid.histogram.at(257), 0U);
...
NOLINT_TEST_F(ExposureGpuTest, NonfiniteSamplesAreRejectedAndNeverBlack)
{
  ...
  EXPECT_EQ(result.histogram.at(256), 1U);
  EXPECT_EQ(result.histogram.at(257), 1U);
  EXPECT_EQ(result.histogram.at(258), 0U);
  EXPECT_EQ(result.histogram.at(260), 2U);
...
NOLINT_TEST_F(ExposureGpuTest, DarkInfluenceDoesNotAttenuatePositiveBinZeroMass)
{
  ...
  EXPECT_GT(zero.histogram.at(0), 0U);
  EXPECT_GT(zero.histogram.at(1), 0U);
  EXPECT_EQ(zero.histogram.at(0) + zero.histogram.at(1), 4095U);
  settings.black_influence = .5F;
  const auto half = Run(signal, settings);
  EXPECT_EQ(half.histogram.at(0), zero.histogram.at(0) + 2048U);
  EXPECT_EQ(half.histogram.at(1), zero.histogram.at(1));
}
...
NOLINT_TEST_F(ExposureGpuTest, BilinearMaskUsesOnlyClampedLinearRed)
{
  ...
  EXPECT_EQ(result.histogram.at(102), 2048U);
  EXPECT_EQ(result.histogram.at(260), 0U);
  const auto high_mask = Uniform(4.0F);
  EXPECT_EQ(Run(Uniform(.25F), {}, 0.0F, &high_mask).histogram.at(102), 4095U);
  const auto low_mask = Uniform(-1.0F);
  EXPECT_EQ(Run(Uniform(.25F), {}, 0.0F, &low_mask).histogram.at(257), 0U);
}
  ...
  ExposureGpuTest, ProfilesAndZeroRadiusUseNormalizedContentCoordinates)
{
  ...
  EXPECT_EQ(Run(signal, settings).histogram.at(102), 6825U);
  settings.metering_mode = engine::MeteringMode::kSpot;
  settings.spot_meter_radius = 1.0e-20F;
  EXPECT_EQ(Run(signal, settings).histogram.at(102), 4095U);
  settings.spot_meter_radius = 0.0F;
  EXPECT_EQ(Run(signal, settings).histogram.at(102), 4095U);
  EXPECT_EQ(Run(Uniform(.25F, 2U, 1U), settings).histogram.at(257), 0U);
}
...
NOLINT_TEST_F(ExposureGpuTest, ContentRectangleExcludesOutputBars)
{
  ...
  const auto result = Run(signal);
  EXPECT_EQ(result.histogram.at(102), 8190U);
  EXPECT_EQ(result.histogram.at(256), 2U);
  EXPECT_NEAR(result.state.raw_metered_ev, std::log2(.25 / .18), 2e-4);
...
{
  std::vector<Pixel> pixels(static_cast<std::size_t>(512U) * 512U,
    Pixel {
      8,
      8,
      8,
  ...
  std::fill_n(pixels.begin(), pixels.size() / 2,
    Pixel {
      .25F,
      .25F,
      .25F,
...
NOLINT_TEST_F(ExposureGpuTest, EightKOutputStillUsesAtMost512SquaredSamples)
{
  ...
  EXPECT_EQ(result.histogram.at(256), 262144U);
  EXPECT_EQ(result.histogram.at(102), 1073479680U);
}
...
NOLINT_TEST_F(ExposureGpuTest, MovingEdgeHasBoundedGridSamplingError)
{
  ...
  for (const unsigned edge : {
    ...
    EXPECT_EQ(result.histogram.at(153), (edge / 2U) * 4095U);
    const auto sampled_bright_pixels = edge / 2U;
    const double exact_sample_ev
      = -2.0 + (5.0 * sampled_bright_pixels / 512.0) - std::log2(.18);
  ...
  ExposureGpuTest, SingleBrightPixelRecordsSamplingAliasingWithoutAreaAveraging)
{
  ...
  for (const unsigned x : {
    ...
    const auto result = Run(MakeSignal(1024U, 1U, pixels), settings);
    EXPECT_EQ(result.histogram.at(153), (x % 2U) * 4095U);
  ...
  ExposureGpuTest, NarrowCdfBoundariesRetainIntegerAndFractionalProductBits)
{
  ...
  for (unsigned low_pixels : {
         1U,
         512U,
         32768U,
         131072U,
       }) {
    std::vector<Pixel> pixels(262144U,
      Pixel {
        .25F,
        .25F,
        .25F,
    ...
    const auto result = Run(MakeSignal(512U, 512U, pixels), settings);
    EXPECT_EQ(result.histogram.at(51), low_pixels * 4095U);
    EXPECT_EQ(result.histogram.at(102), (262144U - low_pixels) * 4095U);
    EXPECT_NEAR(
      result.state.raw_metered_ev, (-11.0 / 3.0) - std::log2(.18), 2e-4)
      << low_pixels;
  }
}
  ...
  ExposureGpuTest, DarkCountersSeparateBlackPositiveBelowWindowAndNegative)
{
  ...
  const auto result = Run(MakeSignal(3U, 1U, pixels));
  EXPECT_EQ(result.histogram.at(256), 3U);
  EXPECT_EQ(result.histogram.at(257), 3U);
  EXPECT_EQ(result.histogram.at(258), 1U);
  EXPECT_EQ(result.histogram.at(259), 1U);
  EXPECT_EQ(result.histogram.at(260), 0U);
  EXPECT_EQ(result.histogram.at(261), 3U);
  EXPECT_EQ(result.state.flags & 31U, 27U);
  ...
  AllNonfiniteInputKeepsAutoEvZeroFallbackIndependentOfManualEv)
{
  ...
  EXPECT_EQ(result.histogram.at(256), 0U);
  EXPECT_EQ(result.histogram.at(260), 1U);
