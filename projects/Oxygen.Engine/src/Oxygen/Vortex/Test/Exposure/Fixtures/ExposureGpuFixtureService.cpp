  ...
  std::optional<float> camera_ev, bool temporary_unit) -> Snapshot
{
  ...
  auto snapshot
    = Snapshot { .state = Read<ExposureStateData>(
  ...
  service.Execute(ctx_.current_view.view_id, ctx_, textures,
    {
      ...
      .scene_signal_srv = signal.srv,
      .bloom_texture_srv
      = options.bloom ? options.bloom->srv : kInvalidShaderVisibleIndex,
      .scene_fallback
      = options.fallback ? options.fallback->texture.get() : nullptr,
      .scene_fallback_srv
      = options.fallback ? options.fallback->srv : kInvalidShaderVisibleIndex,
  ...
  -> std::pair<ExposureStateData, ExposureCompletedStatus>
{
  ...
  EXPECT_EQ(
    status.view_state_identity[1], static_cast<std::uint32_t>(lifetime >> 32U));
