...
public:
  ...
  using CoordinatorT = vortex::upload::InlineTransfersCoordinator;

  OXGN_VRTX_API TransformUploader(observer_ptr<Graphics> gfx,
  ...
  OXGN_VRTX_API ~TransformUploader();
  ...
  OXGN_VRTX_NDAPI auto GetWorldsSrvIndex() const -> ShaderVisibleIndex;

  //! Get the shader-visible index for normal matrices buffer
  OXGN_VRTX_NDAPI auto GetNormalsSrvIndex() const -> ShaderVisibleIndex;

  //! Get read-only access to world matrices for debugging
  OXGN_VRTX_NDAPI auto GetWorldMatrices() const noexcept
