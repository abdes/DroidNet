...
namespace {
  ...
  auto MakeDrawCommand(
    const DrawMetadata& metadata, const std::uint32_t draw_index) -> DrawCommand
  {
    return DrawCommand {
      .draw_index = draw_index,
      .index_count = metadata.is_indexed != 0U ? metadata.index_count
  ...
  auto StableSortFrontToBack(std::vector<SortableDrawCommand>& draw_commands)
    -> void
  {
    std::ranges::stable_sort(draw_commands,
      [](const SortableDrawCommand& lhs, const SortableDrawCommand& rhs)
        -> bool { return lhs.front_to_back_key < rhs.front_to_back_key; });
  }
