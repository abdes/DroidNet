...
auto RegisterTextureView(
  Graphics& gfx, graphics::Texture& texture,
...
auto NeedsStageTextureRebuild(const std::shared_ptr<graphics::Texture>& texture,
...
auto BuildBasePassColorClearFramebuffer(
  ...
  -> graphics::GraphicsPipelineDesc
{
  ...
  return graphics::GraphicsPipelineDesc::Builder {}
    ...
    .SetDebugName(
      alpha_test ? "Vortex.BasePass.GBuffer.Masked"
  ...
  -> graphics::GraphicsPipelineDesc
{
  ...
  return graphics::GraphicsPipelineDesc::Builder {}
    ...
    .SetFramebufferLayout(graphics::FramebufferLayoutDesc {
      ...
      = std::vector<Format> { scene_textures.GetVelocity()->GetDescriptor().format },
      .depth_stencil_format = scene_textures.GetSceneDepth().GetDescriptor().format,
      .sample_count = scene_textures.GetSceneColor().GetDescriptor().sample_count,
      .sample_quality
      = scene_textures.GetSceneColor().GetDescriptor().sample_quality,
    ...
    .SetRootBindings(std::span<const graphics::RootBindingItem>(
      root_bindings.data(), root_bindings.size()))
    .SetDebugName(
      alpha_test ? "Vortex.BasePass.VelocityAux.Masked"
...
auto TryGetVelocityDrawMetadata(
  const PreparedSceneFrame& prepared_frame, const std::uint32_t draw_index)
  -> const VelocityDrawMetadata*
{
...
auto IsMaskedDraw(
  const PreparedSceneFrame& prepared_frame, const BasePassDrawCommand& draw_command)
  -> bool
{
  ...
  RenderContext& ctx, SceneTextures& scene_textures) -> BasePassExecutionResult
{
  ...
  if (requires_velocity_aux) {
    auto& velocity_base_copy = EnsureStageTexture(*gfx, velocity_base_copy_,
      "VelocityBasePassCopy", *scene_textures.GetVelocity(), false, true, false);
    ...
    if (NeedsVelocityAuxFramebufferRebuild(
          velocity_aux_framebuffer_, scene_textures,
          velocity_motion_vector_world_offset_)) {
      velocity_aux_framebuffer_ = gfx->CreateFramebuffer(
        BuildVelocityAuxFramebuffer(
    ...
    if (NeedsVelocityAuxColorClearFramebufferRebuild(
          ...
          velocity_motion_vector_world_offset_)) {
      velocity_aux_color_clear_framebuffer_ = gfx->CreateFramebuffer(
        BuildVelocityAuxColorClearFramebuffer(
          velocity_motion_vector_world_offset_));
    }
    ...
    recorder->FlushBarriers();
    recorder->CopyTexture(*scene_textures.GetVelocity(), graphics::TextureSlice {},
    ...
    const auto velocity_base_copy_srv
      = RegisterTextureView(*gfx, velocity_base_copy,
        MakeTextureSrvDesc(velocity_base_copy));
    ...
    recorder->RequireResourceState(
      *scene_textures.GetVelocity(), graphics::ResourceStates::kUnorderedAccess);
    recorder->FlushBarriers();

    {
      ...
      const auto extent = scene_textures.GetExtent();
      recorder->Dispatch(
        (extent.x + (kVelocityMergeThreadGroupSize - 1U))
