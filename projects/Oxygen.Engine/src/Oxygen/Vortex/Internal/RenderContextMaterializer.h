...
public:
  using ValidationIssue = typename RendererT::ValidationIssue;
  using ValidationReport = typename RendererT::ValidationReport;
  using FrameSessionInput = typename RendererT::FrameSessionInput;
  using OutputTargetInput = typename RendererT::OutputTargetInput;
  ...
  explicit BasicRenderContextMaterializer(RendererT& renderer)
    : renderer_(oxygen::observer_ptr<RendererT>(&renderer))
  {
  }
    ...
    -> std::expected<ValidatedSinglePassHarnessContext, ValidationReport>
  {
    ...
    return ValidatedSinglePassHarnessContext(*renderer_, session, view_id,
      staging.output_target->framebuffer, resolved_view, prepared_frame,
...
private:
  ...
  observer_ptr<RendererT> renderer_;
};
