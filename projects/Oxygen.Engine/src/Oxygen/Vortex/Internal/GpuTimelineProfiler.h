...
public:
  OXGN_VRTX_API virtual ~GpuTimelineSink() = default;
...
public:
  ...
  OXGN_VRTX_API auto OnFrameStart(frame::SequenceNumber frame_sequence) -> void;
  OXGN_VRTX_API auto OnFrameRecordTailResolve() -> void;

  OXGN_VRTX_API auto BeginScope(graphics::CommandRecorder& recorder,
    const graphics::GpuProfileScopeInfo& info,
    graphics::GpuProfileCollectorState& state) -> void override;

  OXGN_VRTX_API auto EndScope(graphics::CommandRecorder& recorder,
    graphics::GpuProfileCollectorState& state) -> void override;
