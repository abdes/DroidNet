  ...
  const oxygen::ViewPort& lhs, const oxygen::ViewPort& rhs) noexcept -> bool
{
  ...
  return glm::all(
           glm::epsilonEqual(glm::vec2 { lhs.top_left_x, lhs.top_left_y },
             glm::vec2 { rhs.top_left_x, rhs.top_left_y }, epsilon))
    && glm::all(glm::epsilonEqual(glm::vec2 { lhs.width, lhs.height },
      glm::vec2 { rhs.width, rhs.height }, epsilon))
    && glm::all(glm::epsilonEqual(glm::vec2 { lhs.min_depth, lhs.max_depth },
  ...
  const CurrentState& current) -> Snapshot
{
  ...
  const bool invalidated = !entry.previous_valid
    ...
    || !Mat4Equal(entry.current.stable_projection_matrix,
      current.stable_projection_matrix, epsilon);
...
auto PreviousViewHistoryCache::TouchStateless(const CurrentState& current) const
  -> Snapshot
{
  return Snapshot {
    .current = current,
