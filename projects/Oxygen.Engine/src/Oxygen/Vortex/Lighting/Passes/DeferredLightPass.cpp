...
struct DeferredLightDraw {
  ...
  DeferredLocalLightDrawMode draw_mode { DeferredLocalLightDrawMode::kOutsideVolume };
  ...
  const float near_clip, const DeferredLightConstants& constants) -> bool
{
  ...
  const auto radial_sq
    = (std::max)(glm::dot(to_camera, to_camera) - axial_distance * axial_distance,
      0.0F);
  const auto expanded_radius
    = (std::max)(axial_distance + near_clip, 0.0F) * outer_tangent + near_clip;
  ...
  -> DeferredLocalLightDrawMode
{
  ...
  const auto near_clip = (std::max)(resolved_view->NearPlane(), 0.001F) * 2.0F;
  ...
  case DeferredLightKind::kPoint:
    return IsCameraInsidePointLightVolume(camera, near_clip,
             DeferredLightConstants {
               .light_position_and_radius = draw.packet.light_position_and_radius,
...
auto BuildDeferredLocalPipelineDesc(const SceneTextures& scene_textures,
  ...
  const DeferredLocalLightDrawMode draw_mode) -> graphics::GraphicsPipelineDesc
{
  auto root_bindings = BuildVortexRootBindings();
...
auto GenerateSphereVertices() -> std::vector<glm::vec4>
{
  ...
  const auto ring_vertex = [](const std::uint32_t ring,
                             const std::uint32_t slice) -> glm::vec4 {
    ...
    return glm::vec4(
      sin_phi * std::cos(theta), std::cos(phi), sin_phi * std::sin(theta), 1.0F);
  };
...
DeferredLightPass::~DeferredLightPass()
{
  ...
  for (auto* buffer : { deferred_light_constants_buffer_.get(), point_geometry_buffer_.get(),
  ...
  const graphics::Texture* directional_shadow_surface) -> ExecutionState
{
  ...
  const auto ensure_geometry_buffer =
    [this, &gfx](std::shared_ptr<graphics::Buffer>& buffer,
      ...
      std::string_view debug_name, std::span<const glm::vec4> vertices) -> void {
  ...
  for (const auto& packet : packets.local_lights) {
    ...
    draws.push_back(DeferredLightDraw {
      ...
      = kind == DeferredLightKind::kPoint ? point_geometry_vertex_count_
  ...
  const auto ensure_pass_constants = [&](const std::uint32_t required_slots)
    ...
    CHECK_F(required_slots > 0U,
      "DeferredLightPass: deferred lighting requires at least one constants slot");
    if (deferred_light_constants_buffer_ != nullptr
    ...
    for (std::uint32_t i = 0U; i < required_slots; ++i) {
      deferred_light_constants_indices_.push_back(RegisterBufferViewIndex(
        *gfx, *deferred_light_constants_buffer_,
        graphics::BufferViewDescription {
          .view_type = graphics::ResourceViewType::kConstantBuffer,
          .visibility = graphics::DescriptorVisibility::kShaderVisible,
          .range = { static_cast<std::uint64_t>(i) * kDeferredLightConstantsStride,
            kDeferredLightConstantsStride },
        }));
    }
    deferred_light_constants_slot_count_ = required_slots;
  ...
  if (NeedsDirectionalFramebufferRebuild(directional_framebuffer_, scene_textures)) {
  ...
  auto* mapped_bytes = static_cast<std::byte*>(deferred_light_constants_mapped_ptr_);
  for (std::size_t i = 0; i < draws.size(); ++i) {
    auto constants = DeferredLightConstants {};
    const auto& draw = draws[i];
    if (draw.kind == DeferredLightKind::kDirectional && packets.directional.has_value()) {
      ...
      constants.light_color_and_intensity = draw.packet.light_color_and_intensity;
      constants.light_direction_and_falloff = draw.packet.light_direction_and_falloff;
  ...
  auto recorder = gfx->AcquireCommandRecorder(queue_key, "LightingService DeferredLighting");
  if (!recorder) {
    return state;
  }
  graphics::GpuEventScope stage_scope(*recorder, "Vortex.Stage12.DeferredLighting",
  ...
  for (std::size_t i = 0; i < draws.size(); ++i) {
    ...
    CHECK_NOTNULL_F(local_framebuffer_.get(),
      "DeferredLightPass: local framebuffer must exist before local-light draws");
    recorder->RequireResourceState(
