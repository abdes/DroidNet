...
class NativeEventHolder : public Object {
public:
...
public:
  template <NativeEventType T> static auto Create()
  {
  ...
  auto SetHandled() const noexcept -> void { handled_ = true; }

  [[nodiscard]] auto NativeEvent() const noexcept
  {
    return impl_->NativeEvent();
  }
  ...
  [[nodiscard]] auto NativeEventAs() const noexcept(false)
  {
    ...
    return my_impl_type == requested_type
      ? static_cast<T*>(impl_->NativeEvent())
      : nullptr;
  }
