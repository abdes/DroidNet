...
{
    SDL_DisplayID* display_ids { nullptr };
    auto _ = Finally([this, display_ids] {
        // Free the memory allocated for the display_ids returned by SDL
        sdl_->Free(display_ids);
    });
    int display_count { 0 };
    display_ids = sdl_->GetDisplays(&display_count);

    auto s_display_ids = std::span(display_ids, static_cast<std::size_t>(display_count));
    std::vector<std::unique_ptr<platform::Display>> displays;
    std::ranges::transform(s_display_ids, std::back_inserter(displays),
        [](auto const& display_id) {
            return std::make_unique<Display>(display_id);
        });
    ...
    -> std::unique_ptr<platform::Display>
{
    ...
    if (const auto it = std::ranges::find(s_display_ids, display_id); it != s_display_ids.end()) {
...
auto Platform::PlatformImpl::PollOne(std::optional<PlatformEvent<SDL_Event>>& event) -> size_t
{
    if (PlatformEvent<SDL_Event> platform_event {};
        sdl_->PollEvent(platform_event.NativeEventPtr())) {
        event.emplace(std::move(platform_event));
        return 1;
    }
    return 0;
