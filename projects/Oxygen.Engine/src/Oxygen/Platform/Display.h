...
public:
  ...
  OXGN_PLAT_API explicit Display(IdType display_id);
  OXGN_PLAT_API virtual ~Display();
  ...
  friend auto to_string(const Display& self) -> std::string;

private:
  IdType display_id_ { kInvalidDisplayId };
};

auto to_string(const DisplayOrientation& orientation) -> std::string;
auto to_string(const Display& self) -> std::string;
