...
static void ImGui_ImplSDL3_PlatformSetImeData(
...
// undocumented)
ImGuiKey ImGui_ImplSDL3_KeyEventToImGuiKey(
...
bool ImGui_ImplSDL3_ProcessEvent(const SDL_Event* event)
{
  ...
  case SDL_EVENT_KEY_UP: {
    ...
    io.SetKeyEventNativeData(key, (int)event->key.key, (int)event->key.scancode,
      (int)event->key
        .scancode); // To support legacy indexing (<1.87 user code). Legacy
...
static void ImGui_ImplSDL3_SetupPlatformHandles(
  ImGuiViewport* viewport, SDL_Window* window)
{
...
static bool ImGui_ImplSDL3_Init(
  SDL_Window* window, SDL_Renderer* renderer, void* sdl_gl_context)
{
  ...
  snprintf(bd->BackendPlatformName, sizeof(bd->BackendPlatformName),
    ...
    SDL_MINOR_VERSION, SDL_MICRO_VERSION, SDL_VERSIONNUM_MAJOR(ver_linked),
...
#  if SDL_HAS_CAPTURE_AND_GLOBAL_MOUSE
  ...
  const char* capture_and_global_state_whitelist[]
...
#  endif
  ...
  platform_io.Platform_OpenInShellFn
    = [](ImGuiContext*, const char* url) { return SDL_OpenURL(url) == 0; };
...
bool ImGui_ImplSDL3_InitForSDLRenderer(
...
static void ImGui_ImplSDL3_UpdateMouseCursor()
{
  ...
  if (io.MouseDrawCursor || imgui_cursor == ImGuiMouseCursor_None) {
    ...
    if (bd->MouseLastCursor != expected_cursor) {
      SDL_SetCursor(
        expected_cursor); // SDL function doesn't have an early out (see #6113)
...
static void ImGui_ImplSDL3_UpdateGamepadButton(ImGui_ImplSDL3_Data* bd,
  ImGuiIO& io, ImGuiKey key, SDL_GamepadButton button_no)
{
...
static void ImGui_ImplSDL3_UpdateGamepadAnalog(ImGui_ImplSDL3_Data* bd,
  ImGuiIO& io, ImGuiKey key, SDL_GamepadAxis axis_no, float v0, float v1)
{
...
static void ImGui_ImplSDL3_UpdateGamepads()
{
  ...
  ImGui_ImplSDL3_UpdateGamepadButton(
    bd, io, ImGuiKey_GamepadStart, SDL_GAMEPAD_BUTTON_START);
  ImGui_ImplSDL3_UpdateGamepadButton(
    bd, io, ImGuiKey_GamepadBack, SDL_GAMEPAD_BUTTON_BACK);
  ...
  ImGui_ImplSDL3_UpdateGamepadButton(
    bd, io, ImGuiKey_GamepadDpadLeft, SDL_GAMEPAD_BUTTON_DPAD_LEFT);
  ImGui_ImplSDL3_UpdateGamepadButton(
    bd, io, ImGuiKey_GamepadDpadRight, SDL_GAMEPAD_BUTTON_DPAD_RIGHT);
  ImGui_ImplSDL3_UpdateGamepadButton(
    bd, io, ImGuiKey_GamepadDpadUp, SDL_GAMEPAD_BUTTON_DPAD_UP);
  ImGui_ImplSDL3_UpdateGamepadButton(
    bd, io, ImGuiKey_GamepadDpadDown, SDL_GAMEPAD_BUTTON_DPAD_DOWN);
  ImGui_ImplSDL3_UpdateGamepadButton(
    bd, io, ImGuiKey_GamepadL1, SDL_GAMEPAD_BUTTON_LEFT_SHOULDER);
  ImGui_ImplSDL3_UpdateGamepadButton(
    bd, io, ImGuiKey_GamepadR1, SDL_GAMEPAD_BUTTON_RIGHT_SHOULDER);
