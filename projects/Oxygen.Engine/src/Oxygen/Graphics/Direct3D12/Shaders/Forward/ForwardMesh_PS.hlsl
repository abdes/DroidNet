...
static float3 ComputeForwardIblTerm(
    ForwardEnvironmentState env_state,
    ...
    SamplerState linear_sampler)
{
    ...
    if (env_state.data.sky_light.irradiance_map_slot != K_INVALID_BINDLESS_INDEX
        && env_state.data.sky_light.prefilter_map_slot != K_INVALID_BINDLESS_INDEX) {
        ...
        ibl_specular = pref_map.SampleLevel(linear_sampler, cube_R,
                (float)env_state.data.sky_light.prefilter_max_mip * surf.roughness).rgb
            * env_state.data.sky_light.tint_rgb
            * env_state.data.sky_light.radiance_scale
            * env_state.data.sky_light.specular_intensity;
    } else {
        const float sky_max_mip = (sky_slot == env_state.data.sky_sphere.cubemap_slot)
            ? (float)env_state.data.sky_sphere.cubemap_max_mip
            : (float)env_state.data.sky_light.cubemap_max_mip;
        ibl_specular = sky_cube.SampleLevel(
                linear_sampler, cube_R, sky_max_mip * surf.roughness).rgb
            * env_state.data.sky_light.tint_rgb
            * env_state.data.sky_light.radiance_scale
            * env_state.data.sky_light.specular_intensity;
...
static ForwardLightingTerms ComputeForwardLightingTerms(
    VSOutput input,
    MaterialSurface surf,
    ...
    SamplerState linear_sampler)
{
    ...
    terms.directional_direct = AccumulateDirectionalLights(
        input.world_pos, env_state.atmosphere, shadow_normal,
        N, V, NdotV, F0, base_rgb, surf.metalness, surf.roughness);
    terms.positional_direct = AccumulatePositionalLightsClustered(
        input.world_pos,
        input.position.xy,
        ...
        surf.metalness,
        surf.roughness);
    terms.direct_gates = AccumulateDirectionalLightGatesDebug(
        input.world_pos, env_state.atmosphere, shadow_normal,
        N, V, NdotV, F0, base_rgb, surf.metalness, surf.roughness);
    terms.direct_brdf_core = AccumulateDirectionalLightsBrdfCore(
        input.world_pos, env_state.atmosphere, shadow_normal,
        N, V, NdotV, F0, base_rgb, surf.metalness, surf.roughness);
...
float4 PS(VSOutput input) : SV_Target0 {
        ...
        if (g_DrawIndex < draw_count) {
            ...
            if (meta.material_handle < material_count) {
                ...
                if ((mat.flags & MATERIAL_FLAG_ALPHA_TEST) != 0u) {
                    ...
                    if (!((mat.flags & MATERIAL_FLAG_NO_TEXTURE_SAMPLING) != 0u) && mat.base_color_texture_index != K_INVALID_BINDLESS_INDEX) {
...
#if 1
    //=== Normal PBR Path ===
    const MaterialSurface surf = EvaluateMaterialSurface(
        input.world_pos, input.world_normal, input.world_tangent,
        input.world_bitangent, input.uv, g_DrawIndex, input.is_front_face);
    ...
    final_color = AccumulateDirectionalLightsRawLambert(surf.N, surf.base_rgb * input.color);
 ...
 #if !defined(DEBUG_DIRECT_LIGHTING_ONLY) && !defined(DEBUG_DIRECT_LIGHTING_FULL) \
