...
CommandList::CommandList(
  const std::string_view name, QueueRole type, const Graphics* gfx)
  : Base(name, type)
{
  DCHECK_NOTNULL_F(gfx, "Graphics context cannot be null!");

  D3D12_COMMAND_LIST_TYPE d3d12_type; // NOLINT(*-init-variables)
  switch (type) // NOLINT(clang-diagnostic-switch-enum) - these are the only
                // valid values
  {
  ...
  default:
    throw std::runtime_error(
      fmt::format("Unsupported CommandListType: {}", nostd::to_string(type)));
  }
  ...
  try {
    ThrowOnFailed(device->CreateCommandAllocator(
                    d3d12_type, IID_PPV_ARGS(&command_allocator_)),
      fmt::format(
        "could not create {} Command Allocator", nostd::to_string(type)));
    NameObject(
      command_allocator_, GetNameForType(d3d12_type) + "Command Allocator");

    ThrowOnFailed(device->CreateCommandList(0, d3d12_type, command_allocator_,
    ...
    ThrowOnFailed(command_list_->Close(),
      "could not close command list after it was created");
...
CommandList::~CommandList() noexcept { ReleaseCommandList(); }
...
void CommandList::OnBeginRecording()
{
  ...
  ThrowOnFailed(
    command_allocator_->Reset(), "could not reset the command allocator");
  ThrowOnFailed(command_list_->Reset(command_allocator_, nullptr),
    "could not reset the command list");
}
