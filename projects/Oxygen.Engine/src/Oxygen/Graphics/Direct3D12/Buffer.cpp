  ...
  -> void
{
  ...
  if (desc_.memory == BufferMemory::kDeviceLocal) {
    ...
    // A more robust implementation would track updates and apply them at
    // submission time
    LOG_F(ERROR,
      "Direct update of device-local buffer {} not implemented, use staging "
      "buffers",
      Base::GetName());
    throw std::runtime_error(
      "Direct update of device-local buffer not implemented");
  }
...
auto Buffer::GetSize() const noexcept -> size_t { return desc_.size_bytes; }
  ...
  const BufferRange& range) const -> NativeView
{
  ...
  if ((desc_.usage & BufferUsage::kConstant) != BufferUsage::kConstant) {
    LOG_F(WARNING,
      "Creating constant buffer view for buffer {} without kConstant usage "
      "flag",
      Base::GetName());
  }
  ...
  const D3D12_CONSTANT_BUFFER_VIEW_DESC cbv_desc = {
    .BufferLocation
    = GetResource()->GetGPUVirtualAddress() + resolved.offset_bytes,
    // CB size must be a multiple of 256 bytes (D3D12 requirement)
    .SizeInBytes
    = static_cast<UINT>(
        (resolved.size_bytes == ~0ull ? desc_.size_bytes : resolved.size_bytes)
        + 255)
      & ~255u,
  };
  ...
  Format format, BufferRange range, uint32_t stride) const -> NativeView
{
  ...
  if (stride > 0 && format != Format::kUnknown) {
    LOG_F(WARNING,
      "Buffer {}: Both format and stride specified for SRV; format will be "
      "ignored",
      Base::GetName());
  }
  ...
  if (view_desc.IsStructuredBuffer()) {
    ...
    srv_desc.Buffer.NumElements = static_cast<UINT>(
      (resolved.size_bytes == ~0ull ? desc_.size_bytes : resolved.size_bytes)
      / stride);
  ...
  Format format, BufferRange range, uint32_t stride) const -> NativeView
{
  ...
  if ((desc_.usage & BufferUsage::kStorage) != BufferUsage::kStorage) {
    LOG_F(WARNING, "Creating UAV for buffer {} without kStorage usage flag",
      Base::GetName());
  }
  ...
  if (stride > 0 && format != Format::kUnknown) {
    LOG_F(WARNING,
      "Buffer {}: Both format and stride specified for UAV; format will be "
      "ignored",
      Base::GetName());
  }
  ...
  if (view_desc.IsStructuredBuffer()) {
    ...
    uav_desc.Buffer.NumElements
      = static_cast<UINT>(resolved_size_bytes / stride);
    ...
    uav_desc.Buffer.NumElements
      = static_cast<UINT>(resolved_size_bytes / element_size);
