...
public:
  ...
  // clang-format off
    ...
    HRESULT STDMETHODCALLTYPE EnumerateMetaCommands(UINT* pNumMetaCommands, D3D12_META_COMMAND_DESC*) override { if (pNumMetaCommands != nullptr) { *pNumMetaCommands = 0; } return E_NOTIMPL; }
    HRESULT STDMETHODCALLTYPE EnumerateMetaCommandParameters(REFGUID, D3D12_META_COMMAND_PARAMETER_STAGE, UINT* pTotalStructureSizeInBytes, UINT* pParameterCount, D3D12_META_COMMAND_PARAMETER_DESC*) override { if (pTotalStructureSizeInBytes != nullptr) { *pTotalStructureSizeInBytes = 0; } if (pParameterCount != nullptr) { *pParameterCount = 0; } return E_NOTIMPL; }
    HRESULT STDMETHODCALLTYPE CreateMetaCommand(REFGUID, UINT, const void*, SIZE_T, REFIID, void**) override { return E_NOTIMPL; }
    HRESULT STDMETHODCALLTYPE CreateStateObject(const D3D12_STATE_OBJECT_DESC*, REFIID, void**) override { return E_NOTIMPL; }
    void STDMETHODCALLTYPE GetRaytracingAccelerationStructurePrebuildInfo(const D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS*, D3D12_RAYTRACING_ACCELERATION_STRUCTURE_PREBUILD_INFO* pInfo) override { if (pInfo != nullptr) { *pInfo = {}; } }
    D3D12_DRIVER_MATCHING_IDENTIFIER_STATUS STDMETHODCALLTYPE CheckDriverMatchingIdentifier(D3D12_SERIALIZED_DATA_TYPE, const D3D12_SERIALIZED_DATA_DRIVER_MATCHING_IDENTIFIER*) override { return D3D12_DRIVER_MATCHING_IDENTIFIER_UNSUPPORTED_TYPE; }
    HRESULT STDMETHODCALLTYPE SetBackgroundProcessingMode(D3D12_BACKGROUND_PROCESSING_MODE, D3D12_MEASUREMENTS_ACTION, HANDLE, BOOL* pbFurtherMeasurementsDesired) override { if (pbFurtherMeasurementsDesired != nullptr) { *pbFurtherMeasurementsDesired = FALSE; } return E_NOTIMPL; }
