...
namespace reference_blobs {
    ...
    std::vector<uint8_t> SerializeRootSignature(const D3D12_ROOT_SIGNATURE_DESC& desc)
    {
        ...
        const auto* blob_data = static_cast<const uint8_t*>(sig_blob->GetBufferPointer());
...
protected:
    void SetUp() override
    {
        ...
        EXPECT_CALL(mock_device_, CreateRootSignature(_, _, _, _, _))
            ...
            .WillRepeatedly(
                [this](
                    UINT /*node_mask*/, const void* blob_data,
    ...
    std::shared_ptr<oxygen::graphics::IShaderByteCode> dummy_bytecode_ = std::make_shared<oxygen::graphics::ShaderByteCode<std::vector<uint32_t>>>(
...
NOLINT_TEST_F(PipelineStateCacheTest, GraphicsPipeline_BindlessCbvSrvTable)
{
    ...
    EXPECT_GT(
        blob_capture_.captured_blob.size(),
...
NOLINT_TEST_F(PipelineStateCacheTest, GraphicsPipeline_MixedParameters)
{
    ...
    EXPECT_TRUE(blob_capture_.blob_captured);
    EXPECT_GT(
        blob_capture_.captured_blob.size(), 150u); // Complex root signature
}
...
NOLINT_TEST_F(PipelineStateCacheTest, CachingBehavior)
{
    ...
    const auto [ps1, rs1] = pipeline_cache_->GetOrCreatePipeline(pipeline_desc, hash1);
    ...
    const auto [ps2, rs2] = pipeline_cache_->GetOrCreatePipeline(pipeline_desc, hash2);
