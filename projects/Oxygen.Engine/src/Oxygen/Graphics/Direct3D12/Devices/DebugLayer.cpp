...
void PrintBreadcrumbHistory(const D3D12_AUTO_BREADCRUMB_NODE1* node) noexcept
{
    ...
    for (const auto& command : commandHistory) {
        LOG_F(INFO, "  [{:3}] {}", &command - commandHistory.data(), nostd::to_string(command));
    }
...
void PrintBreadcrumbContexts(const D3D12_AUTO_BREADCRUMB_NODE1* node) noexcept
{
    ...
    auto contexts = std::span(node->pBreadcrumbContexts, node->BreadcrumbContextsCount);
    for (const auto& context : contexts) {
        if (context.pContextString != nullptr) {
            ...
            try {
                ...
                DLOG_F(ERROR, "Failed to convert context string: {}", e.what());
            }
...
void PrintAllocationNode(const D3D12_DRED_ALLOCATION_NODE* node, const char* prefix) noexcept
{
    for (; node != nullptr; node = node->pNext) {
        LOG_F(INFO, "{}{} {}",
            prefix,
            nostd::to_string(node->AllocationType),
