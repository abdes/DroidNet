...
using oxygen::graphics::d3d12::detail::SynchronizedCommandQueue;
using oxygen::windows::ThrowOnFailed;

SynchronizedCommandQueue::SynchronizedCommandQueue(std::string_view queue_name, QueueRole role)
    : queue_role_(role)
{
    CreateCommandQueue(role, queue_name);
    LOG_F(INFO, "Command queue [name=`{}`, role=`{}`] created", queue_name, nostd::to_string(role));

    auto fence_name = fmt::format("Fence ({})", queue_name);
    CreateFence(fence_name, 0ULL);
    LOG_F(INFO, "Fence [name=`{}`] created", fence_name);
}

SynchronizedCommandQueue::~SynchronizedCommandQueue() noexcept
{
    ...
    ReleaseCommandQueue();
    LOG_F(INFO, "Command Queue [name=`{}`] destroyed", queue_name);
    ReleaseFence();
    LOG_F(INFO, "Fence [name=`Fence ({})`] destroyed", queue_name);
}

void SynchronizedCommandQueue::CreateCommandQueue(QueueRole role, std::string_view queue_name)
{
...
void SynchronizedCommandQueue::ReleaseCommandQueue() noexcept
{
    ObjectRelease(command_queue_);
}

void SynchronizedCommandQueue::CreateFence(std::string_view fence_name, uint64_t initial_value)
{
...
void SynchronizedCommandQueue::ReleaseFence() noexcept
{
    if (fence_event_ != nullptr) {
...
void SynchronizedCommandQueue::Signal(const uint64_t value) const
{
...
auto SynchronizedCommandQueue::Signal() const -> uint64_t
{
...
void SynchronizedCommandQueue::Wait(const uint64_t value, const std::chrono::milliseconds timeout) const
{
...
void SynchronizedCommandQueue::Wait(const uint64_t value) const
{
    Wait(value, std::chrono::milliseconds(std::numeric_limits<DWORD>::max()));
}

void SynchronizedCommandQueue::QueueWaitCommand(const uint64_t value) const
{
    DCHECK_NOTNULL_F(command_queue_, "command queue must be valid");
    DCHECK_NOTNULL_F(fence_, "fence must be initialized");
...
void SynchronizedCommandQueue::QueueSignalCommand(const uint64_t value)
{
    DCHECK_NOTNULL_F(command_queue_, "command queue must be valid");
    DCHECK_NOTNULL_F(fence_, "fence must be initialized");
    ThrowOnFailed(command_queue_->Signal(fence_, value),
...
auto SynchronizedCommandQueue::GetCompletedValue() const -> uint64_t
{
...
auto SynchronizedCommandQueue::GetQueueRole() const -> QueueRole
{
    return queue_role_;
}
