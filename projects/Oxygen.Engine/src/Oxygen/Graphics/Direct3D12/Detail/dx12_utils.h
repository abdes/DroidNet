...
inline void NameObject(ID3D12Object* const object, const std::string& name)
{
  ...
  windows::ThrowOnFailed(object->SetPrivateData(WKPDID_D3DDebugObjectName,
    static_cast<UINT>(name.size() + 1), name.data()));
}
...
inline void NameObject(ID3D12Object* const object, const std::string_view name)
{
  ...
  // Store the name including the null terminator
  windows::ThrowOnFailed(object->SetPrivateData(WKPDID_D3DDebugObjectName,
    static_cast<UINT>(name.size() + 1), full_name.data()));
}

inline auto GetObjectName(ID3D12Object* const object,
  const std::string_view fallback_name) -> std::string
{
  if (object == nullptr) {
    return "<null>";
  }
...
[[nodiscard]] constexpr uint64_t align_size_up(const uint64_t size)
{
...
/// <returns></returns>
template <uint64_t alignment>
[[nodiscard]] constexpr uint64_t align_size_down(const uint64_t size)
{
  static_assert(alignment, "Alignment must be non-zero");
  constexpr uint64_t mask { alignment - 1 };
  static_assert(!(alignment & mask), "Alignment must be a power of 2.");
...
/// that is greater than or equal to 'size'
/// </summary>
[[nodiscard]] constexpr uint64_t align_size_up(
...
/// <returns></returns>
[[nodiscard]] constexpr uint64_t align_size_down(
  const uint64_t size, const uint64_t alignment)
{
  assert(alignment && "Alignment must be non-zero");
  const uint64_t mask { alignment - 1 };
...
constexpr struct {
  D3D12_BLEND_DESC disabled { 0, // AlphaToCoverageEnable
  ...
  D3D12_BLEND_DESC alpha_blend { 0, // AlphaToCoverageEnable
    0, // IndependentBlendEnable
    { {
        1, // BlendEnable
        0, // LogicOpEnable
  ...
  D3D12_BLEND_DESC additive { 0, // AlphaToCoverageEnable
    0, // IndependentBlendEnable
    { {
        1, // BlendEnable
        0, // LogicOpEnable
  ...
  D3D12_BLEND_DESC premultiplied { 0, // AlphaToCoverageEnable
    0, // IndependentBlendEnable
    { {
        0, // BlendEnable
        0, // LogicOpEnable
...
constexpr uint64_t align_size_for_constant_buffer(const uint64_t size)
{
  return align_size_up<D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT>(size);
}

constexpr uint64_t align_size_for_texture(const uint64_t size)
{
...
struct D3d12DescriptorRange : public D3D12_DESCRIPTOR_RANGE1 {
  constexpr explicit D3d12DescriptorRange(
    const D3D12_DESCRIPTOR_RANGE_TYPE range_type,
    const uint32_t descriptor_count, const uint32_t shader_register,
    const uint32_t space = 0,
    const D3D12_DESCRIPTOR_RANGE_FLAGS flags = D3D12_DESCRIPTOR_RANGE_FLAG_NONE,
    const uint32_t offset_from_table_start
    = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND)
    : D3D12_DESCRIPTOR_RANGE1 { range_type, descriptor_count, shader_register,
      space, flags, offset_from_table_start }
  {
  }
};

struct D3d12RootParameter : public D3D12_ROOT_PARAMETER1 {
  constexpr void as_constants(const uint32_t num_constants,
    const D3D12_SHADER_VISIBILITY visibility, const uint32_t shader_register,
    const uint32_t space = 0)
  {
  ...
  constexpr void as_cbv(const D3D12_SHADER_VISIBILITY visibility,
    const uint32_t shader_register, const uint32_t space = 0,
    const D3D12_ROOT_DESCRIPTOR_FLAGS flags = D3D12_ROOT_DESCRIPTOR_FLAG_NONE)
  {
    as_descriptor(
      D3D12_ROOT_PARAMETER_TYPE_CBV, visibility, shader_register, space, flags);
  }

  constexpr void as_srv(const D3D12_SHADER_VISIBILITY visibility,
    const uint32_t shader_register, const uint32_t space = 0,
    const D3D12_ROOT_DESCRIPTOR_FLAGS flags = D3D12_ROOT_DESCRIPTOR_FLAG_NONE)
  {
    as_descriptor(
      D3D12_ROOT_PARAMETER_TYPE_SRV, visibility, shader_register, space, flags);
  }

  constexpr void as_uav(const D3D12_SHADER_VISIBILITY visibility,
    const uint32_t shader_register, const uint32_t space = 0,
    const D3D12_ROOT_DESCRIPTOR_FLAGS flags = D3D12_ROOT_DESCRIPTOR_FLAG_NONE)
  {
    as_descriptor(
      D3D12_ROOT_PARAMETER_TYPE_UAV, visibility, shader_register, space, flags);
  }

  constexpr void as_descriptor_table(const D3D12_SHADER_VISIBILITY visibility,
    const D3d12DescriptorRange* ranges, const uint32_t range_count)
  {
...
private:
  constexpr void as_descriptor(const D3D12_ROOT_PARAMETER_TYPE type,
    const D3D12_SHADER_VISIBILITY visibility, const uint32_t shader_register,
    const uint32_t space, const D3D12_ROOT_DESCRIPTOR_FLAGS flags)
  {
    ParameterType = type;
    ShaderVisibility = visibility;
...
struct D3d12RootSignatureDesc : public D3D12_ROOT_SIGNATURE_DESC1 {
  ...
  constexpr explicit D3d12RootSignatureDesc(
    const D3d12RootParameter* parameters, const uint32_t parameter_count,
    const D3D12_ROOT_SIGNATURE_FLAGS flags = default_flags,
    const D3D12_STATIC_SAMPLER_DESC* static_samplers = nullptr,
    const uint32_t sampler_count = 0)
    : D3D12_ROOT_SIGNATURE_DESC1 { parameter_count, parameters, sampler_count,
      static_samplers, flags }
  {
  }
...
public:
  ...
  constexpr explicit D3D12PipelineStateSubObject(T sub_object)
    : sub_object_ { sub_object }
  {
  }
  D3D12PipelineStateSubObject& operator=(const T& sub_object)
  {
    sub_object_ = sub_object;
    return *this;
  }
...
#define PSS(name, ...)                                                         \
  using D3D12PipelineStateSubObject_##name                                     \
    = D3D12PipelineStateSubObject<__VA_ARGS__>;

PSS(root_signature, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_ROOT_SIGNATURE,
  ID3D12RootSignature*);
...
PSS(stream_output, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_STREAM_OUTPUT,
...
PSS(rasterizer, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_RASTERIZER,
  D3D12_RASTERIZER_DESC);
PSS(depth_stencil, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_DEPTH_STENCIL,
  D3D12_DEPTH_STENCIL_DESC);
PSS(input_layer, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_INPUT_LAYOUT,
  D3D12_INPUT_LAYOUT_DESC);
PSS(ib_strip_cut_value, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_IB_STRIP_CUT_VALUE,
  D3D12_INDEX_BUFFER_STRIP_CUT_VALUE);
PSS(primitive_topology, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_PRIMITIVE_TOPOLOGY,
  D3D12_PRIMITIVE_TOPOLOGY_TYPE);
PSS(render_target_formats,
  D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_RENDER_TARGET_FORMATS,
  D3D12_RT_FORMAT_ARRAY);
PSS(depth_stencil_format,
  D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_DEPTH_STENCIL_FORMAT, DXGI_FORMAT);
PSS(sample_desc, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_SAMPLE_DESC,
  DXGI_SAMPLE_DESC);
PSS(node_mask, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_NODE_MASK, uint32_t);
PSS(cached_pso, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_CACHED_PSO,
  D3D12_CACHED_PIPELINE_STATE);
PSS(
  flags, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_FLAGS, D3D12_PIPELINE_STATE_FLAGS);
PSS(depth_stencil1, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_DEPTH_STENCIL1,
  D3D12_DEPTH_STENCIL_DESC1);
PSS(view_instancing, D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_VIEW_INSTANCING,
...
struct D3d12PipelineStateSubObjectStream {
  ...
  D3D12PipelineStateSubObject_rasterizer rasterizer {
    kRasterizerState.no_cull
  };
  ...
  D3D12PipelineStateSubObject_sample_desc sample_desc { { .Count = 1,
    .Quality = 0 } };
  ...
  D3D12PipelineStateSubObject_depth_stencil depth_stencil1 {
    kDepthState.disabled
  };
