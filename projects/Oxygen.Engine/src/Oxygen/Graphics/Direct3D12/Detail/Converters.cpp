...
auto ConvertResourceStates(ResourceStates states) -> D3D12_RESOURCE_STATES
{
  if (states == ResourceStates::kUnknown) {
    // kUnknown (0) implies no specific state, which is
  ...
  D3D12_RESOURCE_STATES d3d_states
    = D3D12_RESOURCE_STATE_COMMON; // Initialize to 0

  // Helper lambda to check a flag and append D3D12 state(s)
  // Captures 'states' by value and 'd3d_states' by reference.
  auto append_if_set = [&d3d_states, states](const ResourceStates flag_to_check,
                         const D3D12_RESOURCE_STATES d3d12_flags_to_add) {
  ...
  auto is_set
    = [states](const ResourceStates flag) { return (states & flag) == flag; };

  // kUndefined (OXYGEN_FLAG(0)) is not explicitly mapped to a unique D3D12
  // state bit here. If it's the only state (other than kUnknown/kCommon handled
  // above), the result will be D3D12_RESOURCE_STATE_COMMON, which is
  // appropriate.

  append_if_set(ResourceStates::kVertexBuffer,
    D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
  append_if_set(ResourceStates::kConstantBuffer,
    D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
  append_if_set(
  ...
  append_if_set(ResourceStates::kBuildAccelStructureRead,
  ...
  if (is_set(ResourceStates::kShaderResource)
    || is_set(ResourceStates::kInputAttachment)) {
  ...
  if (is_set(ResourceStates::kBuildAccelStructureWrite)
    || is_set(ResourceStates::kRayTracing)) {
    d3d_states |= D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE;
  }

  // If states contained kCommon along with other flags,
  // D3D12_RESOURCE_STATE_COMMON (0) being ORed in doesn't change the specific
  // states. If d3d_states is still 0 (D3D12_RESOURCE_STATE_COMMON) after all
  // checks, it means no specific mappable states were found (e.g., only
  // kUndefined was set), so D3D12_RESOURCE_STATE_COMMON is the correct default.
  // return d3d_states;
...
auto ConvertCullMode(const CullMode value) -> D3D12_CULL_MODE
{
  ...
  case CullMode::kFrontAndBack:
    throw std::runtime_error(
      "D3D12 doesn't support front and back face culling");
  }
...
auto ConvertPrimitiveType(const PrimitiveType value)
  ...
  const DepthStencilStateDesc& desc, D3D12_DEPTH_STENCIL_DESC& d3d_desc)
{
  ...
  d3d_desc.DepthWriteMask = desc.depth_write_enable
...
void TranslateBlendState(
  const std::vector<BlendTargetDesc>& blend_targets, D3D12_BLEND_DESC& d3d_desc)
{
