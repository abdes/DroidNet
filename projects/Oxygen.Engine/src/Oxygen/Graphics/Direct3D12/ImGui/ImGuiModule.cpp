...
void ImGuiModule::ImGuiBackendInit(const oxygen::Graphics* gfx)
{
  ...
  ImGui_ImplDX12_Init(GetCurrentDevice(), kFarmesInFlight,
  kDefaultBackBufferFormat, d3d12_render->SrvHeap().Heap(),
  font_srv_handle_.cpu, font_srv_handle_.gpu);
  */
}
...
void ImGuiModule::ImGuiBackendShutdown()
{
  ...
  /* TODO: Rewrite ImGui integration
  // TODO: this is hacky and will be obsolete once DescriptorHandle is RAII
  friendly GetRenderer().SrvHeap().Free(font_srv_handle_);
  */
}

void ImGuiModule::ImGuiBackendNewFrame() { ImGui_ImplDX12_NewFrame(); }

auto ImGuiModule::ImGuiBackendRenderRawData(
  const oxygen::Graphics* gfx, ImDrawData* draw_data) -> CommandListPtr
{
  /* TODO: Rewrite ImGui integration
  DCHECK_NOTNULL_F(gfx);
  DCHECK_F(!gfx->IsWithoutRenderer());
  ...
  try {
      command_list = std::make_unique<CommandList>(QueueRole::kGraphics, "ImGui
  Command List"); CHECK_NOTNULL_F(command_list);
  ...
  const D3D12_CPU_DESCRIPTOR_HANDLE render_target_views[1] = {
  current_render_target.Rtv().cpu };
  command_list->GetCommandList()->OMSetRenderTargets(1, render_target_views,
  FALSE, nullptr); ID3D12DescriptorHeap* heaps[] = {
