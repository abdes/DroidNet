...
//  2025-06-11: DirectX12: Added support for
...
#  ifdef _MSC_VER
#    pragma comment(                                                           \
      lib, "d3dcompiler") // Automatically link with d3dcompiler.lib as we are
                          // using D3DCompile() below.
#  endif
...
static void ImGui_ImplDX12_SetupRenderState(ImDrawData* draw_data,
  ID3D12GraphicsCommandList* command_list, ImGui_ImplDX12_RenderBuffers* fr)
{
  ...
  ImDrawData* draw_data, ID3D12GraphicsCommandList* command_list)
{
  ...
  if (fr->VertexBuffer == nullptr
    ...
    if (bd->pd3dDevice->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE,
  ...
  for (const ImDrawList* draw_list : draw_data->CmdLists) {
    for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        command_list->DrawIndexedInstanced(pcmd->ElemCount, 1,
...
static void ImGui_ImplDX12_DestroyTexture(ImTextureData* tex)
{
  ...
  bd->InitInfo.SrvDescriptorFreeFn(&bd->InitInfo,
    backend_tex->hFontSrvCpuDescHandle, backend_tex->hFontSrvGpuDescHandle);
...
void ImGui_ImplDX12_UpdateTexture(ImTextureData* tex)
{
  ...
  if (tex->Status == ImTextureStatus_WantCreate) {
    ...
    IM_ASSERT(
      tex->TexID == ImTextureID_Invalid && tex->BackendUserData == nullptr);
  ...
  if (tex->Status == ImTextureStatus_WantCreate
    ...
    HRESULT hr = bd->pd3dDevice->CreateCommittedResource(&props,
    ...
    // FIXME-OPT: Can be use the one from user? (pass ID3D12GraphicsCommandList*
    // to ImGui_ImplDX12_UpdateTextures)
    ...
    for (int y = 0; y < upload_h; y++)
      memcpy((void*)((uintptr_t)mapped + y * upload_pitch_dst),
    ...
    cmdList->CopyTextureRegion(
      &dstLocation, upload_x, upload_y, 0, &srcLocation, nullptr);
...
bool ImGui_ImplDX12_CreateDeviceObjects()
{
  ...
  // Create the root signature
  {
    ...
    if (d3d12_dll == nullptr) {
      ...
      }; // A. current directory, B. used by some games, C. used in Microsoft
         // D3D12On7 sample
    ...
    bd->pd3dDevice->CreateRootSignature(0, blob->GetBufferPointer(),
      blob->GetBufferSize(), IID_PPV_ARGS(&bd->pRootSignature));
    blob->Release();
  }
  ...
  // Create the vertex shader
  {
    ...
    if (FAILED(D3DCompile(vertexShader, strlen(vertexShader), nullptr, nullptr,
          nullptr, "main", "vs_5_0", 0, 0, &vertexShaderBlob, nullptr)))
  ...
  // Create the pixel shader
  {
    ...
    if (FAILED(D3DCompile(pixelShader, strlen(pixelShader), nullptr, nullptr,
          nullptr, "main", "ps_5_0", 0, 0, &pixelShaderBlob, nullptr))) {
