  ...
  const GraphicsPipelineDesc& desc) noexcept -> size_t
{
  ...
  HashCombine(
    seed, static_cast<int>(desc.DepthStencilState().front_face.stencil_func));
  HashCombine(
    seed, static_cast<int>(desc.DepthStencilState().back_face.stencil_fail_op));
  HashCombine(seed,
    static_cast<int>(desc.DepthStencilState().back_face.stencil_depth_fail_op));
  HashCombine(
    seed, static_cast<int>(desc.DepthStencilState().back_face.stencil_pass_op));
  HashCombine(
    seed, static_cast<int>(desc.DepthStencilState().back_face.stencil_func));
  // BlendState
