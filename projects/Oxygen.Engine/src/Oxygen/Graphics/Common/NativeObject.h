...
public:
  ...
  */
  ...
  {
    return integer != kInvalidHandle;
  }

  //! Checks if the `NativeObject` was created with an integer handle.
  [[nodiscard]] constexpr auto IsIntegerHandle() const noexcept
  {
    return !is_pointer_;
  }

  //! Checks if the `NativeObject` was created with a pointer.
  [[nodiscard]] constexpr auto IsPointerHandle() const noexcept
  {
    return is_pointer_;
  }
  ...
  */
  ...
  {
    if (!is_pointer_) {
      throw std::runtime_error("Cannot convert a NativeObject created with "
                               "integer handle to a pointer");
    }
  ...
  */
  template <typename T> [[nodiscard]] auto AsPointer() -> T*
  {
    if (!is_pointer_) {
      throw std::runtime_error("Cannot convert a NativeObject created with "
                               "integer handle to a pointer");
  ...
  [[nodiscard]] constexpr auto OwnerTypeId() const noexcept
  {
    return owner_type_id_;
  }
  ...
  {
    return (owner_type_id_ == other.owner_type_id_)
 ...
 */
template <> struct std::hash<oxygen::graphics::NativeObject> {
  auto operator()(const oxygen::graphics::NativeObject& obj) const noexcept
    -> size_t
  {
    size_t seed = 0;
