...
enum class ResourceStates : uint32_t {
  //! The resource state is unknown to the engine and is managed by the
  //! application (None).
  kUnknown = 0,
  ...
  //! The resource is accessed as a constant (uniform) buffer (Graphics,
  //! Compute).
  ...
  //! The resource is used as an indirect draw/dispatch argument buffer
  //! (Graphics, Compute).
  kIndirectArgument = OXYGEN_FLAG(10),

  //! The resource is used as the destination in a copy operation (Graphics,
  //! Compute, Transfer).
  kCopyDest = OXYGEN_FLAG(11),

  //! The resource is used as the source in a copy operation (Graphics, Compute,
  //! Transfer).
  ...
  //! The resource is used as vertex/index/instance data in AS builds or as
  //! source in AS copy operations (Graphics, Compute).
  ...
  //! The resource is used as an acceleration structure shader resource in a ray
  //! tracing operation (Graphics, Compute).
  kRayTracing = OXYGEN_FLAG(19),

  //! The resource is readable, but transitioning to this state may cause a
  //! pipeline stall or cache flush (Graphics, Compute, Transfer).
  kCommon = OXYGEN_FLAG(20),

  //! The resource is used as a shading rate image (Graphics).
  kShadingRate = OXYGEN_FLAG(21),

  //! A generic read state for multiple resource usages combined (Graphics,
  //! Compute).
  //! Avoid using this state unless necessary, as it is not optimal.
  kGenericRead = kVertexBuffer | kConstantBuffer | kIndexBuffer
    | kShaderResource | kIndirectArgument | kCopySource
};
...
OXGN_GFX_API auto to_string(ResourceStates value) -> std::string;
...
//! String representation of enum values in `QueueFamilyType`.
OXGN_GFX_API auto to_string(ResourceStateTrackingMode value) -> const char*;
