...
enum class ResourceAccessMode : uint8_t {
  ...
  */
  ...
  /*!
    Content cannot be accessed by the CPU. Can be written to many times per
    frame.
  */
  kGpuOnly,
  ...
  /*!
    ...
    \warning Memory accesses must be properly synchronized as it's not
    double-buffered.
  */
  kUpload,
  ...
  */
  ...
  /*!
    The content can't be accessed directly by the GPU (only via Copy
    operations). The data can be read by the CPU.
    \warning Memory accesses must be properly synchronized as it's not
    double-buffered.
  */
  kReadBack
...
OXGN_GFX_API auto to_string(ResourceAccessMode value) -> const char*;
