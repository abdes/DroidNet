...
auto TextureSlice::Resolve(const TextureDesc& desc) const -> TextureSlice
{
    ...
    DCHECK_LT_F(mip_level, desc.mip_levels,
        "Invalid mip level: {} >= {}", mip_level, desc.mip_levels);
...
auto TextureSubResourceSet::Resolve(
    const TextureDesc& desc, const bool single_mip_level) const -> TextureSubResourceSet
{
    ...
    if (single_mip_level) {
        ...
        const auto last_mip_level_plus_one = std::min(base_mip_level + num_mip_levels, desc.mip_levels);
        ret.num_mip_levels = static_cast<MipLevel>(std::max(0u, last_mip_level_plus_one - base_mip_level));
    }
...
auto TextureSubResourceSet::IsEntireTexture(const TextureDesc& desc) const -> bool
{
    if (base_mip_level > 0u || base_mip_level + num_mip_levels < desc.mip_levels) {
        return false;
    }
