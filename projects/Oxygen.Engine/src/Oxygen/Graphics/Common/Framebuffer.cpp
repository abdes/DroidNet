...
auto FramebufferAttachment::ResolveClearColor(const std::optional<Color>& explicit_clear) const -> Color
{
    ...
    -> std::pair<float, uint8_t>
{
    ...
    } else if (texture && texture->GetDescriptor().use_clear_value && format_info.has_depth) {
        depth = texture->GetDescriptor().clear_value.r;
    }
    if (explicit_stencil.has_value()) {
        stencil = explicit_stencil.value();
    } else if (texture && texture->GetDescriptor().use_clear_value && format_info.has_stencil) {
