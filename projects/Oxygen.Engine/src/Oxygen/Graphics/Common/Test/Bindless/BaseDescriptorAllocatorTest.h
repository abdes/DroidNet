...
class NoGrowthDescriptorAllocationStrategy final
  : public DescriptorAllocationStrategy {
public:
  auto GetHeapKey(const ResourceViewType view_type,
    const DescriptorVisibility visibility) const -> std::string override
  {
...
class ZeroCapacityDescriptorAllocationStrategy final
  : public DescriptorAllocationStrategy {
public:
  auto GetHeapKey(const ResourceViewType view_type,
    const DescriptorVisibility visibility) const -> std::string override
    ...
    -> const HeapDescription& override
  {
    ...
    desc.cpu_visible_capacity = oxygen::bindless::Capacity { 0 };
    desc.shader_visible_capacity = oxygen::bindless::Capacity { 0 };
    // Update the heap description in the cache and return it
    return heap_descriptions_[heap_key] = desc;
  }
...
class OneCapacityDescriptorAllocationStrategy final
  : public DescriptorAllocationStrategy {
public:
  auto GetHeapKey(const ResourceViewType view_type,
    const DescriptorVisibility visibility) const -> std::string override
    ...
    -> const HeapDescription& override
  {
    ...
    desc.cpu_visible_capacity = oxygen::bindless::Capacity { 1 };
    desc.shader_visible_capacity = oxygen::bindless::Capacity { 1 };
    // Update the heap description in the cache and return it
    return heap_descriptions_[heap_key] = desc;
  }
