...
using oxygen::graphics::bindless::testing::
  ZeroCapacityDescriptorAllocationStrategy;
...
{
  // Tests that the allocator uses a default allocation strategy when none is
  // provided
  ...
  const auto n_allocator
    = std::make_unique<MockDescriptorAllocator>(heap_strategy);

  // This should NOT cause the process to die
  EXPECT_NO_FATAL_FAILURE({ (void)n_allocator->GetAllocationStrategy(); });
}

NOLINT_TEST_F(
  BaseDescriptorAllocatorInitTest, ZeroInitialCapacityFailsAllocation)
{
  ...
  z_allocator->segment_factory_ = [](auto, auto) {
    ...
    return std::unique_ptr<MockDescriptorSegment> {};
  };
