...
public:
  TestDescriptorHeapSegment(
    const DescriptorVisibility visibility, const uint32_t base_index) noexcept
...
using AllResourceViewTypes = ::testing::Types<
  ...
  std::integral_constant<ResourceViewType,
...
template <typename... Ts> struct TypeCount;
template <typename... Ts> struct TypeCount<::testing::Types<Ts...>> {
  static constexpr std::size_t value = sizeof...(Ts);
};
static_assert(TypeCount<AllResourceViewTypes>::value
    == static_cast<std::size_t>(ResourceViewType::kMaxResourceViewType) - 1,
...
struct ResourceViewTypeNameGenerator {
  template <typename T> static auto GetName(int) -> std::string
  {
...
TYPED_TEST_SUITE(StaticDescriptorHeapSegmentTest, AllResourceViewTypes,
...
TYPED_TEST(StaticDescriptorHeapSegmentTest, ConstructionAndProperties)
{
  ...
  }
  {
    ...
    TestDescriptorHeapSegment<type> seg(
      DescriptorVisibility::kShaderVisible, base);
    EXPECT_EQ(seg.GetViewType(), type);
...
TYPED_TEST(StaticDescriptorHeapSegmentTest, DestructionWhenNotEmpty)
{
  ...
  {
    constexpr ResourceViewType type = TypeParam::value;
    StaticDescriptorHeapSegment<type> seg(
      DescriptorVisibility::kShaderVisible, 0);
