...
// NOLINTNEXTLINE(*)
MATCHER_P(
  HasSameDescription, expected, "RootBindingItem has same description fields")
{
  return arg.binding_slot_desc == expected.binding_slot_desc
    && arg.visibility == expected.visibility && arg.data == expected.data;
}
  ...
  const auto pipeline
    = GraphicsPipelineDesc::Builder {}
        ...
        .SetFramebufferLayout({ .color_target_formats
          = { Format::kRGBA8UNorm, Format::kRGBA8UNorm } })
        .Build();
...
{
  NOLINT_EXPECT_THROW(
    [[maybe_unused]] auto pipeline = ComputePipelineDesc::Builder {}.Build(),
    std::runtime_error);
}
...
NOLINT_TEST(GraphicsPipelineDescTest, ValidDepthOnlyConfiguration)
{
  ...
  EXPECT_EQ(
    *pipeline.FramebufferLayout().depth_stencil_format, Format::kDepth32);
...
NOLINT_TEST(GraphicsPipelineDescTest, AddRootBinding)
{
  ...
  RootBindingItem table(
    { .binding_slot_desc = { .register_index = 3, .register_space = 0 },
      .visibility = ShaderStageFlags::kAll,
      .data
      = DescriptorTableBinding { .view_type = ResourceViewType::kTexture_SRV,
        .base_index = 5,
        .count = 8 } });
...
NOLINT_TEST(RootBindingItemTest, Equality)
{
  ...
  RootBindingItem a(
    { .binding_slot_desc = { .register_index = 1, .register_space = 2 },
      .visibility = ShaderStageFlags::kAll,
      .data
      = DescriptorTableBinding { .view_type = ResourceViewType::kTexture_SRV,
        .base_index = 0,
...
NOLINT_TEST(GraphicsPipelineDescTest, Hashing)
{
  ...
  // Test unordered_map
  std::unordered_map<GraphicsPipelineDesc, int, std::hash<GraphicsPipelineDesc>>
    map;
  map[pipeline1] = 42;
...
NOLINT_TEST(ComputePipelineDescTest, Hashing)
{
  ...
  // Test unordered_map
  std::unordered_map<ComputePipelineDesc, int, std::hash<ComputePipelineDesc>>
    map;
  map[pipeline1] = 99;
...
NOLINT_TEST(GraphicsPipelineDescTest, Hashing_AllFieldsAffectHash)
{
  struct Baseline {
    ...
    DepthStencilStateDesc depth_stencil { .depth_test_enable = true,
      .depth_write_enable = true,
      .depth_func = CompareOp::kLess };
    BlendTargetDesc blend_target { .blend_enable = true,
      .src_blend = BlendFactor::kSrcAlpha,
      .dest_blend = BlendFactor::kInvSrcAlpha,
      .blend_op = BlendOp::kAdd };
    FramebufferLayoutDesc framebuffer { .color_target_formats
      = { Format::kRGBA8UNorm },
      .depth_stencil_format = Format::kDepth32,
      .sample_count = 4 };
  ...
  std::vector<Delta> deltas = {
    ...
    { "RasterizerState.fill_mode not included in hash",
      [](Baseline& b) { b.rasterizer.fill_mode = FillMode::kSolid; } },
    { "RasterizerState.cull_mode not included in hash",
      [](Baseline& b) { b.rasterizer.cull_mode = CullMode::kFront; } },
    { "RasterizerState.multisample_enable not included in hash",
      [](Baseline& b) { b.rasterizer.multisample_enable = false; } },
    { "DepthStencilState.depth_test_enable not included in hash",
      [](Baseline& b) { b.depth_stencil.depth_test_enable = false; } },
    { "DepthStencilState.depth_write_enable not included in hash",
      [](Baseline& b) { b.depth_stencil.depth_write_enable = false; } },
    { "DepthStencilState.depth_func not included in hash",
      [](Baseline& b) { b.depth_stencil.depth_func = CompareOp::kGreater; } },
    { "BlendState.blend_enable not included in hash",
      [](Baseline& b) { b.blend_target.blend_enable = false; } },
    { "BlendState.src_blend not included in hash",
      [](Baseline& b) { b.blend_target.src_blend = BlendFactor::kOne; } },
    { "BlendState.dest_blend not included in hash",
      [](Baseline& b) { b.blend_target.dest_blend = BlendFactor::kZero; } },
    { "BlendState.blend_op not included in hash",
      [](Baseline& b) { b.blend_target.blend_op = BlendOp::kSubtract; } },
    { "FramebufferLayout.color_target_formats not included in hash",
      [](Baseline& b) {
        b.framebuffer.color_target_formats = { Format::kBGRA8UNorm };
      } },
    { "FramebufferLayout.depth_stencil_format not included in hash",
      [](Baseline& b) {
        b.framebuffer.depth_stencil_format = Format::kDepth16;
      } },
    { "FramebufferLayout.sample_count not included in hash",
      [](Baseline& b) { b.framebuffer.sample_count = 8; } },
    { "FramebufferLayout.sample_quality not included in hash",
      [](Baseline& b) { b.framebuffer.sample_quality = 8; } },
    { "RootBindings not included in hash",
      [](Baseline& b) {
        b.root_bindings = { RootBindingItem({ .binding_slot_desc = { 0, 0 },
          .visibility = ShaderStageFlags::kAll,
          .data = PushConstantsBinding { 32 } }) };
      } },
    { "RootBindings not included in hash",
      [](Baseline& b) {
        b.root_bindings = { RootBindingItem({ .binding_slot_desc = { 0, 0 },
