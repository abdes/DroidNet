...
public:
  ...
  auto GetUsage() const noexcept -> BufferUsage override
  {
    return BufferUsage::kNone;
  }
  auto GetMemoryType() const noexcept -> BufferMemory override
  {
    return BufferMemory::kDeviceLocal;
  }
...
NOLINT_TEST_F(
  BufferStateTrackingTest, TransitionToDifferentState_CreatesBarrier)
{
  tracker.BeginTrackingResourceState(buffer1, ResourceStates::kCommon);
...
NOLINT_TEST_F(BufferStateTrackingTest,
  TransitionFromUAVToNonUAVState_CreatesTransitionBarrier)
{
...
NOLINT_TEST_F(BufferStateTrackingTest, RedundantTransitions_MergeBarriers)
{
  ...
  // Pre-condition 2: First transition creates a buffer barrier (not a memory
  // barrier)
  tracker.RequireResourceState(buffer1, ResourceStates::kUnorderedAccess);

  // Pre-condition 3: Second transition is for the same buffer, and no memory
  // barrier in between
  tracker.RequireResourceState(buffer1, ResourceStates::kCopyDest);

  // Only one barrier should exist, and it should be a buffer barrier with
  // merged states
  const auto& barriers = tracker.GetPendingBarriers();
  ASSERT_EQ(barriers.size(), 1);
  ASSERT_TRUE(
  ...
  // Now require a transition to a different state (should create a new buffer
  // barrier, not merge)
  tracker.RequireResourceState(buffer1, ResourceStates::kCopyDest);
  ...
  EXPECT_TRUE(
    std::holds_alternative<BufferBarrierDesc>(barriers2[1].GetDescriptor()));
...
NOLINT_TEST_F(
  BufferStateTrackingTest, AutoMemoryBarriers_FirstUAVAccess_CreatesBarrier)
{
  tracker.BeginTrackingResourceState(buffer1, ResourceStates::kUnorderedAccess);
...
NOLINT_TEST_F(BufferStateTrackingTest,
  AutoMemoryBarriers_SubsequentUAVAccess_CreatesBarrier)
{
  tracker.BeginTrackingResourceState(buffer1, ResourceStates::kUnorderedAccess);
  tracker.EnableAutoMemoryBarriers(buffer1);
  tracker.RequireResourceState(buffer1, ResourceStates::kUnorderedAccess);
...
NOLINT_TEST_F(
  BufferStateTrackingTest, ManualMemoryBarriers_FirstUAVAccess_CreatesBarrier)
{
  tracker.BeginTrackingResourceState(buffer1, ResourceStates::kUnorderedAccess);
...
NOLINT_TEST_F(
  BufferStateTrackingTest, ManualMemoryBarriers_SubsequentUAVAccess_NoBarrier)
{
  tracker.BeginTrackingResourceState(buffer1, ResourceStates::kUnorderedAccess);
  tracker.DisableAutoMemoryBarriers(buffer1);
  tracker.RequireResourceState(buffer1, ResourceStates::kUnorderedAccess);
...
NOLINT_TEST_F(BufferStateTrackingTest,
  ManualMemoryBarrier_ToggleAutoMode_AllowsBarrierAgain)
{
  tracker.BeginTrackingResourceState(buffer1, ResourceStates::kUnorderedAccess);
  tracker.DisableAutoMemoryBarriers(buffer1);
  tracker.RequireResourceState(buffer1, ResourceStates::kUnorderedAccess);
...
NOLINT_TEST_F(BufferStateTrackingTest,
  RequireResourceState_WithIsPermanentTrue_DoesNotThrow)
{
  tracker.BeginTrackingResourceState(buffer1, ResourceStates::kCommon);
  NOLINT_EXPECT_NO_THROW(tracker.RequireResourceStateFinal(
    buffer1, ResourceStates::kUnorderedAccess));
  const auto& barriers = tracker.GetPendingBarriers();
...
NOLINT_TEST_F(
  BufferStateTrackingTest, NoRestoreInitialState_IfKeepInitialStateFalse)
{
