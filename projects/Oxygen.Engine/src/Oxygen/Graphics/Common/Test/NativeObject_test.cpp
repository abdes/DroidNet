...
TEST_F(NativeObjectTest, AsPointerThrowsIfNotPointer)
{
  const NativeObject obj(kTestInteger, kTestTypeId);
  EXPECT_THROW(
    [[maybe_unused]] auto _ = obj.AsPointer<NativeType>(), std::runtime_error);
}
...
TEST_F(NativeObjectTest, EqualityOperator)
{
  ...
  const NativeObject different_pointer_obj(
    reinterpret_cast<void*>(0x1ULL), kTestTypeId);

  EXPECT_TRUE(pointer_obj1 == pointer_obj2);
  EXPECT_FALSE(pointer_obj1 == different_pointer_obj);

  // Test inequality between integer-based and pointer-based NativeObject
  // instances
...
TEST_F(NativeObjectTest, HashFunction)
{
  ...
  // Verify that the hash values are different for objects with different
  // integers
