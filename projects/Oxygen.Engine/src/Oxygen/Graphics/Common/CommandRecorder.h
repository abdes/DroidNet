...
public:
  ...
  */
  ...
  virtual void Dispatch(uint32_t thread_group_count_x,
    ...
    = 0;
  virtual void SetVertexBuffers(uint32_t num,
   ...
   */
  virtual void ClearDepthStencilView(const graphics::Texture& texture,
  ...
  */
  virtual void ClearFramebuffer(const Framebuffer& framebuffer,
  ...
  virtual void CopyBuffer(Buffer& dst, size_t dst_offset, const Buffer& src,
  ...
  */
  ...
  template <Trackable T>
  void RequireResourceState(const T& resource, const ResourceStates state)
  {
    if constexpr (IsBuffer<T>) {
  ...
  template <Trackable T>
  void RequireResourceStateFinal(const T& resource, const ResourceStates state)
  {
    if constexpr (IsBuffer<T>) {
...
private:
  ...
  OXYGEN_GFX_API void DoBeginTrackingResourceState(const Buffer& resource,
    ResourceStates initial_state, bool keep_initial_state);
  OXYGEN_GFX_API void DoBeginTrackingResourceState(const Texture& resource,
    ResourceStates initial_state, bool keep_initial_state);
  ...
  OXYGEN_GFX_API void DoRequireResourceState(
    const Buffer& resource, ResourceStates state);
  OXYGEN_GFX_API void DoRequireResourceState(
    const Texture& resource, ResourceStates state);

  OXYGEN_GFX_API void DoRequireResourceStateFinal(
    const Buffer& resource, ResourceStates state);
  OXYGEN_GFX_API void DoRequireResourceStateFinal(
    const Texture& resource, ResourceStates state);
