...
/*!
  ...
  binding items for all of them. One of the view types is sufficient to represent
  the entire table.
*/
...
struct PushConstantsBinding {
    uint32_t size = 0; //! The number of 32-bit integers in the push constant range.
    auto operator==(const PushConstantsBinding&) const -> bool = default;
};

//! Direct buffer binding: one descriptor handle only (no view description needed)
...
//! Direct texture binding: one descriptor handle only (no view description needed)
...
struct RootBindingDesc {
    ...
    std::variant<PushConstantsBinding, DirectBufferBinding, DirectTextureBinding, DescriptorTableBinding> data;
};
...
struct RootBindingItem {
    ...
    [[nodiscard]] auto GetRootParameterIndex() const noexcept -> uint32_t { return root_parameter_index_; }
    void SetRootParameterIndex(const uint32_t idx)
    {
        if (root_parameter_index_ != ~0u)
            throw std::logic_error("RootBindingItem: root_parameter_index already set");
...
namespace detail {
    ...
    public:
        ...
        OXYGEN_GFX_API void SetRootBindings(
...
public:
    ...
    [[nodiscard]] constexpr auto RootBindings() const noexcept -> std::span<const RootBindingItem>
    {
...
public:
    ...
    //! Set root bindings for the pipeline. Mutually exclusive with AddRootBinding.
    //! You must call either SetRootBindings or AddRootBinding, not both.
    auto SetRootBindings(std::span<const RootBindingItem> bindings) && -> Builder&&
    {
...
public:
    ...
    [[nodiscard]] constexpr auto RootBindings() const noexcept -> std::span<const RootBindingItem>
    {
        return std::span<const RootBindingItem>(root_bindings_.data(), root_bindings_.size());
    }
...
public:
    ...
    //! Set root bindings for the compute pipeline. Mutually exclusive with AddRootBinding.
    //! You must call either SetRootBindings or AddRootBinding, not both.
    auto SetRootBindings(std::span<const RootBindingItem> bindings) && -> Builder&&
    {
    ...
    */
    [[nodiscard]] auto Build() && -> ComputePipelineDesc
    {
        ...
        ComputePipelineDesc desc { std::move(compute_shader_), std::move(debug_name_) };
...
OXYGEN_GFX_API auto HashGraphicsPipelineDesc(const GraphicsPipelineDesc& desc) noexcept -> size_t;
OXYGEN_GFX_API auto HashComputePipelineDesc(const ComputePipelineDesc& desc) noexcept -> size_t;

} // namespace oxygen::graphics

template <>
struct std::hash<oxygen::graphics::GraphicsPipelineDesc> {
...
template <>
struct std::hash<oxygen::graphics::ComputePipelineDesc> {
