...
public:
  ...
  */
  ...
  /*!
   ...
   - Preparing the optional `framebuffer` if it's provided in `Config` and is
     ...
     transitions).
  */
  OXYGEN_GFX_API auto PrepareResources(CommandRecorder& recorder)
    -> co::Co<> override;
  ...
  */
  ...
  OXYGEN_GFX_API void SetViewport(const oxygen::ViewPort& viewport) override;
  auto GetViewport() const -> std::optional<oxygen::ViewPort>
  {
    return viewport_;
  }
...
protected:
  DepthPrePass(RenderController* renderer, std::shared_ptr<Config> config,
  ...
  */
  ...
  virtual auto PrepareDepthStencilView(const Texture& depth_texture_ref)
    -> NativeObject;
...
private:
  ...
  std::optional<oxygen::ViewPort> viewport_ {};
