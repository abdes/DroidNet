...
public:
  ...
  auto WorkAvailable() { return work_available_.Park(); }

private:
  template <typename T, typename> friend struct oxygen::co::EventLoopTraits;
...
template <> struct oxygen::co::EventLoopTraits<RenderTaskDispatcher> {
  static void Run(RenderTaskDispatcher& rtd) { rtd.EventLoop(); }
  static void Stop(RenderTaskDispatcher& rtd) { rtd.Stop(); }
  static auto IsRunning(const RenderTaskDispatcher& rtd) -> bool
  {
    return rtd.IsRunning();
  }
  static auto EventLoopId(RenderTaskDispatcher& rtd) -> EventLoopID
  {
    return EventLoopID(&rtd);
  }
};
    ...
    while (true) {
      ...
      if (begin_frame_fn) {
        try {
          ...
          LOG_F(ERROR,
            "Exception caught in BeginFrame(), and the frame will be dropped: "
            "{}",
            ex.what());
          continue;
        }
      ...
      // management are the responsibility of the application.
      {
        ...
        try {
          ...
          LOG_F(ERROR,
            "Exception caught in Render Frame Task, frame may be garbage: {}",
            ex.what());
        }
      }
      ...
      if (end_frame_fn) {
        try {
          ...
          LOG_F(ERROR,
            "Exception caught in EndFrame(), and the rendering task dispatcher "
            "will be stopped: {}",
            ex.what());
          stop.Trigger();
        }
  ...
  const std::function<Component&(TypeId)>& get_component) noexcept
{
  ...
  impl_->debug_name = meta_data.GetName();
}
