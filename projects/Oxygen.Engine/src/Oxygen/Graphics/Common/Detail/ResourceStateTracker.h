...
public:
  ...
  auto DisableAutoMemoryBarriers(const T& resource) -> void
  {
    auto& ti = GetTrackingInfo(resource.GetNativeResource());
    std::visit(
      [&](auto& info) { info.enable_auto_memory_barriers = false; }, ti);
  }
...
private:
  struct BasicTrackingInfo {
    ...
    [[nodiscard]] auto NeedsTransition(
      const ResourceStates required_state) const
    {
      return current_state != required_state;
    }

    [[nodiscard]] auto NeedsMemoryBarrier(
      const ResourceStates required_state) const
    {
      // Requested state includes UnorderedAccess, AND
      return ((required_state & ResourceStates::kUnorderedAccess)
  ...
  /*!
   For resources which state has been marked as permanent, this method enforces
   that their state cannot be changed. If a state transition is attempted for a
   permanent resource to a state different from its current state, an error is
   logged and an exception is thrown.

   \return `true` if the resource state is permanent and the requested state is
   not different (indicating that no further processing is needed), `false` if
   the resource state is not permanent (allowing state transition)
   \throws std::runtime_error If attempting to change a permanent resource's
   state.
  */
  static auto HandlePermanentState(const BasicTrackingInfo& tracking,
    ResourceStates required_state, const char* resource_type_name) -> bool;
