...
FixedDescriptorHeapSegment::FixedDescriptorHeapSegment(const IndexT capacity,
  const IndexT base_index, const ResourceViewType view_type,
...
auto FixedDescriptorHeapSegment::IsAllocated(
  const uint32_t local_index) const noexcept -> bool
{
...
auto FixedDescriptorHeapSegment::Allocate() noexcept -> uint32_t
{
  ...
  if (!free_list_.empty()) {
    ...
    DLOG_F(2, "allocated new local index {} (remaining: {}/{})", local_index,
  ...
  LOG_F(2, "{}returning global index {}",
    global_index == DescriptorHandle::kInvalidIndex ? "failed: " : "",
    global_index);
...
auto FixedDescriptorHeapSegment::Release(const IndexT index) noexcept -> bool
{
  ...
  try {
    ...
    LOG_F(ERROR, "Failed to add released local index {} to free list: {}",
      local_index, ex.what());
    return false;
  }
  // Mark as released
