...
template <typename T>
concept AnyResource
  = std::is_base_of_v<RegisteredResource, std::remove_cvref_t<T>>;

template <typename T>
concept ViewDescription = std::equality_comparable<T> && requires(T vk) {
...
template <typename T>
concept SupportedResource = (TextureResource<T> || BufferResource<T>
                              || SamplerResource<T> || AnyResource<T>)
  && requires {
       { T::ClassTypeId() } -> std::convertible_to<TypeId>;
     };

template <typename T>
concept ResourceWithViews = SupportedResource<T> && requires {
  typename T::ViewDescriptionT;
