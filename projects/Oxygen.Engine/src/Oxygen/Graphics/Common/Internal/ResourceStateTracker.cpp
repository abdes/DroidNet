  ...
  const ResourceStates before, const ResourceStates after) -> BufferBarrierDesc
{
  return BufferBarrierDesc {
    .resource = native_object, .before = before, .after = after
  };
...
auto ResourceStateTracker::HandlePermanentState(
  const BasicTrackingInfo& tracking, ResourceStates required_state,
  ...
  if (tracking.is_permanent) {
    if (tracking.current_state != required_state) {
      LOG_F(ERROR,
        ...
        resource_type_name, nostd::to_string(tracking.current_state),
  ...
  const Texture& texture, ResourceStates required_state, bool is_permanent)
{
  ...
  if (need_transition) {
    ...
    // but a memory barrier is needed for synchronization between UAV
    // operations.
    pending_barriers_.emplace_back(MemoryBarrierDesc { native_object });
    tracking.first_memory_barrier_inserted = true;
  }
  ...
  // If only a memory barrier was added, the state itself didn't change, but
  // this reaffirms it.
  tracking.current_state = required_state;
}
  ...
  for (auto& [native_object, tracking] : tracking_) {
    ...
    std::visit(
      [&]<typename TDescriptor>(TDescriptor& info) {
        if (!info.is_permanent && info.keep_initial_state
