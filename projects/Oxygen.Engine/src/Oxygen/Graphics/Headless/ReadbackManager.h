...
class Graphics;
class HeadlessBufferReadback;
class HeadlessTextureReadback;

class HeadlessReadbackManager final : public graphics::ReadbackManager {
public:
  OXGN_HDLS_API explicit HeadlessReadbackManager(Graphics& graphics);
  OXGN_HDLS_API ~HeadlessReadbackManager() override;

  OXGN_HDLS_NDAPI auto CreateBufferReadback(std::string_view debug_name)
    -> std::shared_ptr<graphics::GpuBufferReadback> override;
  OXGN_HDLS_NDAPI auto CreateTextureReadback(std::string_view debug_name)
    -> std::shared_ptr<graphics::GpuTextureReadback> override;

  OXGN_HDLS_API auto Await(graphics::ReadbackTicket ticket)
    -> std::expected<graphics::ReadbackResult,
      graphics::ReadbackError> override;
  OXGN_HDLS_API auto AwaitAsync(graphics::ReadbackTicket ticket)
    -> co::Co<void> override;
  OXGN_HDLS_API auto Cancel(graphics::ReadbackTicket ticket)
    -> std::expected<bool, graphics::ReadbackError> override;

  OXGN_HDLS_API auto ReadBufferNow(
    const graphics::Buffer& source, graphics::BufferRange range = {})
    -> std::expected<std::vector<std::byte>, graphics::ReadbackError> override;
  OXGN_HDLS_API auto ReadTextureNow(const graphics::Texture& source,
    graphics::TextureReadbackRequest request = {}, bool tightly_pack = true)
    -> std::expected<graphics::OwnedTextureReadbackData,
      graphics::ReadbackError> override;

  OXGN_HDLS_API auto CreateReadbackTextureSurface(
    const graphics::TextureDesc& desc)
    -> std::expected<std::shared_ptr<graphics::Texture>,
      graphics::ReadbackError> override;
  OXGN_HDLS_API auto MapReadbackTextureSurface(
    graphics::Texture& surface, graphics::TextureSlice slice)
    -> std::expected<graphics::ReadbackSurfaceMapping,
      graphics::ReadbackError> override;
  OXGN_HDLS_API auto UnmapReadbackTextureSurface(graphics::Texture& surface)
    -> void override;

  OXGN_HDLS_API auto OnFrameStart(frame::Slot slot) -> void override;
  OXGN_HDLS_API auto Shutdown(std::chrono::milliseconds timeout)
    -> std::expected<void, graphics::ReadbackError> override;

private:
  friend class HeadlessBufferReadback;
  friend class HeadlessTextureReadback;
