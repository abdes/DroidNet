...
OXGN_HDLS_NDAPI auto Graphics::CreateCommandQueue(
  std::string_view /*queue_name*/, QueueRole /*role*/,
  QueueAllocationPreference /*allocation_preference*/)
  -> std::shared_ptr<graphics::CommandQueue>
{
  return std::make_shared<CommandQueue>("HeadlessQueue");
}
...
{
  // Create a simple CommandList from the common implementation so callers
  // that obtain command lists via the backend factory get a usable object.
  LOG_F(INFO, "Headless CreateCommandList requested");
  return std::make_unique<CommandList>(
    /*name=*/std::string_view("headless-cmdlist"),
    /*role=*/QueueRole::kNone);
}
