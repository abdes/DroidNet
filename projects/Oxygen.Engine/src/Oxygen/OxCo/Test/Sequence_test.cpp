...
struct LValueQualifiedImm {
    ...
    [[noreturn]] auto await_suspend(oxygen::co::detail::Handle) noexcept -> void { oxygen::Unreachable(); }
    auto await_resume() & -> int { return 42; }
};

struct RValueQualifiedImm {
    [[nodiscard]] auto await_ready() const noexcept -> bool { return true; }
    [[noreturn]] void await_suspend(oxygen::co::detail::Handle) noexcept { oxygen::Unreachable(); }
    auto await_resume() && -> int { return 42; }
};
...
TEST_CASE("Sequence")
{
    ...
    Run(el, [&]() -> Co<> {
        ...
        {
            int arr[2] = { 1, 2 };
            auto r = co_await (Just<int&>(arr[0]) | Then([](int& v) { return Just(&v + 1); }));
            CHECK(r == &arr[1]);
        }
        ...
        SECTION("cancellation (second task non cancellable)")
        {
            auto [r, _] = co_await AnyOf(
                el.Sleep(1ms)
                    | Then([&el] {
                          return el.Sleep(3ms, kNonCancellable);
                      }),
                el.Sleep(2ms));
            CHECK(el.Now() == 4ms);
            CHECK(r);
        ...
        SECTION("lifetime")
        {
            Semaphore sem { 1 };
            co_await AllOf(
                sem.Lock() | Then([&el] { return el.Sleep(5ms); }),
                [&]() -> Co<> {
        ...
        SECTION("with custom awaitable")
        {
            ...
            auto r = co_await (CustomAwaitable {} | Then([](const int v) { return Just(v + 1); }));
            CHECK(r == 100);
        }
