...
NOLINT_TEST_F(BroadcastChannelTest, BlocksWhenBufferFull)
{
  ...
  ::Run(*el_, [&]() -> Co<> {
    ...
    OXCO_WITH_NURSERY(n)
    {
      n.Start([&]() -> Co<> { sent = co_await writer.Send(3); });
...
NOLINT_TEST_F(BroadcastChannelTest, MultipleWritersCanSendConcurrently)
{
  ::Run(*el_, [this]() -> Co<> {
    ...
    OXCO_WITH_NURSERY(n)
    {
      n.Start([&]() -> Co<> { co_await writer.Send(1); });
      n.Start([&]() -> Co<> { co_await writer.Send(2); });
...
{
  BroadcastChannel<int> bounded_channel(
    1); // Use bounded channel to force blocking
  ::Run(*el_, [&]() -> Co<> {
    auto reader = bounded_channel.ForRead();
    auto& writer = bounded_channel.ForWrite();
    ...
    OXCO_WITH_NURSERY(n)
    {
      ...
      n.Start([&]() -> Co<> { write_completed = co_await writer.Send(1); });
      ...
      for (size_t w = 0; w < num_writers; ++w) {
        n.Start([&, w]() -> Co<> {
          for (size_t m = 0; m < msg_per_writer; ++m) {
            EXPECT_TRUE(
              co_await writer.Send(static_cast<int>(w * msg_per_writer + m)));
          }
        });
      }
