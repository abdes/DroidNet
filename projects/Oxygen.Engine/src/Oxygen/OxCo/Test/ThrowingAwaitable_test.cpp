...
struct ThrowingAwaitable {
  ...
  // NOLINTNEXTLINE(*-convert-member-functions-to-static)
  [[noreturn]] void await_suspend(oxygen::co::detail::Handle /*unused*/)
  {
...
NOLINT_TEST_F(ThrowingAwaitableTest, FirstInsideMux)
{
  ::Run(*el_, [&]() -> Co<> {
    // NOLINTNEXTLINE
    EXPECT_THROW(
      co_await AnyOf(ThrowingAwaitable(), el_->Sleep(5ms)), std::runtime_error);
...
NOLINT_TEST_F(ThrowingAwaitableTest, LastInsideMux)
{
  ::Run(*el_, [&]() -> Co<> {
    // NOLINTNEXTLINE
    EXPECT_THROW(
      co_await AnyOf(el_->Sleep(5ms), ThrowingAwaitable()), std::runtime_error);
    EXPECT_EQ(el_->Now(), 0ms);
  });
}
...
NOLINT_TEST_F(ThrowingAwaitableTest, InsideNonCancellableMux)
{
  ::Run(*el_, [&]() -> Co<> {
    // NOLINTNEXTLINE
    EXPECT_THROW(
      co_await AnyOf(el_->Sleep(5ms, oxygen::co::testing::kNonCancellable),
        ThrowingAwaitable()),
      std::runtime_error);
