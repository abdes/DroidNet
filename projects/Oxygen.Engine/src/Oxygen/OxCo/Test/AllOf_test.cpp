...
NOLINT_TEST_F(AllOfTest, Smoke)
{
  oxygen::co::Run(*el_, [this]() -> Co<> {
    co_await AllOf(el_->Sleep(2ms), el_->Sleep(3ms),
...
struct LValueQualifiedImm {
  ...
  void await_suspend(Handle) noexcept { oxygen::Unreachable(); }
...
NOLINT_TEST_F(AllOfTest, TakesLRef)
{
  oxygen::co::Run(*el_, []() -> Co<> {
...
{
  oxygen::co::Run(*el_, [this]() -> Co<> {
    co_await AllOf([&]() -> Co<> { co_await el_->Sleep(1ms); },
      [&]() -> Co<> {
        DLOG_F(INFO, "Immediate co_return");
        co_return;
      });
...
NOLINT_TEST_F(AllOfTest, RetVal)
{
  oxygen::co::Run(*el_, []() -> Co<> {
    auto [a, b] = co_await AllOf(
      []() -> Co<int> { co_return 42; }, []() -> Co<int> { co_return 43; });
    EXPECT_EQ(a, 42);
    EXPECT_EQ(b, 43);
  });
}
...
NOLINT_TEST_F(AllOfTest, Exception)
{
  oxygen::co::Run(*el_, [this]() -> Co<> {
    ...
    NOLINT_EXPECT_THROW(co_await AllOf(
      [&]() -> Co<> {
