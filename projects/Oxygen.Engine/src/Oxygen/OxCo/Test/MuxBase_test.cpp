...
public:
  ...
  TestMux(const size_t size, const size_t min_ready,
    const bool internal_cancel_result = false)
...
// For AnyOf semantics where MinReady() is less than Size(), the multiplexer
// should resume the parent coroutine when the condition is met.
...
// await_early_cancel() is called, and all awaitables are successfully
// cancelled.
...
NOLINT_TEST_F(MuxBaseTest, CancellationAfterPartialCompletion)
{
  ...
  // Cancellation should not be synchronous since some awaitables have already
  // completed
  EXPECT_FALSE(result);
...
NOLINT_TEST_F(MuxBaseTest, CountSynchronization)
{
  ...
  // Define the parent coroutine
  const auto parent_coroutine = [](bool& resumed)
    -> TestCoroutine { // NOLINT(cppcoreguidelines-avoid-capturing-lambda-coroutines)
    co_await std::suspend_always();
    resumed = true;
    co_return;
  }(parent_resumed);
...
NOLINT_TEST_F(MuxBaseTest, ParentCoroutineResumption)
{
  ...
  // Since the resumption is already tested, we ensure it's explicitly checked
  // in those test cases. Adding a comment here for completeness.
}
...
NOLINT_TEST_F(MuxBaseTest, AwaitEarlyCancelSkippable)
{
  ...
  EXPECT_TRUE(
    (std::is_same_v<decltype(mux.await_early_cancel()), std::true_type>));
}
