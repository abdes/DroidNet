...
auto MakeEvent() { return "Hello World!"; }
...
TEST_CASE("Event Stream - Synchronized Shared Benchmark")
{
  ...
  private:
    ...
    {
      co_await AllOf(poll_.Park(),
        [this]() -> Co<> { auto _ = co_await CurrentSlot().Lock(); });
    ...
    struct Slot {
      ...
      void Initialize(SharedEventSource* pump)
      {
        ...
        new (&event_awaitable) Shared<AsyncEventPumper>(std::in_place,
          [pump]() -> Co<EventType> { co_return co_await pump->PumpEvent(); });
      }
      ...
      auto Awaitable() -> Shared<AsyncEventPumper>& { return event_awaitable; }
    };
