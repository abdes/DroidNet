...
NOLINT_TEST_F(MostOfTest, Smoke)
{
  oxygen::co::Run(*el_, [this]() -> Co<> {
    co_await MostOf(el_->Sleep(2ms), el_->Sleep(3ms),
...
NOLINT_TEST_F(MostOfTest, NonCancellable)
{
  oxygen::co::Run(*el_, [this]() -> Co<> {
    bool resumed = false;
    auto sub = [&]() -> Co<> {
      auto [a, b, c] = co_await MostOf([&]() -> Co<int> { co_return 42; },
        el_->Sleep(3ms, kNonCancellable), el_->Sleep(5ms));
      EXPECT_TRUE(a);
      EXPECT_EQ(*a, 42);
      EXPECT_TRUE(b);
      EXPECT_FALSE(c);
