...
template <typename T> class OptionalTest : public testing::Test {
protected:
  T value_ = 42;
  T default_value_ = 0;
...
template <> class OptionalTest<int&> : public testing::Test {
protected:
  int value_ = 42;
...
class TestTypeNames {
public:
  template <typename T> static auto GetName(int /*unused*/) -> std::string
  {
...
NOLINT_TYPED_TEST(OptionalTest, ValueThrowsOnNullopt)
{
  ...
  NOLINT_EXPECT_THROW(
    [[maybe_unused]] auto _ = opt.value(), std::bad_optional_access);
}
