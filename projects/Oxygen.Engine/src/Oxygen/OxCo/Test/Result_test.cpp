...
template <typename T> class StorageTest : public testing::Test {
protected:
...
NOLINT_TEST_F(RefStorageTest, WrapAndUnwrapValue)
{
  ...
  auto die = [&wrapped]() {
    const auto bad = St::Unwrap(wrapped);
    (void)bad;
  };
...
NOLINT_TEST_F(NonTrivialStorageTest, WrapAndUnwrapValue)
{
  ...
  EXPECT_EQ(St::Unwrap(St::Wrap(NonTrivialType("Hello, World!"))).data(),
    "Hello, World!");
  EXPECT_EQ(St::UnwrapCRef(St::Wrap(NonTrivialType("Hello, World!"))).data(),
    "Hello, World!");
}
...
template <typename T> class ResultTest : public testing::Test {
protected:
  Result<T> result_;
};
