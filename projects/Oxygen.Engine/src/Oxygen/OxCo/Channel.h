...
namespace detail::channel {
  ...
  //! Channel<T>::Reader.
  template <typename T> class Reader : public ParkingLotImpl<Reader<T>> {
    friend Channel<T>;
    friend Writer<T>;
    ...
    public:
      ...
      {
        DLOG_F(5, "    ...channel {} receive {}", fmt::ptr(&GetChannel()),
          fmt::ptr(this));
        this->DoSuspend(h);
      }
  ...
  //! Channel<T>::Writer.
  template <typename T> class Writer : public ParkingLotImpl<Writer<T>> {
    friend Channel<T>;
    friend Reader<T>;
    ...
    public:
      ...
      void await_suspend(Handle h)
      {
        DLOG_F(5, "    ...channel {} send {}", fmt::ptr(&GetChannel()),
          fmt::ptr(this));
        this->DoSuspend(h);
      }
  ...
  public:
    ...
    template <typename U> auto Send(U&& value) -> Awaitable<bool> auto
    {
      return WriteAwaiter<U>(*this, std::forward<U>(value));
    }
    template <typename U> auto TrySend(U&& value) -> bool
    {
...
public:
  ...
  */
  ...
  ~Channel() override
  {
    DCHECK_F(!GetReader().HasWaiters(),
      "Still some tasks suspended while reading from this channel");
    DCHECK_F(!GetWriter().HasWaiters(),
  ...
  [[nodiscard]] auto GetReader() -> Reader&
  {
  ...
  [[nodiscard]] auto GetWriter() -> Writer&
  {
