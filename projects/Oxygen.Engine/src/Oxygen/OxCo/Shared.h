...
public:
  ...
  /*!
   The above awaitable may trigger an assert if all other awaiting parents were
   cancelled, thus propagating cancellation to the shared awaitable (since
   there's no way to construct any meaningful return value). This function may
   be used to handle such cases, in which it will return an empty optional.
  */
  co::Awaitable auto AsOptional();
...
template <class Awaitable>
auto Shared<Awaitable>::State::Suspend(AwaiterBase* ptr) -> detail::Handle
{
  DLOG_F(5, "    ...on shared awaitable {} (holding {})", fmt::ptr(this),
    fmt::ptr(&awaiter_));
...
Ret Shared<Awaitable>::State::ResultAs()
{
  ...
  // We get here if a new parent tries to join the shared operation after all of
  // its existing parents were cancelled and thus the shared task was cancelled.
  // The new parent never called suspend() so we don't have to worry about
  // removing it from the list of parents. We can't propagate the cancellation
  // in a different context than the context that was cancelled, so assert
  // instead.
  ABORT_F(
    "tasks were previously cancelled, so there is no value for new arrivals to "
    "retrieve (try `co_await shared.asOptional()` to guard against this)");
}
...
{
  DLOG_F(5, "shared awaitable {} (holding {}) resumed", fmt::ptr(this),
    fmt::ptr(&awaiter_));
...
template <class Awaitable>
auto Shared<Awaitable>::AwaiterBase::await_ready() const noexcept -> bool
{
...
template <class Awaitable>
auto Shared<Awaitable>::AwaiterBase::await_suspend(const detail::Handle h)
  -> detail::Handle
{
  parent_ = h;
...
template <class Awaitable>
auto Shared<Awaitable>::AwaiterBase::await_cancel(detail::Handle /*h*/) noexcept
{
...
template <class Awaitable> auto Shared<Awaitable>::Done() const noexcept -> bool
{
  return state_ ? state_->Done() : true;
}

template <class Awaitable> auto Shared<Awaitable>::Get() const -> Awaitable*
{
