...
#define OXCO_WITH_NURSERY(arg_name)                                            \
  ...
  [[nodiscard]] auto TaskCount() const noexcept -> size_t
  {
    return task_count_;
  }
  ...
  */
  template <class Ret = detail::Unspecified, class Callable, class... Args>
    requires(std::invocable<Callable, Args..., detail::TaskStartedTag>
      && Awaitable<
        std::invoke_result_t<Callable, Args..., detail::TaskStartedTag>>)
...
protected:
  ...
  template <class Ret>
  auto AddPromise(detail::Promise<Ret>* promise, TaskParent<Ret>* parent)
    -> detail::Handle;
  ...
  auto Continuation(detail::BasePromise* promise) noexcept
    -> detail::Handle override;
...
template <class Ret> class Nursery::StartAwaiterBase : protected TaskParent {
...
private:
  ...
  {
    CHECK_F(
      IsCancelling(), "Nursery task completed without signalling readiness");
  }
  ...
  auto Continuation([[maybe_unused]] detail::BasePromise* p) noexcept
    -> detail::Handle override
  {
...
public:
  ...
  void await_set_executor(Executor* ex) noexcept { this->executor_.SetPtr(ex); }
  ...
  auto await_suspend(detail::Handle h) -> detail::Handle
  {
    ...
    detail::PromisePtr<void> promise(std::apply(
      [this](auto&&... args) {
        return this->nursery_->MakePromise(
          std::move(callable_), std::move(args)..., MakeTaskStarted(this));
      },
      std::move(args_)));
  ...
  ~StartAwaiter()
  {
    if (this->nursery_) {
      std::apply(
        [this](auto&&... args) {
          this->nursery_->Start(std::move(callable_), std::move(args)...,
...
inline void Nursery::Adopt(detail::BasePromise* promise)
{
  ...
  DLOG_F(5, "pr {} handed to nursery {} ({} tasks total)", fmt::ptr(promise),
    fmt::ptr(this), task_count_ + 1);
  if (exception_) {
    promise->Cancel();
  }
...
template <class Ret>
auto Nursery::AddPromise(detail::Promise<Ret>* promise, TaskParent<Ret>* parent)
  -> detail::Handle
{
...
namespace detail {
  ...
  template <template <typename...> typename Template, typename T>
  constexpr inline bool IsSpecializationOf_v
    = IsSpecializationOf<Template, T>::value;

  // ReSharper disable once CppInconsistentNaming
  template <typename T>
  constexpr bool IsReferenceWrapper_v
    = IsSpecializationOf_v<std::reference_wrapper, T>;
} // namespace detail

template <class Callable, class... Args>
auto Nursery::MakePromise(Callable callable, Args... args)
  -> detail::Promise<void>*
{
  Co<> ret;
  if constexpr ((std::is_reference_v<Callable>
                  && std::is_invocable_r_v<Co<>, Callable>)
...
{
  DoStart(
    MakePromise(std::forward<Callable>(callable), std::forward<Args>(args)...));
}
...
auto Nursery::Start(Callable callable, Args... args) -> co::Awaitable auto
{
  if constexpr (std::is_same_v<Ret, detail::Unspecified>) {
    ...
    return StartAwaiter<Ret, Callable, Args...>(
      this, std::move(callable), std::move(args)...);
  }
}
  ...
  executor->Capture(
    [this] {
      for (detail::BasePromise& t : tasks_) {
        executor_->Schedule(
          +[](detail::BasePromise* p) noexcept { p->Cancel(); }, &t);
      }
    },
    task_count_);
...
inline auto Nursery::Continuation(detail::BasePromise* promise) noexcept
  -> detail::Handle
{
  DLOG_F(1, "pr {} done in nursery {} ({} tasks remaining)", fmt::ptr(promise),
    fmt::ptr(this), task_count_ - 1);
...
class Nursery::JoinAwaiter : public ParentAwaiter<JoinAwaiter<NurseryT>> {
...
public:
  ...
  {
    return nursery_.executor_ == nullptr;
  }
...
public:
  ...
  auto await_suspend(detail::Handle h) -> detail::Handle
  {
    ...
    DLOG_F(1, "    ... nursery {} starting with task {}", fmt::ptr(&nursery_),
      fmt::ptr(body.promise_.get()));
...
namespace detail {
  class BackReferencedNursery final : public Nursery {
    friend auto co::OpenNursery(Nursery*& /*n*/, TaskStarted<> /*started*/)
      -> Co<>;
