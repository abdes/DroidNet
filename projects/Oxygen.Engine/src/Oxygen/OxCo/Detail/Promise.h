...
namespace detail {
  ...
  class BasePromise : /*private*/ TaskFrame,
                      public IntrusiveListItem<BasePromise> {
    ...
    private:
      ...
      template <class T> static auto Control()
      {
    ...
    enum class Execution : size_t { // NOLINT(performance-enum-size)
      kReady = 0, //!< Used for tasks scheduled for execution (i.e. whose proxy
      //!< handled was `Resume()`ed).
      kRunning = 1, //!< Used for tasks currently being executed (i.e. whose
      //!< real handle was `Resume()`ed).
      kStub = 2 //!< Used for promises not associated with a coroutine, such as
      //!< `Just` or `NoOp`.
    ...
    [[nodiscard]] auto ExecutionState() const noexcept
    {
      return info_.state.execution;
    }
    [[nodiscard]] auto CancellationState() const noexcept
    {
      return info_.state.cancellation;
    }

    void ExecutionState(const Execution state) noexcept
    {
      info_.state.execution = state;
    }
    void CancellationState(const Cancellation state) noexcept
    {
      info_.state.cancellation = state;
    }

    [[nodiscard]] auto HasAwaiter() const noexcept
    {
      return ExecutionState() > Execution::kStub;
    }
    [[nodiscard]] auto HasCoroutine() const noexcept
    {
      return ExecutionState() != Execution::kStub;
    }

    [[nodiscard]] auto AwCancel(const Handle h) const noexcept -> bool
    {
      return info_.ccb.Cancel(h);
    }
    [[nodiscard]] auto AwMustResume() const noexcept -> bool
    {
      return info_.ccb.MustResume();
    }

    template <Awaitable Aw> void ResetControlBlock(Aw& aw)
    {
      info_.ccb = CancellationControlBlock(aw);
    }

  public:
    ~BasePromise() override { DLOG_F(5, "pr {} destroyed", fmt::ptr(this)); }
  ...
  protected:
    BasePromise() { DLOG_F(5, "pr {} created", fmt::ptr(this)); }
  ...
  private:
    ...
    [[nodiscard]] auto ProxyHandle() noexcept -> Handle { return ToHandle(); }
    ...
    */
    template <Awaiter Awaiter> auto HookAwaitSuspend(Awaiter& awaiter) -> Handle
    {
    ...
    template <Awaitable Awaitable> class AwaitProxy {
    ...
    public:
      ...
      auto await_resume() -> decltype(auto) { return awaiter_.await_resume(); }
  ...
  public:
    ...
    {
      BasePromise::Parent()->StoreException();
    }
