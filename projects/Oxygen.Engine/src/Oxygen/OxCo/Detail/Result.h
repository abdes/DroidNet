...
public:
  ...
  */
  ...
  //! Signals successful completion of an operation that does not produce a
  //! value.
  void StoreSuccess()
    requires(std::is_same_v<T, void>)
  {
  ...
  {
    return value_.index() != kIncomplete;
  }
  [[nodiscard]] auto HasValue() const -> bool
  {
    return value_.index() == kValue;
  }
  [[nodiscard]] auto HasException() const -> bool
  {
    return value_.index() == kException;
  }
  [[nodiscard]] auto WasCancelled() const -> bool
  {
    return value_.index() == kCancelled;
  }
...
protected:
  std::variant<std::monostate, typename Storage<T>::Type, std::exception_ptr,
    std::monostate>
