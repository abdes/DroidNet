  ...
  auto await_cancel(const Handle h) noexcept
  {
    const bool all_cancelled = [&] {
      ...
      auto guard = ScopeGuard(
        [this, original_parent]() noexcept { parent_ = original_parent; });
...
protected:
  ...
  [[nodiscard]] auto self() const -> const Self&
  {
  ...
  */
  [[nodiscard]] auto DoSuspend(const Handle h) -> bool
  {
    ...
    DLOG_F(5, "   ...on Mux<{}/{}> {}", self().MinReady(), self().Size(),
      fmt::ptr(this));
    parent_ = h;
    return true;
  }
  ...
  [[nodiscard]] auto HasException() const noexcept
  {
...
private:
  ...
  void Invoke(const std::exception_ptr& ex)
  {
    ...
    DLOG_F(5, "Mux<{}/{}> {} invocation {}{}", self().MinReady(), self().Size(),
      fmt::ptr(this), i, ex ? " with exception" : "");
    if (i == self().Size()) {
      parent_.resume();
