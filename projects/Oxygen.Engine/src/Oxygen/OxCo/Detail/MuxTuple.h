...
public:
    ...
    [[nodiscard]] auto await_ready() const noexcept -> bool
    {
        auto impl = [](const auto&... aws) {
            ...
            const size_t n_skip_kickoff = (static_cast<size_t>(aws.IsReady() || aws.IsSkippable()) + ...);
            return n_ready >= Self::MinReady() && n_skip_kickoff == sizeof...(Awaitables);
        };
    ...
    auto await_resume() && -> std::tuple<Optional<AwaitableReturnType<Awaitables>>...>
    {
...
template <class Self>
class MuxTuple<Self> : public MuxBase<Self> {
public:
