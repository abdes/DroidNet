...
*/
template <class T> class TaskAwaiter final : public TaskParent<T> {
public:
  ...
  */
  auto await_resume() && -> T { return std::move(this->result_).Value(); }
...
private:
  ...
  */
  ...
  {
    DCHECK_F(
      this->result_.Completed(), "task exited without `co_return`ing a result");
    promise_ = nullptr;
