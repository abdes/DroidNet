...
*/
template <class T> auto GetAwaiter(T&& t) -> decltype(auto);
...
template <class Aw>
using AwaiterType
  = RemoveRvalueReferenceT<decltype(GetAwaiter(std::declval<Aw>()))>;
...
public:
  // ReSharper disable once CppPossiblyUninitializedMember - lazy init when
  // appropriate
  explicit AwaitableLambda(
    Callable&& c) // NOLINT(cppcoreguidelines-rvalue-reference-param-not-moved)
...
template <class Callable, class Ret>
  requires(std::derived_from<std::invoke_result_t<Callable>, CoTag>
            && (std::same_as<Ret, Unspecified>
              || std::convertible_to<
                typename std::invoke_result_t<Callable>::ReturnType, Ret>))
...
template <class Callable, class ReturnType>
class [[nodiscard]]
YieldToRunImpl<Callable, ReturnType&&> : /*private*/ Callable {
public:
  explicit YieldToRunImpl(Callable cb)
    : Callable(std::move(cb))
  {
  ...
  {
    return static_cast<ReturnType&&>(result_);
  }
  void await_introspect(auto& c) const noexcept { c.node("Yield"); }
...
template <class Callable>
using YieldToRunAwaitable
  = YieldToRunImpl<Callable, std::invoke_result_t<Callable>>;
...
{
  static_assert(
    oxygen::co::Awaitable<T>, "tried to co_await on not an awaitable");
