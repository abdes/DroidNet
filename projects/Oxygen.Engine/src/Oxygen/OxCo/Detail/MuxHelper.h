...
protected:
  ...
  [[nodiscard]] auto InState(State state) const noexcept
  {
...
public:
  explicit MuxHelper(
    Aw&& aw) // NOLINT(cppcoreguidelines-rvalue-reference-param-not-moved)
  ...
  void ReportImmediateResult()
  {
    if (InState(State::kCancellationPending)) {
      ...
      // completes).
      SetState(
        awaitable_.await_must_resume() ? State::kReady : State::kCancelled);
    }
    if (InState(State::kCancelled)) {
...
private:
  ...
  void KickOff()
  {
    ...
    if (awaitable_.await_ready()) {
      ...
      resume_fn
        = +[](CoroutineFrame* frame) { static_cast<Self*>(frame)->Invoke(); };
      try {
  ...
  void ReportResult()
  {
    ...
    try {
      ...
      new (storage_) StorageType(Storage<Ret>::Wrap(awaitable_.await_resume()));
    } catch (...) {
      SetState(State::kFailed);
