...
protected:
  using Item = std::conditional_t<std::is_reference_v<Awaitable>,
    ItemWithoutAwaitable, ItemWithAwaitable>;
...
public:
  ...
  [[nodiscard]] auto MinReady() const noexcept
  {
    return std::min<size_t>(1, this->Size());
  }
...
public:
  ...
  [[nodiscard]] auto MinReady() const noexcept -> size_t
  {
    return this->size();
  }
};
