...
*/
template <bool HaveSpareBitsInPointers> class ProxyFrameImpl;

template <> class ProxyFrameImpl<true> : public CoroutineFrame {
public:
...
template <> class ProxyFrameImpl<false> : public CoroutineFrame {
public:
  void LinkTo(const Handle h)
  {
    // NOLINTNEXTLINE(*-pro-type-reinterpret-cast)
...
{
  return f && ProxyFrame::IsTagged(F::kTag, f) ? static_cast<F*>(f) : nullptr;
}
