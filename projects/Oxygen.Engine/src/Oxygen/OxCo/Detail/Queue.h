...
public:
  explicit Queue(const size_t capacity)
    : buffer_(
        static_cast<T*>(malloc(capacity * sizeof(T)))) // NOLINT(*-no-malloc)
    , capacity_(capacity)
    , head_(buffer_)
    , tail_(buffer_)
  {
  ...
  {
    auto destroy_range
      = [](std::span<T> range) { std::destroy(range.begin(), range.end()); };
  ...
  void PopFront()
  {
    head_->~T();
    if (++head_
      == buffer_ + capacity_) { // NOLINT(*-pro-bounds-pointer-arithmetic)
      head_ = buffer_;
    }
    --size_;
  }

  template <class U> void PushBack(U&& u)
  {
    if (size_ == capacity_) {
      Grow();
    }
  ...
  void Grow()
  {
    ...
    auto move_range = [&](std::span<T> range) {
      if constexpr (std::is_nothrow_move_constructible_v<T>) {
        q.tail_ = std::uninitialized_move(range.begin(), range.end(), q.tail_);
      } else {
        q.tail_ = std::uninitialized_copy(range.begin(), range.end(), q.tail_);
      }
      q.size_ += range.size();
    };
