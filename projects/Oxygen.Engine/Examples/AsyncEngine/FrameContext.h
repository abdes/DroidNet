...
template <typename PointerPolicy> struct GameDataCommon {
  ...
  void CopyFrom(const GameDataCommon<OtherPolicy>& other)
  {
...
struct ViewInfo {
  ...
  std::optional<oxygen::engine::View> view; // view-specific camera/matrices
...
struct GameStateSnapshot {
  ...
  std::vector<RenderSurface>
    surfaces; // per-frame surfaces (frozen at FrameStart)
  std::vector<uint8_t>
    presentable_flags; // per-surface presentable flags (1:1 with surfaces)
...
public:
  ...
  void AdvanceFrameIndex(EngineTag) noexcept { ++frameIndex_; }

  //! Engine-only: Advance epoch by one. Requires EngineTag capability.
  void AdvanceEpoch(EngineTag) noexcept { ++engine_state_.epoch; }
...
public:
  ...
  //! capability.
  void SetGraphicsBackend(std::weak_ptr<Graphics> graphics, EngineTag) noexcept
  {
  ...
  /*!
   Returns the current GPU fence value for synchronization purposes.

   ### Performance Characteristics

   - Time Complexity: O(1)
   - Memory: No allocation
   - Optimization: Lock-free atomic read

   ### Usage Examples

   ```cpp
   uint64_t fence = context.GetFrameFenceValue();
   if (fence > last_completed_fence) {
     // GPU work completed
   }
   ```

   @return Current fence value for GPU synchronization
   @see SetFrameFenceValue
  */
  ...
  void SetFrameFenceValue(uint64_t value, EngineTag) noexcept
  {
  ...
  void SetAsyncUploads(
    observer_ptr<ResourceIntegrationData> uploads, EngineTag) noexcept
  {
  ...
  void SetProfiler(observer_ptr<FrameProfiler> profiler, EngineTag) noexcept
  {
...
public:
  ...
  auto GetEntityCommandBuffer() const noexcept -> EntityCommandBuffer*
  {
    ...
    if (engine_state_.currentPhase == FramePhase::ParallelWork) {
  ...
  void SetEntityCommandBuffer(EntityCommandBuffer* cmds, EngineTag) noexcept
  {
  ...
  void SetUserContext(
    GameStateSnapshot::UserContextHandle context, EngineTag) noexcept
  {
    AssertMutationAllowed();
...
public:
  ...
  void SetThreadPool(oxygen::co::ThreadPool* pool, EngineTag) noexcept
  {
  ...
  void SetRenderGraphBuilder(
    observer_ptr<RenderGraphBuilder> builder, EngineTag) noexcept
  {
    engine_state_.graph_builder_ = std::move(builder);
  }

  auto& GetRenderGraphBuilder() const noexcept
  {
  ...
  */
  auto* GetFrameSnapshot() const noexcept
  {
    uint32_t idx = visibleSnapshotIndex_.load(std::memory_order_acquire);
  ...
  */
  ...
  void SetInputSnapshot(
    std::shared_ptr<const InputSnapshot> inp, EngineTag) noexcept
  {
  ...
  // tasks begin
  void SetViews(std::vector<ViewInfo> v) noexcept
  {
  ...
  void AddView(const ViewInfo& view) noexcept
  {
    std::unique_lock lock(snapshotLock_);
    if (!IsViewMutationAllowed()) {
      // TODO: Replace with project-specific assertion/logging
  ...
  // Only engine code possessing an EngineTag can change the phase. This
  // prevents external modules from changing frame lifecycle state while
  // allowing tests/engine internals to drive phases by producing an
  // EngineTag via internal::EngineTagFactory::Get().
  void SetCurrentPhase(FramePhase p, EngineTag) noexcept
  {
  ...
  // Enhanced mutation validation with detailed phase checking
  // RATIONALE: Different phases have different mutation restrictions.
  // This provides granular control over what operations are allowed when.
  auto CanMutateGameState() const noexcept
  {
    // GameState mutation is only allowed during ordered (Category A) phases
    // Parallel phases (Category B) must use snapshots only
    return engine_state_.currentPhase != FramePhase::ParallelWork
      && engine_state_.currentPhase != FramePhase::CommandRecord;
  }

  auto CanMutateEngineState() const noexcept
  {
    // EngineState can be mutated during most phases except parallel work
    // where only atomic fence reads are allowed
    return engine_state_.currentPhase != FramePhase::ParallelWork;
  }

  auto RequireSnapshot() const noexcept
  {
    // Parallel phases must use snapshot APIs exclusively
    return engine_state_.currentPhase == FramePhase::ParallelWork
      || engine_state_.currentPhase == FramePhase::CommandRecord;
  }

  auto HasSnapshotAccess() const noexcept
  {
    return engine_state_.currentPhase == FramePhase::ParallelWork
      || engine_state_.currentPhase == FramePhase::PostParallel
      || engine_state_.currentPhase == FramePhase::CommandRecord;
  }

  // Enhanced assertion with specific violation reporting
  // RATIONALE: Detailed error reporting helps identify phase violations
  // during development and debugging
  void AssertMutationAllowed() const noexcept
  {
    if (!CanMutateGameState()) {
      // TODO: Replace with project-specific assertion/logging
      // This is an instrumentation point for detecting phase violations
      // Example: LOG_ERROR("GameState mutation attempted during phase: {}",
      // GetCurrentPhase());
    }
  }

  void AssertEngineStateMutationAllowed() const noexcept
  {
    if (!CanMutateEngineState()) {
      // TODO: Replace with project-specific assertion/logging
      // Example: LOG_ERROR("EngineState mutation attempted during phase: {}",
      // GetCurrentPhase());
    }
  }

  void AssertSnapshotRequired() const noexcept
  {
    if (RequireSnapshot()) {
      // TODO: Replace with project-specific assertion/logging
      // Example: LOG_ERROR("Direct state access attempted during parallel
...
private:
  ...
  bool IsStructuralMutationAllowed() const noexcept
  {
    return engine_state_.currentPhase == FramePhase::FrameStart
      || engine_state_.currentPhase == FramePhase::SceneMutation;
  }
  ...
  // passes
  bool IsViewMutationAllowed() const noexcept
  {
    return engine_state_.currentPhase == FramePhase::FrameStart
      || engine_state_.currentPhase == FramePhase::SceneMutation
      || engine_state_.currentPhase == FramePhase::FrameGraph;
  }
...
public:
  ...
  void SetFrameTiming(const FrameTiming& t, EngineTag) noexcept
  {
  ...
  void SetFrameStartTime(
    std::chrono::steady_clock::time_point t, EngineTag) noexcept
  {
    AssertEngineStateMutationAllowed();
    frameStartTime_ = t;
  }
  ...
  void SetBudgetStats(const BudgetStats& stats, EngineTag) noexcept
  {
    AssertEngineStateMutationAllowed();
  ...
  void SetMetrics(const Metrics& metrics, EngineTag) noexcept
  {
    AssertEngineStateMutationAllowed();
    metrics_ = metrics;
  }
  ...
  std::vector<RenderSurface> GetSurfaces() const noexcept
  {
  ...
  void AddSurface(const RenderSurface& s) noexcept
  {
    std::unique_lock lock(snapshotLock_);
  ...
  void ClearSurfaces(EngineTag) noexcept
  {
    std::unique_lock lock(snapshotLock_);
    if (!IsStructuralMutationAllowed()) {
  ...
  void SetSurfaces(std::vector<RenderSurface> surfaces, EngineTag) noexcept
  {
    std::unique_lock lock(snapshotLock_);
  ...
  */
  void SetSurfacePresentable(size_t index, bool presentable) noexcept
  {
    std::shared_lock lock(snapshotLock_);
  ...
  */
  bool IsSurfacePresentable(size_t index) const noexcept
  {
    std::shared_lock lock(snapshotLock_);
    if (index >= engine_state_.presentable_flags.size()) {
      return false; // Index out of bounds
  ...
  */
  std::vector<uint8_t> GetPresentableFlags() const noexcept
  {
    std::shared_lock lock(snapshotLock_);
  ...
  */
  std::vector<RenderSurface> GetPresentableSurfaces() const noexcept
  {
    std::shared_lock lock(snapshotLock_);
    std::vector<RenderSurface> presentable_surfaces;
  ...
  */
  void ClearPresentableFlags(EngineTag) noexcept
  {
    std::unique_lock lock(snapshotLock_);
...
#if !defined(NDEBUG)
  // Comprehensive state validation for debugging
  // RATIONALE: Provides a single point to verify frame context consistency
  // during development and debugging scenarios
  struct ValidationResult {
    bool isValid = true;
    std::vector<std::string> errors;
  };

  auto ValidateState() const noexcept
  {
    ValidationResult result;

    // Check immutable state consistency
    if (!immutable_.config) {
      result.isValid = false;
      result.errors.emplace_back("Missing engine config");
    }

    // Check frame progression consistency
    if (frameIndex_ == 0 && engine_state_.currentPhase != FramePhase::Unknown
      && engine_state_.currentPhase != FramePhase::FrameStart) {
      result.isValid = false;
      result.errors.emplace_back("Invalid phase for frame 0");
    }

    // Check snapshot version consistency
    auto snapshotVersion = GetPublishedSnapshotVersion();
    if (snapshotVersion > frameIndex_) {
      result.isValid = false;
      result.errors.emplace_back("Snapshot version ahead of frame index");
    }
...
private:
  ...
  void PopulateFrameSnapshotViews(FrameSnapshot& frame_snapshot,
  ...
  // and views
  void PopulateFrameSnapshot(FrameSnapshot& frame_snapshot,
    const GameStateSnapshot& game_snapshot) const noexcept;
  ...
  struct GameState {
    ...
    // // Convenience accessors for backwards compatibility
    // auto& scene() { return gameData.scene; }
    // const auto& scene() const { return gameData.scene; }
    // auto& animation() { return gameData.animation; }
    // const auto& animation() const { return gameData.animation; }
    // auto& particles() { return gameData.particles; }
    // const auto& particles() const { return gameData.particles; }
    // auto& materials() { return gameData.materials; }
    // const auto& materials() const { return gameData.materials; }
    // auto& physics() { return gameData.physics; }
    // const auto& physics() const { return gameData.physics; }
    // auto& ai() { return gameData.ai; }
    // const auto& ai() const { return gameData.ai; }
    // auto& audio() { return gameData.audio; }
    // const auto& audio() const { return gameData.audio; }
  ...
  struct EngineState {
    ...
    FramePhase currentPhase = FramePhase::Unknown;

    // Thread pool pointer for spawning coroutine-aware parallel work
    observer_ptr<oxygen::co::ThreadPool> threadPool = nullptr;
