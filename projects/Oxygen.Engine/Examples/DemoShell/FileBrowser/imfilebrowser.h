...
enum ImGuiFileBrowserFlags_ : std::uint32_t {
  ...
  ImGuiFileBrowserFlags_HideRegularFiles = 1
    << 8, // hide regular files when ImGuiFileBrowserFlags_SelectDirectory is
          // enabled
  ImGuiFileBrowserFlags_ConfirmOnEnter = 1
    << 9, // confirm selection when pressing 'ENTER'
  ImGuiFileBrowserFlags_SkipItemsCausingError = 1
    << 10, // when entering a new directory, any error will interrupt the
           ...
           // process to continue.
  ImGuiFileBrowserFlags_EditPathString = 1
    << 11, // allow user to directly edit the whole path string
};
...
inline ImGui::FileBrowser::FileBrowser(
  ImGuiFileBrowserFlags flags, std::filesystem::path defaultDirectory)
  : width_(700)
  , height_(450)
  , posX_(0)
  , posY_(0)
...
inline void ImGui::FileBrowser::Display()
{
  ...
  if (editDir_) {
    if (setFocusToEditDir_) // Automatically set the text box to be focused on
                            // appearing
    {
    ...
    PushItemWidth(-1);
    const bool enter = InputText("##directory", currDirBuffer_.data(),
      currDirBuffer_.size(),
    ...
    if (flags_ & ImGuiFileBrowserFlags_EditPathString) {
      ...
      if (SmallButton("#")) {
        ...
        std::memcpy(
          currDirBuffer_.data(), currDirStr.data(), currDirStr.size());
...

  {
    ...
    for (unsigned int rscIndex = 0; rscIndex < fileRecords_.size();
      ...
      if (Selectable(rsc.showName.c_str(), selected, selectableFlag)) {
        ...
        const bool rangeSelect = canSelect && GetIO().KeyShift
...
inline void ImGui::FileBrowser::UpdateFileRecords()
{
  ...
  // We want [b0, a0, A1] to be sorted into something like [a0, A1, b0] instead
  // of [a0, b0, A1]. Therefore, here we compute a custom key for each filename
  // for sorting.
...
inline void ImGui::FileBrowser::AssignToArrayStyleString(
  std::vector<char>& arr, std::string_view content)
{
  if (content.empty()) {
    if (!arr.empty()) {
