...
void MainModule::Render(const oxygen::Graphics* gfx)
{
  ...
  renderer->Render(surface_, [this, &gfx](const RenderTarget& render_target) {
    return RenderGame(gfx, render_target);
  });
  std::this_thread::sleep_for(std::chrono::milliseconds(distribution(gen)));
}

void MainModule::OnShutdown() noexcept { platform_.reset(); }

auto MainModule::RenderGame(const oxygen::Graphics* gfx,
  const RenderTarget& render_target) const -> CommandLists
{
  ...
  const auto vertex_shader = renderer->GetEngineShader(MakeShaderIdentifier(
    ShaderType::kVertex, "Passes/Forward/ForwardMesh.hlsl"));

  const auto pixel_shader = renderer->GetEngineShader(MakeShaderIdentifier(
    ShaderType::kPixel, "Passes/Forward/ForwardMesh.hlsl"));
  ...
  const auto& [TopLeftX, TopLeftY, Width, Height, MinDepth, MaxDepth]
    = render_target.GetViewPort();
  command_recorder->SetViewport(
    TopLeftX, Width, TopLeftY, Height, MinDepth, MaxDepth);
  ...
  auto vertex_buffer
    = renderer->CreateVertexBuffer(vertices, sizeof(vertices), sizeof(Vertex));
