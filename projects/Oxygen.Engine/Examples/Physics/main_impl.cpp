...
auto RegisterEngineModules(oxygen::examples::DemoAppContext& app) -> void
{
  ...
  // Register built-in engine modules (one-time)
  {
    ...
    register_module(
      std::make_unique<oxygen::examples::physics_demo::MainModule>(app));
  ...
  -> co::Co<int>
{
  ...
  OXCO_WITH_NURSERY(n)
  {
    ...
    n.Start([&app, &n]() -> co::Co<> {
      ...
      LOG_F(
        INFO, "Physics example: last window closed -> shutting down engine");
...
extern "C" auto MainImpl(std::span<const char*> args) -> void
{
  ...
  try {
    ...
    auto cli
      = CliBuilder()
          .ProgramName("Oxygen.Examples.Physics")
          .Version("0.1")
          .About(
            "A demofor the end-to-end Oxygen physics integration using a "
            "procedural Ramp Gauntlet to Bowl scenario. The demo builds a "
            "scene from generated primitives (inclined ramp, static/dynamic "
            "cube obstacles, hinged flippers, and a bottom settling bowl), "
            "then exercises PhysicsModule runtime behavior through the full "
            "gameplay path. It includes a compact ImGui control panel for "
            "runtime reset/spawn controls and live diagnostics (active bodies, "
            "contacts, settle state), serving as both a developer showcase and "
            "a practical validation harness for scene hydration contracts and "
    ...
    app.engine = std::make_shared<AsyncEngine>(
      ...
      app.gfx_weak,
      EngineConfig {
        .application = { .name = "Physics Demo", .version = 1u, },
