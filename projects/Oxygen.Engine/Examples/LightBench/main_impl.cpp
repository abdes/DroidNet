...
extern "C" auto MainImpl(std::span<const char*> args) -> void
{
  ...
  try {
    ...
    auto cli = CliBuilder()
                 .ProgramName("light-bench")
                 .Version("0.1")
                 .About("LightBench PBR validation demo")
                 .WithHelpCommand()
                 .WithVersionCommand()
    ...
    LOG_F(INFO, "Starting LightBench for {} frames (target {} fps)", frames,
      target_fps);
    ...
    app.engine = std::make_shared<AsyncEngine>(
      ...
      app.gfx_weak,
      EngineConfig {
        .application = { .name = "LightBench", .version = 1u, },
