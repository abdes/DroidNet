...
Conan cache builds must match their recipe options exactly. UI instrumentation
is an ABI choice: an instrumented graph also requires its development examples.
It cannot be disabled through local output narrowing.
...
Developer `build-tree generate` enables UI instrumentation by default;
`-UiTests:$false` selects an uninstrumented graph. Conan dependency consumers
default to it being disabled. Keep ImGui's binary variant, Oxygen's compiled
headers and exported target definitions consistent across all configurations.
