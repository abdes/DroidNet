...
- `ToneMapPassConfig` (Runtime Engine Config, see [forward_pipeline.md](forward-pipeline-design.md))
...
The `PostProcessPanel` directly influences the structure of the `ForwardPipeline` render coroutine. For a deeper dive into the rendering architecture, see [Forward Pipeline Design](forward-pipeline-design.md).
...
1. **Standard PBR (Offscreen -> Composite)**:
    ...
    - **Mechanism**: The **Interceptor Pattern** ([REQ02](forward-pipeline-design.md#requirements-specification)) redirects view outputs (ViewContext) to an internal `Format::kRGBA16Float` HDR intermediate.
...
> Realign `ForwardPipeline` with the **Coroutine Graph** and **Interceptor Pattern** as specified in [forward_pipeline.md, REQ01-REQ10](forward-pipeline-design.md#requirements-specification).
...
- [x] **Camera Controls**: Implement "Manual (Camera)" exposure mode (Aperture/Shutter/ISO to EV100 conversion).
  ...
  - See [physical_lighting.md, Phase 2](physical-lighting-roadmap.md#phase-2--exposure-system-manual--physical) for `CameraExposure` struct specification.
