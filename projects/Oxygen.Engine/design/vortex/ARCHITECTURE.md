...
| `VelocityRendering` | distributed velocity writes across `DepthPrepassModule`, `BasePassModule`, and future late translucent/distortion handling | Motion-vector work is cross-stage in Vortex, not a standalone stage module. `SceneVelocity` in `SceneTextures` is the authoritative product. Written by `DepthPrepassModule` (partial), `BasePassModule` (completion), with optional late completion for remaining contributors. ~1.34k lines in UE5. |
| `SceneOcclusion` / HZB | `OcclusionModule` stage module in `SceneRenderer/Stages/Occlusion/` | Owns HZB generation, occlusion query batching/testing, feedback passes, HZB temporal handoff. Per-view persistent state (HZB textures, query pools) lives on view state. ~2.3k lines in UE5. |
| `BasePassRendering` | `BasePassModule` stage module in `SceneRenderer/Stages/BasePass/` | Owns GBuffer MRT passes, base-pass mesh processor, base-pass shader families, base-pass uniform setup. Canonical opaque material stage; deferred default, forward branch only where allowed. Completes `SceneVelocity` for skinned/WPO geometry. ~3.35k lines in UE5. |
...
| 9 | `RenderBasePass` | `BasePassModule::Execute` | BasePassModule | GBuffer MRT writes, emissive accumulation, `SceneVelocity` completion for skinned/WPO geometry. Deferred default, forward branch only where allowed. |
| 10 | Rebuild scene textures with GBuffers | inline `SceneTextures::RebuildWithGBuffers()` | SceneRenderer | ~50 lines. State transition, not a standalone module. `SceneVelocity` completion status finalized here. |
...
| 14 | `ComputeVolumetricFog` / `RenderHeterogeneousVolumes` / `RenderVolumetricCloud` | reserved `EnvironmentLightingService` volumetrics stages | future EnvironmentLightingService | UE splits this stage family across multiple volumetric systems. Vortex groups the stage family under Environment ownership. |
| 15 | `RenderSkyAtmosphere` / `RenderFog` | `EnvironmentLightingService::RenderSkyAndFog` | EnvironmentLightingService | Same high-level environment role in Vortex. |
...
| 19 | `RenderDistortion` / translucent `RenderVelocities` | reserved `DistortionModule::Execute` | future DistortionModule | Distortion mesh processing, intermediate resources, and late translucent velocity completion when required. |
...
| Velocity is distributed across stages 3, 9, and 19 | UE velocity is distributed across early/base/late passes depending on policy | Legacy has no equivalent dedicated deferred velocity family | Correct; do not reintroduce a standalone velocity stage module |
...
| `SceneVelocity` | scene velocity | per-pixel motion vectors | active |
...
| --- | --- |
| after stage 3 | `SceneDepth` plus `PartialDepth` and any early `SceneVelocity` contribution |
| after stage 9 | active GBuffers plus `SceneColor`; opaque velocity may still be incomplete |
...
| future volumetrics | directional/grid data may be reused through published products | a separate volumetrics-owned forward-light family |
...
| Indirect Lighting | `IndirectLighting/` | `IndirectLightRendering` | GI, reflections, SSR, subsurface scattering, SSAO production | reserved |
...
| environment shaders | `EnvironmentLightingService` | `Vortex/Services/Environment/` | `SkyAtmosphere*`, `HeightFog*`, `VolumetricFog*`, `VolumetricCloud*` |
...
| indirect-lighting shaders | future `IndirectLightingService` | `Vortex/Services/IndirectLighting/` | SSAO / reflections / GI / subsurface / Lumen-adjacent families |
