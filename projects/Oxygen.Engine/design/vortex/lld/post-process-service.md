...
[ten-slice delivery plan](../milestones/exposure/README.md).
...
Production uses FP32/P=1. The
[four decision pairs](../milestones/exposure/EX05.1/validation.md#ex051-05-results-and-decision)
explain the choice: admitted half storage saved 106.000/398.625 MiB at 1080p/4K,
but increased GPU frame p95 by 3.063/6.640 ms. C02/I02 retained FP32 while still
paying admission cost. All compared outputs met the image budgets.

The production path therefore skips recurring FP16 admission. `fp32-only`
remains the benchmark reference; `fp32` keeps certification with FP32 storage;
`qualified` exercises checked-half admission and retained-reader behavior.
Dynamic production admission was rejected for its measured time cost. Resource
lifetime and numerical requirements are unchanged. Further CPU reduction is
[VX-CPU-01](../OPEN_ITEMS.md).
...
ordinary-build isolation and final acceptance are recorded in
[EX10 closeout](../milestones/exposure/EX10/validation.md).
