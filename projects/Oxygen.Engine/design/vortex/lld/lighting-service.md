...
LightingService's publisher owns one immutable model-2 GGX energy texture for
all views. `BrdfEnergyData` verifies the compiled revision and payload hash;
`BrdfEnergyResources` publishes a single 32x32 RG32Float directional albedo/Schlick
texture. Its 8 KiB payload uses a square-root view coordinate and hardware
bilinear filtering. Direct and existing indirect consumers share the same
view-dependent compensation and diffuse energy preservation. The numerical
integrators and previous model data remain under Test, outside shipping code.

Preparation submits the one-time upload on the graphics queue before publishing
light headers. Later recordings consume the queue-ordered copy without a CPU
wait. Upload buffers retire through the graphics reclaimer, including their known
resource states; texture and descriptor retire together. Missing/mismatched data
or failed required allocation prevents complete lighting publication.

`tools/vortex/BuildGgxEnergyData.py generate` resamples the independently qualified
Test reference into `Lighting/Data/GgxEnergy.bin` and its measured-error manifest.
Its `embed` command generates the private build header. Never edit generated bytes.
