...
Status: **EX07A source audit and target mapping; implementation unqualified.**
Read with the [LightingService contract](lighting-service.md),
[GPU ABI contract](lighting-gpu-abi.md) and
[A review decisions](../plan/EX07A-contract-review.md#approved-decisions).
...
| LP05 `common.exposure_compensation_ev` | N/S/J/P/L/E/I/M; 0 EV; inspector [-10,10]; native finite value with representable resolved intensity                                 | CPU retains EV/base units; evaluation boundary resolves `2^EV` exactly once into lux/candela; atmosphere uses the same effective source                                               | V/A. +/-1 EV, zero intensity, compensation overflow, change camera exposure without changing authored light. Currently dropped by selection.                          |
...
| LP15 `range`                                                | N/S/J/P/L/E/I/M/T; 10 m; finite >=0, checked positive reciprocal/support; zero has zero influence                                                   | Authored range and inverse (zero for range zero); finite-source support uses per-emitter distances and center bound range+radius                                           | V/H. Zero clearing/reactivation; near/at/beyond support; off-screen finite extent; reject negative/nonfinite/unrepresentable candidates instead of flooring range.                     |
...
| LP18 `source_radius`                                        | N/S/J/P/L/E/I/M; 0 m, finite >=0 with representable derived evaluation; D3 physical sphere/disk extent                                              | PBR finite-emitter equations shared by diffuse/specular; range+radius conservative bounds and matching shadow coverage                                                     | V/H. Independent integration, flux, punctual/far-field limits, emitter-surface/interior, horizon/cone/range boundaries and in-flight radius edits.                                     |
| LP19 `inner_cone_angle_radians`, `outer_cone_angle_radians` | Spot N/S/J/P/L/E/I/M/T; 0.4/0.6 rad half-angles. 0<=inner<outer<=pi/2; hard extension 0<inner=outer<pi/2. Finite, nonzero representable solid angle | Preserve radians; GPU stores squared half-angle sine. One normalized profile for conversion/evaluation/bounds; cube/multiple-face shadows for finite/hemispherical support | V/H. Atomic cone pairs, negative/zero/reversed/nonfinite rejection, narrow and 90-degree soft cones, hard limit below 90, tangency, glTF candela-to-flux conversion using both angles. |
