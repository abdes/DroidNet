...
model 2 semantics and the implemented ingress/mutation contracts.
...
The atmosphere slot, per-pixel transmittance and RGB disk scale now cross the
source schema, packed record, loader, Lua and managed/editor adapters. EX07-C
records the migration and its qualification; the earlier repair inventory is
historical.

| Code | Current owner / route                                                                                                                  | Contract                                                                                              |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
...
field and rendered acceptance remains [ED-M08](../milestones/ED-M08/README.md).
...
| LP21 `AtmosphereLightSlot`                                     | Native None/Primary/Secondary; authored None. Explicit J/P/L/S/E/I/M transport                                                  | CPU source identity plus explicit slot; GPU None=invalid, Primary=0, Secondary=1                                            | Q/A/H. Uniqueness across stored inactive components; rejected conflict names occupant, lone Secondary retains slot, no fallback sun.             |
| LP22 `AngularSizeRadians`                                      | N/S/J/P/L/E/I/M; native 0; authored 0.00935 rad full diameter. Finite nonnegative with valid atmosphere solid-angle calculation | Atmosphere disk geometry/luminance only; not directional surface record radius                                              | A. Full versus half angle, zero/positive disk behavior, role None inactivity; PCSS/GGX broadening deferred (EV01-LIGHT-FINITE-SOURCE).           |
| LP23 `UsePerPixelAtmosphereTransmittance`                      | Native false; explicit J/P/L/S/E/I/M transport                                                                                  | CPU authority flag -> GPU mode bit; common atmosphere directional helper in each surface family                             | V/A. On/off with controlled altitude/path and each slot, no double ground+per-pixel attenuation.                                                 |
| LP24 `AtmosphereDiskLuminanceScale`                            | Native RGB defaults (1,1,1); finite >=0 with checked radiance. Stored and transported as three components                       | RGB atmosphere-model/disk publication; new source/packed/native/script/editor transport preserves RGB. Direct lux unchanged | A. Non-default RGB scale, both slots, save/cook/load/live mutation, radiance validation; no unused alpha lane or invented alpha control.         |
...
These fields cross native N, script S, J/P/L and managed/editor M/E/I.
DirectionalLightSource carries all cascade controls. Preserve inactive stored values. The common shadow
