...
| EX051-13 | Final performance acceptance / rendering owner | in_progress | 12 | Existing harness now separates 60 startup frames, steady samples, and the fixed I02 matched/event cycles; delayed status retains the normal bounded queue. Release harness and presentation app compile. First cell run selects a duration-safe count from its own warmup; two repeats reuse that count. Native script qualification, 16 production cells/48 valid runs and normal presentation remain. No performance acceptance claimed yet. |
