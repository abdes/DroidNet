...
| VTX-M05D | Conventional shadow parity and local-light expansion | `planned` | Directional ShadowService baseline exists, but M05D now starts with a UE5.7 CSM parity/stability audit because `CityEnvironmentValidation` projected shadows are unstable under camera movement. Updated ShadowService/local-light LLDs and dedicated M05D plan exist. | UE5.7 CSM audit, city-scale instability root cause and remediation, spot-light shadows, point-light strategy, publication, capture proof, debug-layer evidence, and required user visual confirmation. |
