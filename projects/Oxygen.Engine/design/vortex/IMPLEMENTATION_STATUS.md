...
| VTX-M05B | Occlusion consumer closeout | `in_progress` | LLD/plan updated and committed (`cd57ac692`); Slice B result substrate exists; Slice C now has structured candidate/result buffers, `VortexOcclusionTestCS`, conservative readback fallback, focused OcclusionModule tests (6/6), and ShaderBake/catalog proof for the new module. | Prove real D3D12 readback/result decode, add consumer filtering and diagnostics, then collect runtime/capture and D3D12 debug-layer evidence. |
