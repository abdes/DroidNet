...
| VTX-M05B | Occlusion consumer closeout | `in_progress` | LLD/plan updated and committed (`cd57ac692`); result substrate exists; HZB tester shader/catalog path has focused tests plus ShaderBake proof; base-pass solid/overlay consumers now filter valid occlusion results with focused SceneRendererDeferredCore tests. | Prove real D3D12 readback/result decode, add diagnostics export, then collect runtime/capture and D3D12 debug-layer evidence. |
