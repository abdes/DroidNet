...
**Next action:** implement the approved EX051-11A recorder-ownership candidate,
then EX051-11B constant/publication work. The
...
| EX051-11A | Adjacent exposure recorder ownership / PostProcess + ExposurePass | planned | 11 attribution | Record final SceneColor range and histogram/solve in one enclosing exposure submission; expected two-view I02 count 8 -> 6. Early frame resolve and pre-environment range remain separate. Preserve barriers, independent GPU scopes, submitted-state publication, failure/retry/acknowledgement semantics and reader/fence retention. Focused Debug numerical/range/failure/transition/queued-reader checks, then owning exposure gate and one matched Release I02 CPU comparison. Accept only with unchanged correctness and demonstrated preparation/submission benefit; otherwise preserve evidence and reject. No final matrix. |
