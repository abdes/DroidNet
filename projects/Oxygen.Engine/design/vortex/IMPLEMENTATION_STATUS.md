...
| VTX-M05B | Occlusion consumer closeout | `in_progress` | LLD/plan updated and committed (`cd57ac692`) to map UE5.7 `FHZBOcclusionTester`/`HZBOcclusion.usf` behavior onto a Vortex-native HZB consumer over `ScreenHzbModule`; Slice B result substrate/module shell is implemented with `Oxygen.Vortex.OcclusionModule` build plus OcclusionModule/SceneRendererPublication/SceneRendererDeferredCore tests passing. | Implement HZB tester pass/readback, consumer filtering, ShaderBake/catalog validation, runtime/capture proof, and D3D12 debug-layer evidence. |
