...
**Next milestone:** `VTX-M06A - Multi-view proof closeout`

**Why:** `VTX-M05A` through `VTX-M05D` are validated. The next planned gate is
heterogeneous per-view rendering proof, PiP/multi-surface routing, and per-view
capability/product isolation.

**Active work package:** `VTX-M06A - Multi-view proof closeout`

**Plan:** follow
the VTX-M06A row in [`PLAN.md`](PLAN.md).

**Current M05D status:** `validated`.
