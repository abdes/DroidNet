...
- Height-fog cubemap inscattering remains explicitly unavailable and deferred.
  ...
  closure.
- Immediate environment priority is city-scale AP/fog artifact remediation:
  aerial-perspective volume resolution/depth mapping, DemoShell environment
  override behavior, local-fog view enablement, volumetric-fog runtime behavior,
  and capture/analyzer proof.
...
| VTX-M04D.2 | UE5.7 exponential height fog parity | `in_progress` | Height fog model/pass code exists; VTX-M04D.2 widens the CPU/HLSL fog payload, translates primary/secondary layers, start/end/cutoff/max-opacity, directional and sky-atmosphere controls, removes the simplified fog fallback path, replaces the fog pass shader with a `HeightFogCommon`-shaped line-integral path, preserves far-depth sky exclusion, and records focused build/test/ShaderBake evidence below. | City-scale runtime/capture proof and artifact remediation remain open. Cubemap inscattering resource binding/sampling remains explicitly unavailable and deferred; it is not a blocker for the current city-scene AP/fog priority. Real local fog, volumetric fog, SkyLight capture/filtering, and runtime environment closure remain later milestones. |
...
| City-scale environment output shows visible AP/fog artifacts. | Environment parity and runtime proof. | Prioritize diagnosing and fixing the `CityEnvironmentValidation` path: AP LUT resolution/depth mapping, DemoShell override behavior, local-fog request plumbing, volumetric-fog runtime behavior, and capture/analyzer proof. Cubemap fog remains deferred. |
