...
**Next action:** obtain the owner disposition of the two documented CPU/copy
qualification exceptions, then finalize 14/GATE. The 48-run matrix, event scripts,
output comparisons and presentation are complete; benchmark reruns are prohibited. The
...
| EX051-13 | Final performance acceptance / rendering owner | in_progress | 12 | Collection/analysis complete: 48 valid runs, 296,100 steady frames; 981 frozen inputs unchanged; three passing fixed I02 event scripts, 64 passing output pairs, zero steady creation churn and one passing presentation check. All 1080p whole-frame and explicit-dispatch/scaling targets pass. Remaining gate disposition: active-only exposure CPU/scaling proof and exact color-copy attribution within the combined color/depth resolve timer. No pass claimed for those sub-budgets; no benchmark reruns. [Results and proposed exceptions](lld/post-process-service.md#ex051-13-final-collection-and-gate-disposition), [checkpoint](../../out/build-ninja/analysis/vortex/exposure-lightbench/slice51/acceptance13/checkpoint-manifest.json). |
| EX051-14 | Owner and evidence closeout | in_progress | 13 | Owner results, comparison decisions, implementation commits, raw evidence index and Slice 5.2 handoff boundary are reconciled. Final status depends only on owner disposition of the two explicit qualification exceptions. No new implementation or measurement programme; benchmark reruns prohibited. |
| EX051-GATE | Slice acceptance | in_progress | 12, 13, 14 | Numerical/integration, 48-run collection, whole-frame targets, explicit exposure/scaling, transitions, resource stability and presentation are qualified. Active-only CPU and exact exposure-copy sub-budget qualification remain unproven; proposed explicit exceptions require owner acceptance. Slice 5.2 is prepared but not unblocked. |
