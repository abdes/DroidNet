...
| VTX-M04D.6 | UE5.7 aerial perspective parity | `planned` | Current source inspection found Vortex aerial-perspective shader sampling and atmosphere LUT infrastructure, and UE5.7 references were checked for camera AP generation, view publication, and base-pass application. This is only preservation evidence, not parity proof. | Build the AP source-to-target mapping, remediate stale/partial implementation behavior, prove camera-volume generation/sampling and height-fog coupling, propagate any AP impact back to VTX-M04D.1 and VTX-M04D.2 with fresh evidence, add focused tests, run shader validation where needed, and produce runtime/capture proof before any validated claim. |
...
| Aerial perspective is not UE5.7 parity-closed. | Atmosphere parity and environment runtime proof. | Complete VTX-M04D.6: map current AP generation/sampling/application to UE5.7, remediate gaps, propagate any AP impact to VTX-M04D.1 and VTX-M04D.2, add focused tests, run shader validation where needed, and record runtime/capture proof. |
...
| Atmosphere LUT family | Implementation exists for transmittance, multi-scattering, distant sky light, sky view, and camera aerial perspective. Current evidence preserves behavior but does not prove aerial-perspective parity. | VTX-M04D.1 preserve / VTX-M04D.6 AP parity | Focused environment tests plus capture/resource proof where applicable; VTX-M04D.6 must specifically prove camera AP volume generation, sampling, main-pass application, height-fog coupling, and resource validity. |
