...
**Next action:** 11 CPU attribution using existing profiling; separate active
exposure preparation/submission from queue waits. The user approved the state table and measured memory tradeoff.
...
| EX051-11 | Active CPU attribution / PostProcess + Graphics callers | in_progress | 05, 09 | One existing owner scope added; Debug smoke and instrumented Release I02 pass, with 6,000 samples and 1,005 frozen inputs unchanged. Exposure elapsed union is 0.776/0.946 ms p95/p99; frame-start waits are separate. This upper bound includes driver/scheduling time and does not demonstrate the active-CPU target. Localize acquisition/recording/submission before choosing a correction. Status polling is negligible. No general submission-system rewrite. [CPU checkpoint](../../out/build-ninja/analysis/vortex/exposure-lightbench/slice51/cpu-attribution/checkpoint-manifest.json). |
