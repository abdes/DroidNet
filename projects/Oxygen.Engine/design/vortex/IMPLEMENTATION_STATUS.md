...
| 5.1 — Exposure performance | in_progress | EX051-10 H4 depth-snapshot sharing is retained; remaining recycling is open. H3 producer-bound reduction was rejected. EX051-07 H1 maximum reuse is committed; H2 scan fusion is retained after 216/216 owning Debug tests and two Release comparisons. Qualification savings repeat; whole-frame improvement is not established. Remaining workload/reference coverage and final performance budgets stay open. | [Current results and next action](#31-current-work), [tasks and budgets](#321-slice-51-performance-qualification-and-correction) |
...
**Active item:** EX051-10 — resource lifetime and allocation cost.
**H4 is complete and retained for this increment:** resolved and previous depth
share one immutable, retained snapshot. Its [closeout manifest](../../out/build-ninja/analysis/vortex/exposure-lightbench/slice51/h4-closeout-manifest.json)
records source/commit identities, 91 owning checks, two native queued-consumer
passes and the two final publication-guard controls. No build or native run is
active. Next: address remaining measured allocation/recycling work using existing
resource ownership; do not introduce an unmeasured cache rewrite.

The matched I02 pair reduces texture creation 14 -> 12/frame and live placement
842.520 -> 777.707 MiB; resolve/cleanup mean/p95 is 0.327/0.335 -> 0.216/0.220 ms.
The pair predates the subsequent failure-path readiness guard, which passed its
focused controls. Its measured source is preserved; these are not new timings
for the final guarded revision. Whole-frame variation, the worse 60.704 ms wall
maximum and all budget misses remain recorded.

Twelve texture and two buffer creations per two-view frame remain, along with
lifecycle peak/retirement qualification. H4 has no dependency on EX051-09's
admission policy; that bounded sequence exception does not close EX051-10 or the
slice. H3 remains rejected in its [closeout](../../out/build-ninja/analysis/vortex/exposure-lightbench/slice51/h3-closeout-manifest.json).

**Retained checkpoint:** EX051-07 H1/H2 are committed; H2 is `efecd6185`.
[H2 closeout and detailed results](../../out/build-ninja/analysis/vortex/exposure-lightbench/slice51/h2-closeout-manifest.json)
record 216/216 owning Debug passes, two Release comparisons, unchanged output
budgets and the repeated qualification saving. Whole-frame speedup is not
established; all outliers and budget misses remain in the evidence and task row.
EX051-07 overall remains open.

**Remaining acceptance:** EX051-03/04/05 retain workload, reference, memory and
attribution gaps. The performance matrix, CPU attribution, resource churn and
final 60-fps/subsystem budgets remain open. Instrumentation overhead is accepted
and frozen; no further H2 or overhead run is queued.

**Next gate:** EX051-GATE precedes Slice 5.2, whose quality edits require the
EX052-03 diagnostic-inventory/design agreement. Both approved inserted slices
must close before original Slice 6; the full package remains active.
