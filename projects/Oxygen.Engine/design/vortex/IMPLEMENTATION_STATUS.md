...
| VTX-M05B | Occlusion consumer closeout | `in_progress` | LLD/plan updated and committed (`cd57ac692`); result substrate exists; HZB tester shader/catalog path has focused tests plus ShaderBake proof; base-pass solid/overlay consumers filter valid occlusion results; diagnostics now publish compact occlusion/base-pass draw facts and export stable descriptors while filtering transient bindless indices. Focused OcclusionModule, ShaderBake, SceneRendererDeferredCore, SceneRendererPublication, and DiagnosticsCaptureManifest validations have passed. | Prove real D3D12 readback/result decode in runtime, collect capture/debug-layer evidence, and obtain required user visual confirmation before closeout. |
