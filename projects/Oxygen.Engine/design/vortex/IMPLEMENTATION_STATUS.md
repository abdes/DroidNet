...
**Performance resumed (2026-09-20): 09 validated; 11 CPU attribution is active.** The eight
...
**Next action:** decide the bounded scope extension identified by
[11 CPU attribution](lld/post-process-service.md#ex051-11-cpu-attribution-checkpoint).
The existing traces do not demonstrate the active-CPU budget; no production
correction has been selected. The user approved the precision state table and measured memory tradeoff.
...
| EX051-11 | Active CPU attribution / PostProcess + Graphics callers | in_progress | 05, 09 | Original and detailed frozen traces each pass Debug smoke 1/1 and Release I02 1/1, with 6,000 samples and 1,005 unchanged inputs. Detailed exposure elapsed union is 0.762/0.902 ms p95/p99, eight submissions/frame, zero explicit fence waits inside exposure. Subtracting native submission still leaves 0.563/0.662 ms; this is not an active-only result. Acquisition/finalization/binding account for 59% of aggregate elapsed time. No bounded correction yet demonstrates the target. [Scope decision and attribution](lld/post-process-service.md#ex051-11-cpu-attribution-checkpoint) are pending before broader recorder/publication work; no production correction or general submission rewrite. [CPU checkpoint](../../out/build-ninja/analysis/vortex/exposure-lightbench/slice51/cpu-attribution/checkpoint-manifest.json). |
