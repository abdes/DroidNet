...
**Next milestone:** `VTX-M05D - Conventional shadow parity and local-light expansion`

**Why:** `VTX-M05A`, `VTX-M05B`, and `VTX-M05C` are validated. The directional
CSM portion of `VTX-M05D` is validated, and Slice E spot-light conventional
shadows are validated. The explicit point-light strategy/implementation still
remains in the M05D scope.

**Active work package:** `VTX-M05D - Conventional shadow parity and local-light
expansion`

**Plan:** follow
[`plan/VTX-M05D-conventional-shadow-parity.md`](plan/VTX-M05D-conventional-shadow-parity.md).

**Current M05D status:** `in_progress`.
...
| VTX-M05D | Conventional shadow parity and local-light expansion | `in_progress` | Directional CSM UE5.7 audit/remediation is recorded in `shadow-service.md` and `VTX-M05D-conventional-shadow-parity.md`: stable/no-AA frusta, sphere bounds, texel snapping, 5000-unit directional depth extent, UE-style non-last transition overlap coverage, optional CSM constant/slope depth bias, default zero bias, and resolution-hint wiring are present. Follow-up validation fixed the Release scene descriptor/loader path to use Serio, corrected the `VsmTwoCubes` local-scale CSM descriptor/settings, recooked the scene, ran the Release `RenderScene --scene VsmTwoCubes --directional-shadows conventional` smoke with exit 0, captured `release_frame60_vsm_two_cubes_verified5_capture.rdc`, and ran sequential RenderDoc analyses: shadow timing `Vortex.Stage8.ShadowDepths` success with 12 work events, shader timing `Vortex.Stage9.BasePass.MainPass` success with 9 work events, and shadow focus success with 12 inspected work events. Slice E spot-light conventional shadows are implemented across ShadowService/LightingService/Stage 8/Stage 12/HLSL bindings, with focused `SpotShadowValidation` scene and `ProbeRenderDocSpotShadow.py`. Diagnostic capture `spot-shadow-validation.bias01.spot-shadow-probe.txt` isolated the all-clear spot-depth failure to bias moving perspective `SV_Position`; the fix writes biased spot-axis reversed depth via `SV_Depth` while keeping raster projection unbiased. The scene was recooked with authored spot bias `0.0`, and the user confirmed the resulting projected spot shadows were perfect. Review remediation also clears published spot slices when there are no casters, keeps spot depth unclamped until pixel `SV_Depth`, adds CPU/HLSL ABI guards, and documents Stage 18 spot-shadow consumption as deferred. Focused build/shader validation passed; tests passed ShadowService 8/8, SceneRendererDeferredCore 41/41, LightingService 4/4, ShaderBakeCatalog 4/4. CDB report `spot-shadow-validation.bias0.final.debug-layer.report.txt` passed with runtime exit 0, no debugger break, 0 D3D12/DXGI errors, and 0 blocking warnings. RenderDoc probe `spot-shadow-validation.bias0.final.spot-shadow-probe.txt` proves Stage 8 spot draws 168/171, non-clear `Vortex.SpotShadowSurface` (`max=0.463512063`, center=0.447184265), Stage 12 spot draw 248, and Stage 12 spot SRV binding. | Point-light conventional shadow strategy/implementation is not implemented/proven or explicitly deferred with approval. Next task is Slice F: point-light conventional shadows or approved deferral. |
