...
gate first audits/remediates directional CSM parity and city-scale camera
stability, then expands conventional shadow coverage beyond the directional
baseline.
...
| VTX-M05D | Conventional shadow parity and local-light expansion | `in_progress` | Directional CSM UE5.7 audit mapping is recorded in `shadow-service.md` and `VTX-M05D-conventional-shadow-parity.md`. Current remediation touches `CascadeShadowSetup` stable/no-AA frusta, sphere bounds, texel snapping, 5000-unit directional depth extent, UE-style non-last transition overlap coverage, UE-style optional CSM constant/slope depth bias for nonzero `shadow.bias`, engine default `shadow.bias = 0.0`, and Environment-panel shadow resolution hint wiring into the conventional shadow surface allocator. Earlier focused build/ShaderBake and Vortex shadow tests passed before the latest source-only edits. | Rebuild/run validation is intentionally pending because the user is the only one building/running during this regression; then physics-domain proof, user visual validation when requested, spot-light shadows, point-light strategy, publication, and final local-light proof. |
