...
integration and proof script hardening have landed. Release-default policy and
direct EngineShaderCatalog introspection are now proven. Diagnostics panel
runtime UI smoke and runtime/capture/debug-layer evidence remain open;
ShaderBake evidence is required only if later M05A changes alter shader-visible
requests.
