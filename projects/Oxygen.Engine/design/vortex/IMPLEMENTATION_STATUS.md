...
- **Open visual report: all-white MultiView meshes.** The user observed all
  ...
  lit-scene report. EX05-04/29 remain open until the original mesh/material scene
  has a reproduced, diagnosed and visually verified result in main and lit PiP.
- **Active item: EX05-15 — final consumer error-bound propagation.** Producer
  ...
  **Completed substep: opaque input range before environment attenuation.**
  ...
  units; GPU storage is 144 bytes and CPU readback remains 80. Debug/Release each
...
| EX05-29 | Complete standard/auxiliary/offscreen/feature layout matrix | in_progress | Routing and selected captures exist. User-reported all-white meshes remain an open visual issue: the earlier atmosphere capture confirms washed-out mesh/ground output, while later isolated emissive cards do not qualify the original lit scene. Reproduce and diagnose the original mesh/material scene, then inspect corrected main/lit-PiP output. Not every lit pane/layout has complete visual and standalone-equivalence proof. The blue unavailable-shadow diagnostic does not prove the directional-shadow layout. | [Failed earlier appearance](../../out/build-ninja/analysis/vortex/exposure-lightbench/multiview/atmosphere-Debug-42.exposure.png), [diagnostic scope/limitations](../../out/build-ninja/analysis/vortex/exposure-lightbench/multiview/diagnostic-manifest.json), [required scenarios](plan/exposure-and-lightbench-correction.md#75-multiview-visual-acceptance) |
