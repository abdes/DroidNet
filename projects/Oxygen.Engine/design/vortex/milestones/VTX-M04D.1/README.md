...
Dependencies: VTX-M02, current environment code

## Delivered scope

Truthful environment binding/publication state, SkyLight/IBL truth, Stage 14 observability.
...
Fog parity belongs to the later fog milestones. This package prepares the environment
contract so later fog parity work can be implemented and verified reliably.

Validation evidence is recorded in
[Milestone record](../../PLAN.md). Real SkyLight
...
- update `PLAN.md` and `milestone README` if implementation findings
...
The milestone plan must audit these publications.
...
- record every field whose truth rule changed in `milestone README`
...
| Area            | Paths                                           |
| --------------- | ----------------------------------------------- |
| status ledger   | `design/vortex/PLAN.md`                         |
| milestone plan  | `design/vortex/PLAN.md`                         |
| detailed plan   | `design/vortex/milestones/VTX-M04D.1/README.md` |
...
7. `milestone README` records files changed, commands run, results,
