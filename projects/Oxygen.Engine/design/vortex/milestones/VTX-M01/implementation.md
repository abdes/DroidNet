...
- `design/vortex/PLAN.md` for current completion state and
  evidence ledger
...
6. `milestone README` records the evidence literally and does not claim
...
9. **Using this LLD as live status instead of stable design.** Current
   execution state must come from `milestone README` and the active
