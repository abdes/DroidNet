...
| Field     | Summary                                                       |
| --------- | ------------------------------------------------------------- |
| Outcome   | RenderScene preview sun, profile persistence and UI behavior. |
| Remaining | None in the recorded scope.                                   |
| Evidence  | [Record below](#renderscene-preview-sun)                      |

Completed 2026-09-16. Debug and Release builds, 97 focused tests, scripted native checks,
and manual visual/UI validation support this scope. The subsequent EX07 and EX08–EX10 packages record the lighting and
exposure corrections; this milestone covers the preview-sun/profile workflow.
...
| Manual visual/UI acceptance | Manual checks confirmed the requested help readability, Custom edit/reopen persistence, sun-only behavior, and final Sponza sun/profile checks on 2026-09-16. Validation method: manual visual/UI checks.                                                                   |

The native scenario matrix predates the final observer/contrast refinement; the
97-test run and manual acceptance cover the final implementation. Earlier
...
That issue was outside the preview-sun workflow and was subsequently handled
by the exposure package. See [EX09](../exposure/EX09/README.md).
Exposure correction remains in
[its owning plan](../exposure/README.md). These checks do not close
ED-M08 native data, renderer parity, captured-sky IBL, or full visual qualification.

## Capture tooling correction

The capture scripts must preserve reliable process and replay handling. The attempted replay
