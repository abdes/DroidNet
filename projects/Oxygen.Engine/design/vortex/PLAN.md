...
RenderDoc scripted proof. Deferred diffuse SkyLight consumption, allocation
churn proof, user visual confirmation, and proof closeout remain open.
...
| Cubemap skybox / static specified-cubemap SkyLight | VTX-M08 | `in_progress` | Slice A data/plan state is validated. Slice B product-state/source-validation, CPU cubemap/SH processing, renderer-owned GPU product upload, shader-facing SkyLight ABI publication, and existing forward/debug/local-fog consumer migration are focused-test/shader validated. Slice C visual SkySphere publication and Sky pass shader branching are source/shader validated; runtime cubemap skybox proof now exists through RenderScene startup-skybox loading, CDB/debug-layer audit, RenderDoc capture, and scripted skybox analysis. Remaining work: directional sun interaction proof, deferred static SkyLight diffuse lighting, allocation-churn proof, visual confirmation, and closure evidence. Captured-scene SkyLight, real-time capture, cubemap blending, SkyLight occlusion, baked/static-lightmap integration, and broader reflection probes remain deferred. |
