...
| Feature-gated variants | VTX-M06C | `in_progress` | Detailed plan exists. Slice B feature-profile vocabulary/carry, Slice C depth-only/shadow-only stage gates, and Slice D no-environment/no-shadowing/no-volumetrics gates have focused source/test evidence. Diagnostics-only gates and runtime proof remain open. |
