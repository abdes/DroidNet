...
validated. Slice B product-state/resource-validation groundwork is in progress
with focused tests and ShaderBake/catalog validation. Cubemap processing,
skybox rendering, diffuse SkyLight consumption, and proof closeout remain open.
...
| Cubemap skybox / static specified-cubemap SkyLight | VTX-M08 | `in_progress` | Slice A data/plan state is validated. Slice B product-state/source-validation groundwork is in progress and focused-test/shader validated, but no processed cubemap or SH product is published yet. Remaining work: visual cubemap skybox background, cubemap product processing, deterministic procedural-sky versus skybox selection in runtime, directional sun interaction proof, static SkyLight diffuse lighting, shader consumer migration, proof tooling, and closure evidence. Captured-scene SkyLight, real-time capture, cubemap blending, SkyLight occlusion, baked/static-lightmap integration, and broader reflection probes remain deferred. |
