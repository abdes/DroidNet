...
- `EnvironmentLightingService` is substantial but not fully parity-closed.
  ...
  fog, and volumetric fog are validated in VTX-M04D.2/VTX-M04D.3/VTX-M04D.4,
  while aerial perspective proof, full runtime closure, and real SkyLight
...
`VTX-M04D.3 — UE5.7 Local Fog Volume Parity`, and `VTX-M04D.4 — UE5.7
Volumetric Fog Parity`. These stabilize truthful environment publication,
validated exponential height fog, analytical local-fog volumes, and the
validated integrated volumetric-fog product path before full runtime closure.

**Active work packages:** `VTX-M04D.6 — UE5.7 Aerial Perspective Parity`.
...
height-fog cubemap inscattering. Cubemap fog is explicitly deferred. The next
implementation focus is the remaining `CityEnvironmentValidation` AP proof and
environment runtime closure path. AP LUT resolution/depth mapping, DemoShell
override behavior, local-fog request plumbing, volumetric-fog runtime behavior,
height-fog runtime/capture proof, and city-scale fog capture proof now have
evidence. A first Vortex-native
...
validates the analytical local-fog volume path, and VTX-M04D.4 has a first
local-fog participating-media injection slice that consumes the validated
Stage-14 tiled local-fog products before the volumetric pass records. The
volumetric material setup now also evaluates UE-shaped spatial density from
the primary and secondary exponential height-fog layers instead of using a
constant froxel extinction. A first temporal-history slice also exists:
per-view integrated-scattering textures are retained across matching
volumetric grids and reprojected through Oxygen's previous-view bindings with
UE5.7's default `0.9` history weight. The temporal path now also uses
UE-shaped Halton jitter, reset behavior, and history-miss supersampling, with
paired VortexBasic capture proof that temporal reprojection changes the
sampled `IntegratedLightScattering` volume. This remains an Oxygen-shaped
single-product temporal path: UE's conservative-depth history fixup,
pre-exposure transfer, and separate `LightScattering` history product are not
claimed. Focused VortexBasic proof now also samples the captured
`IntegratedLightScattering` 3D product itself across representative z slices
and records nonzero RGB plus valid transmittance alpha.
The small VortexBasic proof scene now has narrow term controls for disabling
local-fog volumetric injection, SkyLight volumetric scattering, and directional
volumetric shadow visibility during follow-up captures. Paired focused
...
local-fog volumes, SkyLight LUT, temporal reprojection, height-fog static
payload/application, and debug-layer-clean D3D12 execution.
...
- VTX-M04D.6 has a first implementation pass and focused build/test/shader-bake
  evidence recorded in `IMPLEMENTATION_STATUS.md`.
...
- The milestone remains `in_progress` until capture/analyzer proof exists and
  reflection/360-view AP behavior is proven or explicitly deferred.
...
| ShadowService directional baseline | VTX-M03 / VTX-M04D.4 prerequisite | `validated` | Directional conventional shadow data publication and projected receiver shadows are validated for the directional CSM path. The city-scale loader follow-up now preserves cooked manual CSM split mode, max distance, transition, and fadeout settings on runtime directional lights. VTX-M04D.4 consumes this product for volumetric directional shadowing rather than treating projection correctness as open. |
