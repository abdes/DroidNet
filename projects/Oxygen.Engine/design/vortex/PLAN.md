...
validated. Slice B product-state/resource-validation and CPU cubemap/SH
processing groundwork is in progress with focused tests and ShaderBake/catalog
validation. GPU product publication, skybox rendering, diffuse SkyLight
consumption, and proof closeout remain open.
...
| VTX-M08 | Skybox and static specified-cubemap SkyLight | `in_progress` | VTX-M07, VTX-M04D environment publication truth, VTX-M05D shadows, VTX-M06C feature gates | Slice A data and plan state is implemented and focused-test validated: SkyLight authored fields, 96-byte SkyLight environment record with legacy 84-byte load support, scene descriptor/PakGen/PakDump/DemoShell propagation, and per-view visible-sky plan state with no-environment gating. Slice B is in progress: explicit static SkyLight product state, dedicated `diffuse_sh_srv` probe binding, CPU/HLSL frame-binding mirror update, unavailable-state classification, cached specified-cubemap asset resolution, source TextureCube/HDR validation, static product-key source descriptors, and CPU-side specified-cubemap processing for yaw source selection, lower-hemisphere replacement, FP16-domain radiance scale metadata, average brightness, and UE5.7-shaped packed diffuse SH are implemented and validated with focused environment tests plus prior ShaderBake/catalog validation. Remaining Slice B-E work: GPU processed cubemap/SH publication, skybox rendering, diffuse SkyLight consumption, full shader consumer migration, proof tooling, runtime proof, and closure evidence. |
...
| Cubemap skybox / static specified-cubemap SkyLight | VTX-M08 | `in_progress` | Slice A data/plan state is validated. Slice B product-state/source-validation and CPU cubemap/SH processing groundwork is in progress and focused-test/shader validated, but no GPU processed cubemap or SH product is published yet. Remaining work: visual cubemap skybox background, GPU cubemap product publication, deterministic procedural-sky versus skybox selection in runtime, directional sun interaction proof, static SkyLight diffuse lighting, shader consumer migration, proof tooling, and closure evidence. Captured-scene SkyLight, real-time capture, cubemap blending, SkyLight occlusion, baked/static-lightmap integration, and broader reflection probes remain deferred. |
