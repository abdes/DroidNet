...
The producer-depth and unused-field repairs now have committed-code checkpoint
evidence in `grid-depth-*.json`, `grid-depth-negative.log` and
`grid-depth-forward-report.txt`. Finish the final contract/catalog/document
reconciliation before changing A's status. The active goal covers all EX07
slices; passing A will not close B–F or the overall goal.
