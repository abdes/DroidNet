...
The allocation query was rerun on 2026-09-23 through the versioned
[`ShadowAllocationRequirements_test.cpp`](../../../src/Oxygen/Vortex/Test/Lighting/ShadowAllocationRequirements_test.cpp).
It uses the backend's descriptor conversion with the same typeless/depth-target/
shader-resource flags as the conventional allocator, then calls
`GetResourceAllocationInfo` without creating shadow textures.

The [current query](../../../out/build-ninja/analysis/vortex/exposure-lightbench/ex07a/allocation-requirements-current.json)
...
Reproduce in either configuration after building `Oxygen.Vortex.LightingGpuAbi.Tests`:

```powershell
./out/build-ninja/bin/Debug/Oxygen.Vortex.LightingGpuAbi.Tests.exe -v=-1 --gtest_filter=LightingGpuAbiTest.NativeShadowAllocationRequirements --gtest_output=json:out/build-ninja/analysis/vortex/exposure-lightbench/ex07a/allocation-refresh-debug.json
```

The GTest case's `shadow_allocation_requirements` property contains the complete
JSON report. Debug and Release independently record the same allocation matrix.
The original unversioned allocation-query files are no longer available; this
versioned query supersedes them.
...
reproducible GPU gate. Before production D32 adoption, reproduce raster, masked
discard, sampling and full-renderer validation under the owning EX07-10/11 work.
The refreshed allocation query does not establish any of those rendering claims.
