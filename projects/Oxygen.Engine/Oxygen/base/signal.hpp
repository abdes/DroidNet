...
struct has_call_operator<
    F,
    void_t<decltype(&std::remove_reference<F>::type::operator())>>
    : std::true_type {};
...
struct object_pointer<
    T,
    std::enable_if_t<!trait::is_pointer_v<T> && !trait::is_weak_ptr_v<T>
...
std::enable_if_t<!trait::is_signal_v<std::decay_t<Arg>>, connection> connect(
    signal_base<Lockable, T...> &sig, Arg &&arg, Args &&...args) {
