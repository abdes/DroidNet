...
namespace {
  ...
  struct Item
  {
    ...
    explicit Item(std::string str)
      : value(std::move(str)), constructed(true)
    {
    }
...
namespace base::resources {
  ...
  protected:
    ...
    ResourceTableElementAccessTest() : table_(kItemType, kCapacity) {}
  ...
  TEST_F(ResourceTableElementAccessTest, ItemAt_GenerationMismatch) {
    ...
    ResourceHandle mismatched_handle = handle;
    mismatched_handle.NewGeneration();
    EXPECT_THROW(auto & _ = table_.ItemAt(mismatched_handle), std::invalid_argument);
  }
  ...
  protected:
    ...
    ResourceTableInsert() : table_(kItemType, kCapacity) {}
  ...
  protected:
    ...
    ResourceTableErase() : table_(kItemType, kCapacity) {}
  ...
  TEST_F(ResourceTableErase, EraseItemWhenContainsThrows) {
    ...
    // Manually invalidate the handle to simulate a condition where Contains would throw
  ...
  protected:
    void SetUp() override {
      ...
      std::ranges::for_each(
        handles_, [this](const auto& handle)
        {
          EXPECT_TRUE(table_.Contains(handle));
        });
    }
  ...
  TEST(ResourceTableTest, RandomInsertEraseAndDefragment) {
    ...
    for (size_t i = 0; i < kOperations; ++i) {
      if (dist(rng) < 5 && !handles.empty()) {
        // Erase a random item
        std::uniform_int_distribution<IndexType> erase_dist(0, static_cast<IndexType>(handles.size() - 1));
    ...
    table.Defragment([](const Item& a, const Item& b) { return a.value < b.value; });
