...
Composition::~Composition() noexcept
{
  ...
  while (!components.empty()) {
    ...
    try {
      ...
      DLOG_F(ERROR, "Exception caught while destructing components: %s", e.what());
    }
  }
}
...
Composition::Composition(const Composition& other)
  : pimpl_(other.pimpl_)
{
  DCHECK_NOTNULL_F(pimpl_, "Composition copy constructed from a moved composition!");
}
...
void Composition::ValidateDependencies(const TypeId comp_id, const std::span<const TypeId> dependencies)
{
...
void Composition::EnsureDependencies(const std::span<const TypeId> dependencies) const
{
...
auto Composition::ExpectExistingComponent(const TypeId id) const -> bool
{
  ...
  if (!pimpl_->component_index_.contains(id)) {
    DLOG_F(WARNING, "Attempt to remove or replace a component that is not there");
    return false;
  }
  return true;
}
...
auto Composition::AddComponentImpl(std::unique_ptr<Component> component) const -> Component&
{
  DCHECK_NOTNULL_F(pimpl_, "Composition used after having been moved!");
  DCHECK_NOTNULL_F(component, "Component must not be null");
  DCHECK_F(!pimpl_->component_index_.contains(component->TypeId()), "Component already exists");

  pimpl_->components_.emplace_back(std::move(component));
  const auto& entry = pimpl_->components_.back();
  pimpl_->component_index_[entry->TypeId()] = pimpl_->components_.size() - 1;

  return *entry;
}

auto Composition::ReplaceComponentImpl(const TypeId old_id, std::unique_ptr<Component> new_component) const -> Component&
{
  DCHECK_NOTNULL_F(pimpl_, "Composition used after having been moved!");
  DCHECK_NOTNULL_F(new_component, "Component must not be null");
  DCHECK_F(pimpl_->component_index_.contains(old_id), "Old component must exist");

  const auto new_id = new_component->TypeId();
...
void Composition::RemoveComponentImpl(const TypeId id, const bool update_indices) const
{
  DCHECK_NOTNULL_F(pimpl_, "Composition used after having been moved!");
  const auto index = pimpl_->component_index_.at(id);
  pimpl_->components_.erase(pimpl_->components_.begin() + static_cast<ComponentsCollection::difference_type>(index));
  pimpl_->component_index_.erase(id);

  if (update_indices) {
    const auto size = pimpl_->components_.size();
    for (size_t i = index; i < size; ++i) {
      pimpl_->component_index_[pimpl_->components_[i]->TypeId()] = i;
    }
  }
}
...
{
  DCHECK_NOTNULL_F(other.pimpl_, "Composition deep copied from a moved composition!");
  pimpl_ = std::make_shared<ComponentManager>();
  for (const auto& entry : other.pimpl_->components_) {
    if (const auto* comp = entry.get(); comp->IsCloneable()) {
      ...
      pimpl_->component_index_[clone->TypeId()] = pimpl_->components_.size() - 1;
    } else {
...
auto Composition::IsComponentRequired(const TypeId id) const -> bool
{
  ...
  for (const auto& comp : pimpl_->components_) {
    if (std::ranges::find(comp->Dependencies(), id) != comp->Dependencies().end()) {
      return true;
    }
  }
  return false;
...
void Composition::PrintComponents() const
{
  DCHECK_NOTNULL_F(pimpl_, "Composition used after having been moved!");
  ...
  std::cout << "> Object \"" << object_name << "\" has " << components_.size() << " components:\n";
  for (const auto& entry : components_) {
    std::cout << "   [" << entry->TypeId() << "] " << entry->TypeName();
    if (!entry->Dependencies().empty()) {
      ...
      for (size_t i = 0; i < entry->Dependencies().size(); ++i) {
        ...
        std::cout << dep_component.TypeName();
