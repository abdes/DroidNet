...
{
 public:
  void set_force_fail(const bool value) noexcept
  {
    force_fail_ = value;
  }
  ...
  auto write(const char* data, const std::streamsize size) -> std::basic_ostream<char>&
  {
    if (force_fail_) {
      stream_.setstate(std::ios::failbit);
    }
  ...
  [[nodiscard]] auto get_data() const -> std::string
  {
    return stream_.str();
  }
...
class FileStreamTest : public Test
{
 protected:
  ...
  void SetUp() override
  {
    ...
    sut_ = std::make_unique<FileStream<MockStream>>(
      test_path_,
