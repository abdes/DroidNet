...
{
 public:
  OXYGEN_API ~Component() override = default;

  // All components should implement proper copy and move semantics to handle
  // copying and moving as appropriate.
  OXYGEN_API Component(const Component&) = default;
  OXYGEN_API Component& operator=(const Component&) = default;
  OXYGEN_API Component(Component&&) = default;
  OXYGEN_API Component& operator=(Component&&) = default;
 ...
 protected:
  OXYGEN_API Component() = default;
  OXYGEN_API virtual void UpdateDependencies(const Composition& [[maybe_unused]] composition) { }

 private:
  friend class Composition;
  static constexpr auto ClassDependencies() -> std::span<const oxygen::TypeId> { return {}; }
  [[nodiscard]] virtual constexpr auto HasDependencies() const -> bool { return false; }
  [[nodiscard]] virtual constexpr auto Dependencies() const -> std::span<const oxygen::TypeId> { return ClassDependencies(); }
};
...
class Composition
{
 ...
 public:
  OXYGEN_API virtual ~Composition() noexcept;

  //! Copy constructor, make a shallow copy of the composition.
  OXYGEN_API Composition(const Composition&);

  //! Copy assignment operator, make a shallow copy of the composition.
  OXYGEN_API Composition& operator=(const Composition&);

  //! Moves the composition to the new object and leaves the original in an empty state.
  OXYGEN_API Composition(Composition&& other) noexcept;

  //! Moves the composition to the new object and leaves the original in an empty state.
  OXYGEN_API Composition& operator=(Composition&& other) noexcept;
  ...
  OXYGEN_API virtual auto begin() -> iterator;
  OXYGEN_API virtual auto end() -> iterator;
  OXYGEN_API virtual auto begin() const -> const_iterator;
  OXYGEN_API virtual auto end() const -> const_iterator;
  OXYGEN_API virtual auto cbegin() const -> const_iterator;
  OXYGEN_API virtual auto cend() const -> const_iterator;

  OXYGEN_API void PrintComponents() const;

 protected:
  OXYGEN_API Composition();
 ...
 private:
  OXYGEN_API static void ValidateDependencies(TypeId comp_id, std::span<const TypeId> dependencies);
  OXYGEN_API void EnsureDependencies(std::span<const TypeId> dependencies) const;
  OXYGEN_API [[nodiscard]] auto ExpectExistingComponent(TypeId id) const -> bool;
  OXYGEN_API [[nodiscard]] auto IsComponentRequired(TypeId id) const -> bool;
  OXYGEN_API [[nodiscard]] auto HasComponentImpl(TypeId id) const -> bool;
  OXYGEN_API [[nodiscard]] auto AddComponentImpl(std::unique_ptr<Component> component) const -> Component&;
  OXYGEN_API [[nodiscard]] auto ReplaceComponentImpl(TypeId old_id, std::unique_ptr<Component> new_component) const -> Component&;
  OXYGEN_API [[nodiscard]] auto GetComponentImpl(TypeId id) const -> Component&;
  OXYGEN_API void RemoveComponentImpl(TypeId id, bool update_indices = true) const;
  OXYGEN_API void DeepCopyComponentsFrom(const Composition& other);
