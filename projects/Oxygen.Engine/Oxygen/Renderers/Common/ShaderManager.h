...
#include "Oxygen/Renderers/Common/ShaderByteCode.h"
#include "Oxygen/Renderers/Common/Shaders.h"
...
class ShaderManager : public Mixin<ShaderManager, Curry<MixinNamed, const char*>::mixin, MixinInitialize, MixinShutdown>
{
 public:
  ...
  OXYGEN_API ~ShaderManager() override = default;
  ...
  OXYGEN_API [[nodiscard]] void Load();
  //! Persists shader bytecode and metadata to the archive file
  OXYGEN_API [[nodiscard]] void Save() const;
  //! Removes all shaders from the archive
  OXYGEN_API void Clear() noexcept;

  // Shader management
  //! Adds pre-compiled shader bytecode to the archive
  OXYGEN_API [[nodiscard]] bool AddCompiledShader(CompiledShader shader);
  //! Retrieves compiled shader bytecode by name
  OXYGEN_API [[nodiscard]] std::shared_ptr<IShaderByteCode> GetShaderBytecode(
    std::string_view unique_id) const;

  // State queries
  //! Checks if a shader exists in the archive
  OXYGEN_API [[nodiscard]] bool HasShader(std::string_view unique_id) const noexcept;
  //! Returns profiles of all shaders that need recompilation
  OXYGEN_API [[nodiscard]] std::vector<ShaderProfile> GetOutdatedShaders() const;
  //! Returns the total number of shaders in the archive
  OXYGEN_API [[nodiscard]] size_t GetShaderCount() const noexcept;

  // Update operations
  //! Compiles all shaders whose source files have changed
  OXYGEN_API [[nodiscard]] void UpdateOutdatedShaders();
  //! Forces recompilation of all shaders in the archive
  OXYGEN_API [[nodiscard]] bool RecompileAll();
