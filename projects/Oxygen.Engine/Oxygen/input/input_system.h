...
namespace oxygen::input {
  ...
  public:
    ...
    OXYGEN_API explicit InputSystem(Platform& platform);
    OXYGEN_API ~InputSystem() override;

    OXYGEN_MAKE_NON_COPYABLE(InputSystem);
    OXYGEN_MAKE_NON_MOVEABLE(InputSystem);

    OXYGEN_API void AddAction(const std::shared_ptr<Action>& action);
    OXYGEN_API void RemoveAction(const std::shared_ptr<Action>& action);
    OXYGEN_API void ClearAllActions();
    OXYGEN_API [[nodiscard]] auto GetActionByName(std::string_view name) const
      ->std::shared_ptr<Action>;

    OXYGEN_API void AddMappingContext(
      const std::shared_ptr<InputMappingContext>& context, int32_t priority);
    OXYGEN_API void RemoveMappingContext(
      const std::shared_ptr<InputMappingContext>& context);
    OXYGEN_API void ClearAllMappingContexts();
    OXYGEN_API [[nodiscard]] auto GetMappingContextByName(std::string_view name) const
      ->std::shared_ptr<InputMappingContext>;
    OXYGEN_API void ActivateMappingContext(
      const std::shared_ptr<InputMappingContext>& context);
    OXYGEN_API void DeactivateMappingContext(
      const std::shared_ptr<InputMappingContext>& context);

    OXYGEN_API void ProcessInput(const platform::InputEvent& event) override;
    OXYGEN_API void Update(
