...
ActionTriggerDown::ActionTriggerDown() {
  SetActuationThreshold(0.5F);
}
...
void ActionTriggerCombo::AddComboStep(std::shared_ptr<Action> action,
                                      const ActionStates completion_states,
                                      const float time_to_complete_seconds) {
  assert(action);
  if (action) {
    combo_steps_.push_back(
...
void ActionTriggerCombo::ClearComboSteps() {
  combo_steps_.clear();
}

void ActionTriggerCombo::AddComboBreaker(std::shared_ptr<Action> action,
                                         const ActionStates completion_states) {
  assert(action);
  if (action) {
...
void ActionTriggerCombo::ClearComboBreakers() {
  combo_breakers_.clear();
}
