...
.PARAMETER UiTests
generate only: instrument ImGui and demo applications (default: enabled).
Use -UiTests:$false for an uninstrumented build.
.PARAMETER Clean
...
try {
    ...
    $invalidOptions = if ($Command -eq 'configure') { @('Generator', 'DependencyBuild', 'WithTracy', 'UiTests', 'Clean') } else { @('Define') }
