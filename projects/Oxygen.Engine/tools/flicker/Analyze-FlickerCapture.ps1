<#
.SYNOPSIS
Runs the auto-exposure RenderDoc deep-dive analyzer for a flicker capture.
...
& $invokeScript `
  ...
  -UiScriptPath (Join-Path $PSScriptRoot 'AnalyzeRenderDocAutoExposureDeepDive.py') `
  -PassName 'AutoExposurePass' `
  -ReportPath "$outputStemFullPath.auto_exposure_deep_dive.txt"

Write-Output "Flicker analysis complete for $captureFullPath"
