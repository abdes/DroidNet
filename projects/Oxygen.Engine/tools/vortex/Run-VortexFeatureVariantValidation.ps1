<#
.SYNOPSIS
Runs the VTX-M06C feature-variant runtime validation flow.
#>
[CmdletBinding()]
param(
  [Parameter()]
...
$assertScript = Join-Path $PSScriptRoot 'Assert-VortexFeatureVariantProof.ps1'
$analysisScript = Join-Path $PSScriptRoot 'AnalyzeRenderDocVortexFeatureVariants.py'
...
if ([string]::IsNullOrWhiteSpace($Output)) {
  $Output = Join-Path $buildRoot 'analysis\vortex\m06c-feature-variants\feature-variants'
}
...
$debugArgs = @(
  ...
  '--frames', "$RunFrames",
  '--fps', "$Fps",
  '--feature-variant-proof-layout', 'true',
...
Invoke-VortexRenderDocAnalysis `
  ...
  -PassName 'VortexFeatureVariants' `
...
$allocationReportLines = @(
  ...
  'analysis_profile=vortex_feature_variants_allocation_churn'
...
Write-Output "VTX-M06C Feature-variant validation passed."
