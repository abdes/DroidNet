<#
.SYNOPSIS
Validates an existing VortexBasic occlusion RenderDoc capture and logs.
#>
[CmdletBinding()]
param(
...
if ([string]::IsNullOrWhiteSpace($ReportPath)) {
  $ReportPath = [System.IO.Path]::ChangeExtension(
    $captureFullPath,
    "$([System.IO.Path]::GetExtension($captureFullPath))_vortex_occlusion_report.txt")
}
...
Invoke-VortexRenderDocAnalysis `
  ...
  -UiScriptPath (Join-Path $repoRoot 'tools\vortex\AnalyzeRenderDocVortexOcclusion.py') `
  -PassName 'VortexOcclusion' `
...
if (@($runtimeLogLines | Select-String -Pattern "Parsed with-occlusion option = true").Count -eq 0) {
  throw "Runtime log does not prove --with-occlusion true: $runtimeLogFullPath"
}
if (@($runtimeLogLines | Select-String -Pattern "vtx\.occlusion\.enable' value='true'").Count -eq 0) {
  throw "Runtime log does not prove vtx.occlusion.enable was applied: $runtimeLogFullPath"
}
...
if (@($drawWriteCounts | Where-Object { $_ -ne 3 }).Count -ne 0) {
  throw "Runtime log draw metadata writes are not all 3: $($drawWriteCounts -join ',')"
}

Add-Content -LiteralPath $reportFullPath -Value @(
  'runtime_log_occlusion_enabled=true'
  'runtime_log_draw_metadata_count=3'
