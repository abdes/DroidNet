<#
.SYNOPSIS
Asserts VTX-M06C feature-variant proof artifacts.
#>
[CmdletBinding()]
param(
...
Assert-ReportValue -Report $captureReport -Key 'depth_products_present' -Expected 'true' -Label 'renderdoc'
Assert-ReportValue -Report $captureReport -Key 'shadow_products_present' -Expected 'true' -Label 'renderdoc'
Assert-ReportValue -Report $captureReport -Key 'scene_lighting_scope_count_match' -Expected 'true' -Label 'renderdoc'
Assert-ReportValue -Report $captureReport -Key 'reduced_variants_omitted_scene_lighting' -Expected 'true' -Label 'renderdoc'
Assert-ReportValue -Report $captureReport -Key 'no_volumetric_overrun' -Expected 'true' -Label 'renderdoc'
Assert-ReportValue -Report $captureReport -Key 'stage22_scene_lighting_only' -Expected 'true' -Label 'renderdoc'
Assert-ReportValue -Report $captureReport -Key 'stage3_before_stage9' -Expected 'true' -Label 'renderdoc'
Assert-ReportValue -Report $captureReport -Key 'stage9_before_stage12' -Expected 'true' -Label 'renderdoc'
Assert-ReportValue -Report $captureReport -Key 'stage12_before_composition' -Expected 'true' -Label 'renderdoc'
...
$layoutMatches = @(
  $runtimeLogLines | Select-String -Pattern 'Parsed feature variant proof layout option = true'
)
if ($layoutMatches.Count -eq 0) {
  throw "Runtime logs do not prove that MultiView ran with --feature-variant-proof-layout true: $($runtimeLogFullPaths -join ', ')"
}
...
foreach ($profile in $expectedProfiles) {
  $matches = @(
    $runtimeLogLines |
      Select-String -Pattern "Vortex\.FeatureVariantProof\.View .* profile=$profile "
  )
  if ($matches.Count -eq 0) {
    throw "Runtime logs do not prove feature-variant view profile '$profile': $($runtimeLogFullPaths -join ', ')"
  }
  $profileCounts[$profile] = $matches.Count
}

$reportLines = @(
  'analysis_result=success'
  'analysis_profile=vortex_feature_variants_assert'
  ...
  "depth_only_runtime_records=$($profileCounts['depth-only'])"
  "shadow_only_runtime_records=$($profileCounts['shadow-only'])"
  "no_environment_runtime_records=$($profileCounts['no-environment'])"
  "no_shadowing_runtime_records=$($profileCounts['no-shadowing'])"
  "no_volumetrics_runtime_records=$($profileCounts['no-volumetrics'])"
  "diagnostics_only_runtime_records=$($profileCounts['diagnostics-only'])"
