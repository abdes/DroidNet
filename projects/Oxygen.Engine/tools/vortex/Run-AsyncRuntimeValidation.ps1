<#
.SYNOPSIS
Runs the end-to-end Async runtime capture flow and initializes the baseline artifact pack.
#>
[CmdletBinding()]
param(
  [Parameter()]
  [string]$Output = 'build/artifacts/vortex/phase-4/async',
...
$captureStem = Join-Path $outputDirectory 'baseline_renderdoc'
$finalCapturePath = Join-Path $outputDirectory 'baseline_renderdoc.rdc'
$stdoutPath = Join-Path $outputDirectory 'baseline.stdout.log'
$stderrPath = Join-Path $outputDirectory 'baseline.stderr.log'
$buildLogPath = Join-Path $outputDirectory 'baseline.build.log'
$captureFilter = 'baseline_renderdoc*.rdc'
...
Write-Host "Behavior notes: $(Join-Path $outputDirectory 'baseline_behaviors.md')"
Write-Host "Validation report: $(Join-Path $outputDirectory 'baseline_renderdoc.rdc.validation.txt')"
