<#
.SYNOPSIS
Runs the VTX-M06B offscreen runtime validation flow.
#>
[CmdletBinding()]
param(
  [Parameter()]
...
$assertScript = Join-Path $PSScriptRoot 'Assert-VortexOffscreenProof.ps1'
$analysisScript = Join-Path $PSScriptRoot 'AnalyzeRenderDocVortexOffscreen.py'
...
if ([string]::IsNullOrWhiteSpace($Output)) {
  $Output = Join-Path $buildRoot 'analysis\vortex\m06b-offscreen\offscreen-proof'
}
...
$allocationReportLines = @(
  ...
  'analysis_profile=vortex_offscreen_allocation_churn'
...
Write-Output "VTX-M06B Offscreen validation passed."
