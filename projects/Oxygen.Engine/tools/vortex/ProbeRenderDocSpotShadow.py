"""Probe Vortex spot-shadow state from a RenderDoc capture."""
...
REPORT_SUFFIX = "_spot_shadow_probe.txt"
...
def append_descriptor_dump(report, controller, resource_names, label, used_descriptor):
    descriptor = used_descriptor.descriptor
    report.append(
    ...
    values_u32, values_f32 = dump_u32_f32(controller, descriptor, 192)
    if values_u32:
...
def build_report(controller, report: ReportWriter, capture_path: Path, report_path: Path):
    ...
    spot_draws = [
        ...
        for record in action_records
        if record.name == "ID3D12GraphicsCommandList::DrawInstanced()"
        and "Vortex.Stage12.SpotLight" in record.path
    ]

    report.append("analysis_profile=vortex_spot_shadow_probe")
    ...
    report.append("stage12_spot_draw_events={}".format(",".join(str(r.event_id) for r in spot_draws)))
    ...
    spot_draw = spot_draws[-1]
    controller.SetFrameEvent(spot_draw.event_id, True)
    state = controller.GetPipelineState()
    report.append("stage12_spot_event={}".format(spot_draw.event_id))
    ...
    report.append("stage12_spot_vertex_readonly_count={}".format(len(vertex_readonly)))
    report.append("stage12_spot_pixel_readonly_count={}".format(len(pixel_readonly)))
    for index, used_descriptor in enumerate(vertex_readonly):
        append_descriptor_dump(
            report, controller, resource_names,
            "stage12_spot_vertex_ro_{}".format(index), used_descriptor)
    for index, used_descriptor in enumerate(pixel_readonly):
        append_descriptor_dump(
            report, controller, resource_names,
            "stage12_spot_pixel_ro_{}".format(index), used_descriptor)

    outputs = state.GetOutputTargets()
    report.append("stage12_spot_output_count={}".format(len(outputs)))
    for index, descriptor in enumerate(outputs):
        ...
        if output_sample is None:
            continue
        report.append(
            "stage12_spot_output_{}={}|slice={}|dims={}x{}|min={}|max={}|center={}".format(
                index,
                output_sample["name"],
                output_sample["slice"],
                output_sample["width"],
