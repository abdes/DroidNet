...
| --- | --- | --- | --- |
| 1 | The migrated Vortex substrate builds and the Vortex-local smoke/regression surface runs. | ✓ VERIFIED | `cmake --build --preset windows-debug --target oxygen-vortex Oxygen.Vortex.LinkTest Oxygen.Vortex.RendererCapability.Tests Oxygen.Vortex.RendererCompositionQueue.Tests --parallel 4` passed, and `ctest --test-dir out/build-ninja -C Debug -R '^Oxygen\.Vortex\.' --output-on-failure` passed. |
...
| 7 | The reopened proof suite is stronger than the earlier sign-off. | ✓ VERIFIED | The final proof pack combined the Vortex build, the hardened `Oxygen.Vortex.LinkTest` boundary scan, the new Vortex-local regressions, and the final target-edge query on the same repaired tree. |
...
| `FOUND-03` | Vortex does not depend on `Oxygen.Renderer` through module, API-contract, runtime ownership, or bridge seams. | ✓ SATISFIED | The Vortex source seam scan remains clean, `Oxygen.Vortex.LinkTest` passes with the hermeticity scan active, and the final Ninja target query shows no `Oxygen.Renderer` edge. |
...
6. `cmake --build --preset windows-debug --target oxygen-vortex Oxygen.Vortex.LinkTest Oxygen.Vortex.RendererCapability.Tests Oxygen.Vortex.RendererCompositionQueue.Tests --parallel 4`
7. `ctest --test-dir out/build-ninja -C Debug -R '^Oxygen\.Vortex\.' --output-on-failure`
8. `ninja -C out/build-ninja -f build-Debug.ninja -t query bin/Debug/Oxygen.Vortex-d.dll`
