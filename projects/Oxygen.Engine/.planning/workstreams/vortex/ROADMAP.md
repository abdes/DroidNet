...
- [ ] **Phase 2: SceneTextures and SceneRenderer Shell** - Establish
...
> **Planning repaired 2026-04-13 after the `01-04` and `01-08` execute-time
> blockers:** the remaining Phase 1 set now reflects the real dependency order
> and the checker file-budget constraint. `01-04` lands only the Vortex-local
> prerequisite ABI bundle; `01-05` owns `Resources/*`; `01-08` now lands only
> the public step-`1.7` headers; and `01-10` now owns the step-`1.6` pass-base
> migration together with the later-wave root contracts, stripped orchestrator,
> and final post-orchestrator `FOUND-03` dependency-edge proof. Gap plans
> `01-12` and `01-13` then removed the last `FramePlanBuilder` source/API seam,
> added a Vortex-side hermeticity guard, and `01-14` re-closed Phase 1 with
> Vortex-only build/test/hermeticity/dependency-edge proof.
...
Plans:
- [ ] 02-01-PLAN.md — Seed Wave 0 proof surfaces and close the shell vocabulary seams
- [ ] 02-02-PLAN.md — Implement the full `SceneTextures` contract and routing slot
- [ ] 02-03-PLAN.md — Wire the `SceneRenderer` shell and lock the bootstrap extent contract
- [ ] 02-04-PLAN.md — Land renderer-owned publication plus thin resolve/cleanup hooks
...
Plans:
- [ ] 03-01: Establish Vortex shader foundation and catalog entries
- [ ] 03-02: Implement InitViews orchestration
- [ ] 03-03: Implement depth prepass and early velocity
- [ ] 03-04: Implement base pass and scene-texture promotion
- [ ] 03-05: Implement deferred lighting and GBuffer debug validation
...
| 1. Substrate Migration | 13/13 | Complete | 2026-04-13 |
| 2. SceneTextures and SceneRenderer Shell | 0/3 | Not started | - |
| 3. Deferred Core | 0/5 | Not started | - |
