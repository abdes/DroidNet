...
| Geometry and all existing material slots | `property-inspector.md` Geometry table and section 7.6; `asset-primitives.md`; `content-pipeline.md` section 19 | Every mesh-provided material slot supports independent per-instance assignment and clearing; clearing restores the mesh material. Slot creation/removal and topology editing are excluded. All selected geometry identities synchronize and cook. The historical built-in set is Cube, SubdividedCube, Sphere, IcoSphere/GeodesicSphere, Plane, Cylinder, Cone, Quad, Torus and ArrowGizmo; canonical picker choices remain under individual M08 review. |
