...
| Basic perspective camera | `property-inspector.md` PerspectiveCamera table | Approved pose, vertical FOV in degrees, aspect and near/far in metres synchronize and cook; runtime loads the explicitly selected authored camera faithfully, independently of editor navigation. Physical-camera authoring is deferred. Aspect-fitting policy remains an individual decision. |
