...
| Geometry and all existing material slots | `property-inspector.md` Geometry table and section 7.6; `asset-primitives.md`; `content-pipeline.md` section 19 | Every mesh-provided material slot supports independent per-instance assignment and clearing; clearing restores the mesh material. Slot creation/removal and topology editing are excluded. Approved creation palette: Cube, Sphere, Capsule, Cylinder, Cone, Plane, Quad, IcoSphere, Torus, plus SubdividedCube under Advanced. All ten must synchronize, cook and render. ArrowGizmo is internal; useful GeodesicSphere references migrate to IcoSphere and the alias is removed. Primitive defaults remain an individual decision. |
